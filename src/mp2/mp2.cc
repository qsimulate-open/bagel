...
void MP2::compute() {
  ...
  shared_ptr<const View2<double>> ocoeff = ref_->coeff()->slice(ncore_, ncore_+nocc);
  shared_ptr<const View2<double>> vcoeff = ref_->coeff()->slice(ncore_+nocc, ncore_+nocc+nvirt);
