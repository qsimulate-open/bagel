...
#ifndef __BAGEL_LONDON_RELMOFILE_LONDON_H
#define __BAGEL_LONDON_RELMOFILE_LONDON_H
...
class RelMOFile_London {
  protected:
    ...
    std::array<std::list<std::shared_ptr<RelDFHalf_London>>,2> half_complex_coulomb_;
    std::array<std::list<std::shared_ptr<RelDFHalf_London>>,2> half_complex_gaunt_;

  public:
    RelMOFile_London(const std::shared_ptr<const Geometry_London>, std::shared_ptr<const ZMatrix>, const bool gaunt, const bool breit);
...
//  const std::complex<double>& mo1e(const std::bitset<2>& b, const size_t i) const { return mo1e_.at(b)->data(i); }
    ...
    std::array<std::list<std::shared_ptr<RelDFHalf_London>>,2> half_complex_coulomb() const { return half_complex_coulomb_; }
    std::array<std::list<std::shared_ptr<RelDFHalf_London>>,2> half_complex_gaunt() const { return half_complex_gaunt_; }
    ...
    static std::unordered_map<std::bitset<2>, std::shared_ptr<const RelDFFull_London>>
      ...
      for (size_t t = 0; t != 4; ++t) {
        std::list<std::shared_ptr<RelDFFull_London>> dffull;
        for (auto& i : half[t/2])
          dffull.push_back(std::make_shared<RelDFFull_London>(i, rocoeff[t%2], iocoeff[t%2]));
        DFock_London::factorize(dffull);
...
class RelJop_London : public RelMOFile_London {
  protected:
  ...
  public:
    RelJop_London(const std::shared_ptr<const Geometry_London> geo, const int c, const int d, std::shared_ptr<const ZMatrix> coeff, const bool gaunt, const bool breit)
      : RelMOFile_London(geo, coeff, gaunt, breit) { init(c, d); }
};


#if 0
class RelHtilde : public ZHtilde_Base, public RelMOFile_London {
  protected:
  ...
  public:
    RelHtilde(const std::shared_ptr<const Reference> b, const int c, const int d, std::shared_ptr<const ZMatrix> h1, std::shared_ptr<const ZMatrix> h2)
      : ZHtilde_Base(h1, std::move(h2)), RelMOFile_London(b) {
