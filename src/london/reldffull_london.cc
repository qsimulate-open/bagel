...
RelDFFull_London::RelDFFull_London(shared_ptr<const RelDFHalf_London> df, array<shared_ptr<const Matrix>,4> rcoeff, array<shared_ptr<const Matrix>,4> icoeff) : RelDFBase(*df) {
  ...
  basis_ = df->basis();
  if (basis_.size() != 1)
    throw logic_error("RelDFFull_London should be called with basis_.size() == 1");
...
RelDFFull_London::RelDFFull_London(const RelDFFull_London& o) : RelDFBase(o.cartesian_) {
  basis_ = o.basis_;
...
void RelDFFull_London::ax_plus_y(complex<double> a, const RelDFFull_London& o) {
  if (imag(a) == 0.0) {
...
void RelDFFull_London::scale(complex<double> a) {
  if (imag(a) == 0.0) {
    const double fac = real(a);
    dffull_[0]->scale(fac);
    dffull_[1]->scale(fac);
...
shared_ptr<ZMatrix> RelDFFull_London::form_4index_1fixed(shared_ptr<const RelDFFull_London> a, const double fac, const int i) const {
#ifndef NDEBUG
...
/*
shared_ptr<RelDFFull_London> RelDFFull_London::apply_2rdm(shared_ptr<const ZRDM<2>> inp) const {
