...
// Filename: zharrison_rdm_london.cc
...
void ZHarrison_London::compute_rdm12() {

  // for one-body RDM
  auto space1 = make_shared<RelSpace_London>(norb_, nele_-1);
  rdm1_.clear();
  rdm2_.clear();
    ...
    for (int i = 0; i != 4; ++i) {
      for (int j = 0; j != 4; ++j) {
        ...
        if (rdm2_[istate].find(target) != rdm2_[istate].end()) {
          ...
          throw logic_error("debug .. ZHarrison_London::compute_rdm12()");
#endif
        }
      }
    }
