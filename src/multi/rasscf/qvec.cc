...
RASQvec::RASQvec(const int n, const int m, shared_ptr<const Matrix> coeff, const size_t nclosed, shared_ptr<const RASCI> rasci, shared_ptr<const RDM<2>> rdm)
 : Matrix(n,m) {

  assert(n == coeff->mdim());

  // one index transformed integrals (active)
  shared_ptr<const DFHalfDist> half = rasci->jop()->mo2e_1ext();
