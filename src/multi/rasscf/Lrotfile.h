...
// Filename: multi/rasscf/largerotfile.h
...
#ifndef __BAGEL_RASSCF_LROTFILE_H
#define __BAGEL_RASSCF_LROTFILE_H
...
class LargeRotationMatrix {
  ...
  public:
    LargeRotationMatrix(const int iclos, const int iact, const int ivirt)
     : nclosed_(iclos), nact_(iact), nvirt_(ivirt), size_(iclos*iact+iclos*ivirt+iact*ivirt), data_(new DataType[size_]) {
      zero();
    }
    LargeRotationMatrix(const LargeRotationMatrix& o) : nclosed_(o.nclosed_), nact_(o.nact_), nvirt_(o.nvirt_), size_(o.size_), data_(new DataType[o.size_]) {
      *this = o;
    }
    LargeRotationMatrix(std::shared_ptr<const LargeRotationMatrix> o)
      : nclosed_(o->nclosed_), nact_(o->nact_), nvirt_(o->nvirt_), size_(o->size_), data_(new DataType[o->size_]) {
      *this = *o;
    }
    LargeRotationMatrix(std::shared_ptr<const Matrix_base<DataType>> o, const int iclos, const int iact, const int ivirt)
      : nclosed_(iclos), nact_(iact), nvirt_(ivirt), size_(iclos*iact+iclos*ivirt+iact*ivirt), data_(new DataType[size_]) {
    ...
    std::shared_ptr<LargeRotationMatrix<DataType>> clone() const {
      return std::make_shared<LargeRotationMatrix<DataType>>(nclosed_, nact_, nvirt_);
    }
    std::shared_ptr<LargeRotationMatrix<DataType>> copy() const {
      return std::make_shared<LargeRotationMatrix<DataType>>(*this);
    }

    // overloaded operators
    LargeRotationMatrix<DataType> operator+(const LargeRotationMatrix<DataType>& o) const { LargeRotationMatrix<DataType> out(*this); out.ax_plus_y(1.0, o); return out; }
    LargeRotationMatrix<DataType> operator-(const LargeRotationMatrix<DataType>& o) const { LargeRotationMatrix<DataType> out(*this); out.ax_plus_y(-1.0, o); return out; }
    LargeRotationMatrix<DataType>& operator+=(const LargeRotationMatrix<DataType>& o) { ax_plus_y(1.0, o); return *this; }
    LargeRotationMatrix<DataType>& operator-=(const LargeRotationMatrix<DataType>& o) { ax_plus_y(-1.0, o); return *this; }
    LargeRotationMatrix<DataType>& operator*=(const DataType a) { scale(a); return *this; }
    LargeRotationMatrix<DataType>& operator/=(const LargeRotationMatrix<DataType>& o) { for (int i = 0; i != size(); ++i) data(i)/= o.data(i); return *this; }
    LargeRotationMatrix<DataType>& operator*=(const LargeRotationMatrix<DataType>& o) { for (int i = 0; i != size(); ++i) data(i)*= o.data(i); return *this; }
    LargeRotationMatrix<DataType> operator/(const LargeRotationMatrix<DataType>& o) const { LargeRotationMatrix<DataType> out(*this); return out /= o; }
    LargeRotationMatrix<DataType> operator*(const LargeRotationMatrix<DataType>& o) const { LargeRotationMatrix<DataType> out(*this); return out *= o; }
    LargeRotationMatrix<DataType>& operator=(const LargeRotationMatrix<DataType>& o) { std::copy_n(o.data(), size(), data());  return *this; }
    ...
    // returns dot product
    DataType dot_product(const LargeRotationMatrix<DataType>& o) const { return blas::dot_product(data(), size_, o.data()); }
    DataType dot_product(const std::shared_ptr<const LargeRotationMatrix<DataType>> o) const { return dot_product(*o); }
    // scale function
    ...
    // daxpy added to self
    void ax_plus_y(const DataType& a, const LargeRotationMatrix& o) { std::transform(o.data(), o.data()+size_, data(), data(), [&a](DataType p, DataType q) { return p*a+q; }); }
    void ax_plus_y(const DataType& a, const std::shared_ptr<const LargeRotationMatrix> o) { ax_plus_y(a, *o); }

    // orthogonalize to the liset of LargeRotationMatrix's
    double orthog(std::list<std::shared_ptr<const LargeRotationMatrix<DataType>>> c) {
...
using LargeRotFile = LargeRotationMatrix<double>;
