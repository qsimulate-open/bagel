...
shared_ptr<const RASRotFile> RASBFGS::compute_denom(shared_ptr<const Matrix> cfock, shared_ptr<const Matrix> afock, shared_ptr<const Matrix> qxr, shared_ptr<const Matrix> rdm1, shared_ptr<const Matrix> mcfock) const {
  cout << "denom started" << endl;
  auto out = make_shared<RASRotFile>(nclosed_, nact_, nvirt_, ras_);
