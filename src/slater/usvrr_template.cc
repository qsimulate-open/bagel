...
void SlaterBatch::perform_USVRR3() {
  ...
  if (ang == 66560) { // (2, 1, 0, 0)
    ...
    for (int j = 0; j != screening_size_; ++j) {
      ...
      const double wr0 = weights_[offset] * roots_[offset];
      const double wr1 = weights_[offset + 1] * roots_[offset + 1];
      const double wr2 = weights_[offset + 2] * roots_[offset + 2];
      {
        double* current_data = &data_[data_offset_ii];
        const double scale0 = coeff_[ii] * (weights_[offset    ] - wr0);
        const double scale1 = coeff_[ii] * (weights_[offset + 1] - wr1);
        const double scale2 = coeff_[ii] * (weights_[offset + 2] - wr2);
    ...
    for (int j = 0; j != screening_size_; ++j) {
      ...
      const int ii3 = 3 * ii;
      {
        ...
        {
          double* current_data = &data_[data_offset_ii];
          const double s0 = coeff_[ii] * (weights_[offset    ] - wr0);
          const double s1 = coeff_[ii] * (weights_[offset + 1] - wr1);
          const double s2 = coeff_[ii] * (weights_[offset + 2] - wr2);
