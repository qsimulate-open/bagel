...
";
  ...
  for (int rank = 3; rank != 14; ++rank) {
    ss << "void SlaterBatch::perform_USVRR" << rank << "() {" << endl;
    ...
    ss << "    double* current_data2 = &data2_[data_offset_ii];" << endl;
    ss << "" << endl;
    ...
    ss << "    }   " << endl;
    ss << "    cix.scale_data_t(workx2, wt, coeffy_[ii]);" << endl;
    ...
    ss << "          const int offsetz = " << rank << " * (amax1_ * iz + jz);" << endl;
    ss << "          for (int jy = 0; jy <= amax_ - jz; ++jy) {" << endl;
    ss << "            const int offsety = " << rank << " * (amax1_ * iy + jy);" << endl;
    ss << "            const int jyz = amax1_ * (jy + amax1_ * jz);" << endl;
    ss << "            for (int i = 0; i != " << rank << "; ++i) {" << endl;
    ss << "              iyiz[i] = worky[offsety + i] * workz[offsetz + i]; " << endl;
    ss << "            }   " << endl;
    ...
    ss << "                const int offsetx = " << rank << " * (amax1_ * ix + jx);" << endl;
    ...
    ss << "                current_data2[ijposition] = 0.0;" << endl;
    ss << "                for (int i = 0; i != " << rank << "; ++i) {" << endl;
    ss << "                  current_data[ijposition]  += iyiz[i] * workx[offsetx + i]; " << endl;
    ss << "                  current_data2[ijposition] += iyiz[i] * workx2[offsetx + i]; " << endl;
    ss << "                }   " << endl;
    ss << "              }   " << endl;
    ss << "            }   " << endl;
    ss << "          }   " << endl;
    ...
    ss << "  stack_->release(worksize, workx2);" << endl;
