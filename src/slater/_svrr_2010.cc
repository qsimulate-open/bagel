...
void SVRRList::_svrr_2010(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 3; ++t)
    data_[12+t] = C00_[t] * data_[9+t] + B00_[t];

  for (int t = 0; t != 3; ++t)
    data_[15+t] = C00_[t] * data_[12+t] + B10_[t] * data_[9+t] + B00_[t] * data_[3+t];
}
