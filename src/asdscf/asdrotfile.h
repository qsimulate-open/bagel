...
class ASDRotationMatrix {
  ...
  public:
    ASDRotationMatrix(const int iclos, const int iact, const int ivirt)
     : nclosed_(iclos), nact_(iact), nvirt_(ivirt), size_(iclos*iact+iclos*ivirt+iact*ivirt), data_(new DataType[size_]) {
      zero();
    }
    ASDRotationMatrix(const ASDRotationMatrix& o) : nclosed_(o.nclosed_), nact_(o.nact_), nvirt_(o.nvirt_), size_(o.size_), data_(new DataType[o.size_]) {
      *this = o;
    }
    ASDRotationMatrix(std::shared_ptr<const ASDRotationMatrix> o)
      : nclosed_(o->nclosed_), nact_(o->nact_), nvirt_(o->nvirt_), size_(o->size_), data_(new DataType[o->size_]) {
      *this = *o;
    }
    ASDRotationMatrix(std::shared_ptr<const Matrix_base<DataType>> o, const int iclos, const int iact, const int ivirt)
      : nclosed_(iclos), nact_(iact), nvirt_(ivirt), size_(iclos*iact+iclos*ivirt+iact*ivirt), data_(new DataType[size_]) {
    ...
    std::shared_ptr<ASDRotationMatrix<DataType>> clone() const {
      return std::make_shared<ASDRotationMatrix<DataType>>(nclosed_, nact_, nvirt_);
    }
    std::shared_ptr<ASDRotationMatrix<DataType>> copy() const {
      return std::make_shared<ASDRotationMatrix<DataType>>(*this);
    }

    // overloaded operators
    ASDRotationMatrix<DataType> operator+(const ASDRotationMatrix<DataType>& o) const { ASDRotationMatrix<DataType> out(*this); out.ax_plus_y(1.0, o); return out; }
    ASDRotationMatrix<DataType> operator-(const ASDRotationMatrix<DataType>& o) const { ASDRotationMatrix<DataType> out(*this); out.ax_plus_y(-1.0, o); return out; }
    ASDRotationMatrix<DataType>& operator+=(const ASDRotationMatrix<DataType>& o) { ax_plus_y(1.0, o); return *this; }
    ASDRotationMatrix<DataType>& operator-=(const ASDRotationMatrix<DataType>& o) { ax_plus_y(-1.0, o); return *this; }
    ASDRotationMatrix<DataType>& operator*=(const DataType a) { scale(a); return *this; }
    ASDRotationMatrix<DataType>& operator/=(const ASDRotationMatrix<DataType>& o) { for (int i = 0; i != size(); ++i) data(i)/= o.data(i); return *this; }
    ASDRotationMatrix<DataType>& operator*=(const ASDRotationMatrix<DataType>& o) { for (int i = 0; i != size(); ++i) data(i)*= o.data(i); return *this; }
    ASDRotationMatrix<DataType> operator/(const ASDRotationMatrix<DataType>& o) const { ASDRotationMatrix<DataType> out(*this); return out /= o; }
    ASDRotationMatrix<DataType> operator*(const ASDRotationMatrix<DataType>& o) const { ASDRotationMatrix<DataType> out(*this); return out *= o; }
    ASDRotationMatrix<DataType>& operator=(const ASDRotationMatrix<DataType>& o) { std::copy_n(o.data(), size(), data());  return *this; }
    ...
    // returns dot product
    DataType dot_product(const ASDRotationMatrix<DataType>& o) const { return blas::dot_product(data(), size_, o.data()); }
    DataType dot_product(const std::shared_ptr<const ASDRotationMatrix<DataType>> o) const { return dot_product(*o); }
    // scale function
    ...
    // daxpy added to self
    void ax_plus_y(const DataType& a, const ASDRotationMatrix& o) { std::transform(o.data(), o.data()+size_, data(), data(), [&a](DataType p, DataType q) { return p*a+q; }); }
    void ax_plus_y(const DataType& a, const std::shared_ptr<const ASDRotationMatrix> o) { ax_plus_y(a, *o); }

    // orthogonalize to the liset of ASDRotationMatrix's
    double orthog(std::list<std::shared_ptr<const ASDRotationMatrix<DataType>>> c) {
...
using ASDRotFile = ASDRotationMatrix<double>;
