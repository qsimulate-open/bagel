  ...
  // jvec_ = J^{-1/2} (D|ij) Gamma_ij,kl
  {
    ...
    rdm2all.fill(0.0);
    {
      ...
      for (int i = 0; i != nact; ++i) {
        for (int j = 0; j != nact; ++j) {
          for (int k = 0; k != nact; ++k) {
            copy_n(rdm->data()+nact*(k+nact*(j+nact*i)), nact, rdm2all.data()+nclosed+nocc*(k+nclosed+nocc*(j+nclosed+nocc*(i+nclosed))));
          }
        }
      }
      // active-closed
      for (int i = 0; i != nclosed; ++i) {
        for (int j = 0; j != nact; ++j) {
          for (int k = 0; k != nact; ++k) {
            rdm2all(i, i, k+nclosed, j+nclosed) += rdm1_av->element(k,j) * 2.0;
            rdm2all(k+nclosed, j+nclosed, i, i) += rdm1_av->element(k,j) * 2.0;
            rdm2all(i, k+nclosed, j+nclosed, i) -= rdm1_av->element(k,j);
            rdm2all(k+nclosed, i, i, j+nclosed) -= rdm1_av->element(k,j);
          }
        }
      }
    }
    jvec_ = in->apply_2rdm(rdm2all)->apply_J();
    rdm2_all_ = make_shared<RDM<2>>(move(rdm2all));
#if 0
