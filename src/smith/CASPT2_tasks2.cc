  ...
  for (auto& a4 : *range_[2]) {
    for (auto& a2 : *range_[2]) {
      for (auto& c1 : *range_[0]) {
        ...
        // tensor label: I55
        std::unique_ptr<double[]> i1data = in(1)->get_block(c1, a2, c5, a4);
        std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(c1, a2, c5, a4)]);
        sort_indices<3,1,0,2,0,1,1,1>(i1data, i1data_sorted, c1.size(), a2.size(), c5.size(), a4.size());
...
void Task53::Task_local::compute() {
  ...
  const Index a4 = b(3);
  // tensor label: I55
  std::unique_ptr<double[]> odata = out()->move_block(c1, a2, c5, a4);
  {
    // tensor label: t2
    std::unique_ptr<double[]> i0data = in(0)->get_block(c1, a2, c5, a4);
    sort_indices<0,1,2,3,1,1,-4,1>(i0data, odata, c1.size(), a2.size(), c5.size(), a4.size());
  }
  out()->put_block(odata, c1, a2, c5, a4);
}
...
void Task57::Task_local::compute() {
  ...
  for (auto& c3 : *range_[0]) {
    for (auto& a2 : *range_[2]) {
      for (auto& c1 : *range_[0]) {
        ...
        // tensor label: I59
        std::unique_ptr<double[]> i1data = in(1)->get_block(c1, a2, c3, a5);
        std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(c1, a2, c3, a5)]);
        sort_indices<2,1,0,3,0,1,1,1>(i1data, i1data_sorted, c1.size(), a2.size(), c3.size(), a5.size());
  ...
  sort_indices<0,1,1,1,1,1>(odata_sorted, odata, a4.size(), a5.size());
  out()->put_block(odata, a4, a5);
}
...
void Task58::Task_local::compute() {
  ...
  const Index a5 = b(3);
  // tensor label: I59
  std::unique_ptr<double[]> odata = out()->move_block(c1, a2, c3, a5);
  {
    // tensor label: t2
    std::unique_ptr<double[]> i0data = in(0)->get_block(c1, a2, c3, a5);
    sort_indices<0,1,2,3,1,1,4,1>(i0data, odata, c1.size(), a2.size(), c3.size(), a5.size());
  }
  out()->put_block(odata, c1, a2, c3, a5);
}
