...
#include <src/smith/CAS_test_tasks19.h>
  ...
  for (auto& x0 : *range_[1]) {
    for (auto& c2 : *range_[0]) {
      for (auto& x1 : *range_[1]) {
        ...
        sort_indices<0,2,3,1,0,1,1,1>(i0data, i0data_sorted, x0.size(), a1.size(), c2.size(), x1.size());
        // tensor label: I957
        ...
        sort_indices<2,3,0,1,0,1,1,1>(i1data, i1data_sorted, x1.size(), x2.size(), x0.size(), c2.size());
        dgemm_("T", "N", a1.size(), x2.size(), x1.size()*x0.size()*c2.size(),
               1.0, i0data_sorted, x1.size()*x0.size()*c2.size(), i1data_sorted, x1.size()*x0.size()*c2.size(),
  ...
  for (auto& x3 : *range_[1]) {
    for (auto& x4 : *range_[1]) {
      for (auto& x5 : *range_[1]) {
        ...
        sort_indices<3,1,0,2,4,5,0,1,1,1>(i1data, i1data_sorted, x5.size(), x4.size(), x1.size(), x3.size(), x2.size(), x0.size());
    ...
    for (auto& a1 : *range_[2]) {
      for (auto& c2 : *range_[0]) {
        for (auto& x1 : *range_[1]) {
          ...
          sort_indices<2,4,5,0,1,3,0,1,1,1>(i1data, i1data_sorted, x1.size(), x2.size(), x0.size(), c3.size(), a1.size(), c2.size());
          dgemm_("T", "N", 1, x2.size()*c3.size(), x1.size()*x0.size()*a1.size()*c2.size(),
                 1.0, i0data_sorted, x1.size()*x0.size()*a1.size()*c2.size(), i1data_sorted, x1.size()*x0.size()*a1.size()*c2.size(),
                 1.0, odata_sorted, 1);
        }
      }
    }
    ...
    for (auto& a2 : *range_[2]) {
      for (auto& x0 : *range_[1]) {
        for (auto& x1 : *range_[1]) {
          ...
          sort_indices<5,4,2,1,0,3,0,1,1,1>(i1data, i1data_sorted, x2.size(), x1.size(), x0.size(), c3.size(), a2.size(), c1.size());
          dgemm_("T", "N", 1, x2.size()*c3.size(), x1.size()*x0.size()*a2.size()*c1.size(),
...
void Task920::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata = out()->move_block(a1, a3);
  {
    ...
    sort_indices<1,0,1,1,1,1>(i0data, odata, a3.size(), a1.size());
  }
...
void Task921::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata_sorted(new double[out()->get_size(a3, a1)]);
  std::fill_n(odata_sorted.get(), out()->get_size(a3, a1), 0.0);
  for (auto& x0 : *range_[1]) {
    for (auto& c2 : *range_[0]) {
      for (auto& x1 : *range_[1]) {
        // tensor label: t2
        ...
        sort_indices<0,3,1,2,0,1,1,1>(i1data, i1data_sorted, x0.size(), x1.size(), a3.size(), c2.size());
        dgemm_("T", "N", a1.size(), a3.size(), x0.size()*x1.size()*c2.size(),
               1.0, i0data_sorted, x0.size()*x1.size()*c2.size(), i1data_sorted, x0.size()*x1.size()*c2.size(),
               1.0, odata_sorted, a1.size());
      }
    }
  }
  sort_indices<1,0,1,1,1,1>(odata_sorted, odata, a1.size(), a3.size());
    ...
    for (auto& a1 : *range_[2]) {
      for (auto& c2 : *range_[0]) {
        for (auto& x1 : *range_[1]) {
          ...
          sort_indices<1,5,2,0,3,4,0,1,1,1>(i1data, i1data_sorted, x1.size(), x0.size(), c2.size(), a4.size(), c3.size(), a1.size());
          dgemm_("T", "N", 1, a4.size()*c3.size(), x1.size()*x0.size()*c2.size()*a1.size(),
    ...
    for (auto& a2 : *range_[2]) {
      for (auto& x0 : *range_[1]) {
        for (auto& x1 : *range_[1]) {
          ...
          sort_indices<2,5,1,0,3,4,0,1,1,1>(i1data, i1data_sorted, x1.size(), x0.size(), c1.size(), a4.size(), c3.size(), a2.size());
          dgemm_("T", "N", 1, a4.size()*c3.size(), x1.size()*x0.size()*c1.size()*a2.size(),
  ...
  for (auto& c1 : *range_[0]) {
    for (auto& x0 : *range_[1]) {
      for (auto& x1 : *range_[1]) {
        ...
        sort_indices<3,2,1,0,0,1,1,1>(i1data, i1data_sorted, x2.size(), x1.size(), x0.size(), c1.size());
        dgemm_("T", "N", a2.size(), x2.size(), x1.size()*x0.size()*c1.size(),
  ...
  for (auto& x0 : *range_[1]) {
    for (auto& a1 : *range_[2]) {
      for (auto& x1 : *range_[1]) {
        ...
        sort_indices<0,3,2,1,0,1,1,1>(i1data, i1data_sorted, x0.size(), x2.size(), x1.size(), a1.size());
        dgemm_("T", "N", a2.size(), x2.size(), x0.size()*x1.size()*a1.size(),
...
void Task944::Task_local::compute() {
  ...
  {
    // tensor label: I1010
    std::unique_ptr<double[]> i0data = in(0)->get_block(c3, c1);
    sort_indices<0,1,1,1,1,1>(i0data, odata, c3.size(), c1.size());
  }
...
void Task945::Task_local::compute() {
  ...
  // tensor label: I1010
  std::unique_ptr<double[]> odata = out()->move_block(c3, c1);
  std::unique_ptr<double[]> odata_sorted(new double[out()->get_size(c3, c1)]);
  std::fill_n(odata_sorted.get(), out()->get_size(c3, c1), 0.0);
  for (auto& a2 : *range_[2]) {
    for (auto& x0 : *range_[1]) {
      for (auto& x1 : *range_[1]) {
        // tensor label: t2
        ...
        sort_indices<2,1,0,3,0,1,1,1>(i1data, i1data_sorted, x1.size(), x0.size(), a2.size(), c3.size());
        dgemm_("T", "N", c1.size(), c3.size(), x1.size()*x0.size()*a2.size(),
               1.0, i0data_sorted, x1.size()*x0.size()*a2.size(), i1data_sorted, x1.size()*x0.size()*a2.size(),
               1.0, odata_sorted, c1.size());
      }
    }
  }
  sort_indices<1,0,1,1,1,1>(odata_sorted, odata, c1.size(), c3.size());
  out()->put_block(odata, c3, c1);
}
