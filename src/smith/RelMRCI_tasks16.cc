...
void Task773::Task_local::compute() {
  ...
  const Index x1 = b(3);
  // tensor label: I245
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(a1, a3, x0, x1);
...
void Task777::Task_local::compute() {
  ...
  const Index x0 = b(3);
  // tensor label: I1378
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(a1, c4, x3, x0);
...
void Task779::Task_local::compute() {
  ...
  const Index x0 = b(3);
  // tensor label: I1381
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(a3, c4, x3, x0);
...
void Task784::Task_local::compute() {
  ...
  for (auto& x5 : *range_[1]) {
    for (auto& x4 : *range_[1]) {
      ...
      zgemm3m_("T", "N", a3.size()*c2.size(), a1.size()*x0.size(), x5.size()*x4.size(),
...
void Task787::Task_local::compute() {
  ...
  for (auto& x5 : *range_[1]) {
    for (auto& x4 : *range_[1]) {
      ...
      // tensor label: I1393
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(a3, x5, x0, x4);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(a3, x5, x0, x4)]);
      sort_indices<1,3,0,2,0,1,1,1>(i1data, i1data_sorted, a3.size(), x5.size(), x0.size(), x4.size());
      zgemm3m_("T", "N", a1.size()*c2.size(), a3.size()*x0.size(), x5.size()*x4.size(),
             1.0, i0data_sorted, x5.size()*x4.size(), i1data_sorted, x5.size()*x4.size(),
             1.0, odata_sorted, a1.size()*c2.size());
    }
  }
...
void Task788::Task_local::compute() {
  ...
  const Index x4 = b(3);
  // tensor label: I1393
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(a3, x5, x0, x4);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(a3, x5, x0, x4)]);
  ...
  for (auto& x3 : *range_[1]) {
    for (auto& x2 : *range_[1]) {
      for (auto& x1 : *range_[1]) {
        ...
        zgemm3m_("T", "N", x5.size()*x0.size()*x4.size(), a3.size(), x3.size()*x2.size()*x1.size(),
               1.0, i0data_sorted, x3.size()*x2.size()*x1.size(), i1data_sorted, x3.size()*x2.size()*x1.size(),
...
void Task790::Task_local::compute() {
  ...
  for (auto& x1 : *range_[1]) {
    for (auto& c4 : *range_[0]) {
      ...
      // tensor label: I1402
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(a3, c4, x0, x1);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(a3, c4, x0, x1)]);
...
void Task791::Task_local::compute() {
  ...
  const Index x1 = b(3);
  // tensor label: I1402
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(a3, c4, x0, x1);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(a3, c4, x0, x1)]);
  ...
  for (auto& x3 : *range_[1]) {
    for (auto& x2 : *range_[1]) {
      ...
      // tensor label: t2
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x3, a3, c4, x2);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x3, a3, c4, x2)]);
      sort_indices<0,3,1,2,0,1,-1,1>(i1data, i1data_sorted, x3.size(), a3.size(), c4.size(), x2.size());
      zgemm3m_("T", "N", x0.size()*x1.size(), a3.size()*c4.size(), x3.size()*x2.size(),
             1.0, i0data_sorted, x3.size()*x2.size(), i1data_sorted, x3.size()*x2.size(),
             1.0, odata_sorted, x0.size()*x1.size());
    }
  }
...
void Task793::Task_local::compute() {
  ...
  for (auto& x1 : *range_[1]) {
    for (auto& c4 : *range_[0]) {
      ...
      // tensor label: I1405
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(a1, c4, x0, x1);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(a1, c4, x0, x1)]);
...
void Task796::Task_local::compute() {
  ...
  for (auto& x1 : *range_[1]) {
    for (auto& c4 : *range_[0]) {
      ...
      // tensor label: I1408
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(a3, c4, x1, x0);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(a3, c4, x1, x0)]);
...
void Task797::Task_local::compute() {
  ...
  const Index x0 = b(3);
  // tensor label: I1408
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(a3, c4, x1, x0);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(a3, c4, x1, x0)]);
  ...
  for (auto& x3 : *range_[1]) {
    for (auto& x2 : *range_[1]) {
      ...
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x3, a3, c4, x2)]);
      sort_indices<0,3,1,2,0,1,1,1>(i1data, i1data_sorted, x3.size(), a3.size(), c4.size(), x2.size());
      zgemm3m_("T", "N", x1.size()*x0.size(), a3.size()*c4.size(), x3.size()*x2.size(),
             1.0, i0data_sorted, x3.size()*x2.size(), i1data_sorted, x3.size()*x2.size(),
             1.0, odata_sorted, x1.size()*x0.size());
    }
  }
...
void Task798::Task_local::compute() {
  ...
  const Index x2 = b(3);
  // tensor label: I1409
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x3, a3, c4, x2);
  {
    // tensor label: t2
  ...
  {
    // tensor label: t2
    std::unique_ptr<std::complex<double>[]> i1data = in(0)->get_block(c4, a3, x3, x2);
    sort_indices<2,1,0,3,1,1,-1,1>(i1data, odata, c4.size(), a3.size(), x3.size(), x2.size());
  }
