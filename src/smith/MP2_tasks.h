...
class Task2 : public Task<T> {
          ...
          for (auto& a4 : virt_) {
            for (auto& c1 : closed_) {
              ...
              }
              {
                ...
                std::unique_ptr<double[]> i2data = v2->get_block(i2hash);
              ...
              }
              {
                ...
                std::unique_ptr<double[]> i3data = v2->get_block(i3hash);
  ...
  public:
    Task2(std::vector<std::shared_ptr<Tensor<T> > > t, std::vector<IndexRange> i, const double e) : Task<T>() {
      closed_ = i[0];
      active_ = i[1];
      virt_   = i[2];
      I0 = t[0];
