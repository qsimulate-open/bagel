...
void Task975::Task_local::compute() {
  ...
  {
    // tensor label: I1782
    std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x0, c3, a2, c1);
...
void Task976::Task_local::compute() {
  ...
  const Index c1 = b(3);
  // tensor label: I1782
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x0, c3, a2, c1);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(x0, c3, a2, c1)]);
  ...
  for (auto& x1 : *range_[1]) {
    ...
    std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(c3, a2, c1, x1)]);
...
void Task981::Task_local::compute() {
  ...
  {
    // tensor label: I1790
    std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x1, x0, a2, c1);
...
void Task982::Task_local::compute() {
  ...
  const Index c1 = b(3);
  // tensor label: I1790
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x1, x0, a2, c1);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(x1, x0, a2, c1)]);
...
void Task985::Task_local::compute() {
  ...
  const Index a1 = b(3);
  // tensor label: I1794
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x0, x2, x1, a1);
...
void Task990::Task_local::compute() {
  ...
  for (auto& x1 : *range_[1]) {
    ...
    zgemm3m_("T", "N", x0.size(), a1.size()*c2.size()*a3.size(), x1.size(),
           1.0, i0data_sorted, x1.size(), i1data_sorted, x1.size(),
