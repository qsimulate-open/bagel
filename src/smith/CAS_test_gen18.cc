...
#include <src/smith/CAS_test_tasks18.h>
    ...
    for (auto& a2 : *range[2])
      for (auto& c3 : *range[0])
        for (auto& x1 : *range[1])
          for (auto& x2 : *range[1])
            for (auto& x0 : *range[1])
              subtasks_.push_back(make_shared<Task_local>(array<const Index,6>{{x0, x2, x1, c3, a2, c1}}, in, t[0], range));
}
