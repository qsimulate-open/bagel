...
Task750::Task750(vector<shared_ptr<Tensor>> t) {
  d_ =  t[0];
}
    ...
    for (auto& c1 : *range[0])
      for (auto& x2 : *range[1])
        for (auto& x3 : *range[1])
          for (auto& x1 : *range[1])
            for (auto& x0 : *range[1])
              subtasks_.push_back(make_shared<Task_local>(array<const Index,6>{{x0, x1, x3, x2, c1, c2}}, in, t[0], range));
}
    ...
    for (auto& a1 : *range[2])
      for (auto& x2 : *range[1])
        for (auto& x3 : *range[1])
          for (auto& x1 : *range[1])
            for (auto& x0 : *range[1])
              subtasks_.push_back(make_shared<Task_local>(array<const Index,6>{{x0, x1, x3, x2, a1, c2}}, in, t[0], range));
}
