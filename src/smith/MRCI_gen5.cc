...
Task204::Task204(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c2 : *range[0])
    for (auto& x3 : *range[1])
      for (auto& x4 : *range[1])
        for (auto& c1 : *range[0])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c1, x4, x3, c2}}, in, t[0], range));
}
...
Task207::Task207(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x3 : *range[1])
    for (auto& c1 : *range[0])
      for (auto& c2 : *range[0])
        for (auto& x4 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x4, c2, c1, x3}}, in, t[0], range));
}
...
Task232::Task232(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x3 : *range[1])
    for (auto& a2 : *range[2])
      for (auto& x4 : *range[1])
        for (auto& c1 : *range[0])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c1, x4, a2, x3}}, in, t[0], range));
}
...
Task236::Task236(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x7 : *range[1])
    for (auto& c1 : *range[0])
      for (auto& x8 : *range[1])
        for (auto& x9 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x9, x8, c1, x7}}, in, t[0], range));
}
