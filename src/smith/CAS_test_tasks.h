...
class Task3 : public Task<T> {
  ...
  public:
    Task3(std::vector<std::shared_ptr<Tensor<T>>> t,  std::array<std::shared_ptr<const IndexRange>,3> range, const double e) : Task<T>() {
      std::array<std::shared_ptr<const Tensor<T>>,2> in = {{t[1], t[2]}};
...
class Task13 : public EnergyTask<T> {
  ...
  public:
    Task13(std::vector<std::shared_ptr<Tensor<T>>> t, std::array<std::shared_ptr<const IndexRange>,3> range) : EnergyTask<T>() {
      std::array<std::shared_ptr<const Tensor<T>>,2> in = {{t[1], t[2]}};
...
template <typename T>
class Task15 : public CorrectionTask<T> {
  protected:
    class Task_local : public SubTask<4,2,T> {
      protected:
        const std::array<std::shared_ptr<const IndexRange>,3> range_;
  ...
  public:
    Task15(std::vector<std::shared_ptr<Tensor<T>>> t, std::array<std::shared_ptr<const IndexRange>,3> range) : CorrectionTask<T>() {
      std::array<std::shared_ptr<const Tensor<T>>,2> in = {{t[1], t[2]}};
...
template <typename T>
class Task16 : public CorrectionTask<T> {
  protected:
    class Task_local : public SubTask<4,1,T> {
      protected:
        const std::array<std::shared_ptr<const IndexRange>,3> range_;
  ...
  public:
    Task16(std::vector<std::shared_ptr<Tensor<T>>> t, std::array<std::shared_ptr<const IndexRange>,3> range) : CorrectionTask<T>() {
      std::array<std::shared_ptr<const Tensor<T>>,1> in = {{t[1]}};
...
template <typename T>
class Task17 : public DensityTask<T> {
  protected:
...
template <typename T>
class Task18 : public Task<T> {  // associated with gamma
  protected:
...
class Task19 : public DensityTask<T> {
  ...
  public:
    Task19(std::vector<std::shared_ptr<Tensor<T>>> t, std::array<std::shared_ptr<const IndexRange>,3> range) : DensityTask<T>() {
      std::array<std::shared_ptr<const Tensor<T>>,1> in = {{t[1]}};
...
class Task25 : public DensityTask<T> {
      ...
      public:
        ...
        void compute() override {
          ...
          std::unique_ptr<double[]> odata = out()->move_block(c3, c5);
          {
            ...
            sort_indices<1,0,1,1,1,1>(i0data, odata, c5.size(), c3.size());
          }
  ...
  public:
    Task25(std::vector<std::shared_ptr<Tensor<T>>> t, std::array<std::shared_ptr<const IndexRange>,3> range) : DensityTask<T>() {
      ...
      for (auto& c5 : *range[0])
        for (auto& c3 : *range[0])
          subtasks_.push_back(std::shared_ptr<Task_local>(new Task_local(std::array<const Index,2>{{c3, c5}}, in, t[0], range)));
    };
    ~Task25() {};
};
      ...
      public:
        ...
        void compute() override {
          ...
          std::unique_ptr<double[]> odata = out()->move_block(a5, a4);
          {
            // tensor label: I34
            std::unique_ptr<double[]> i0data = in(0)->get_block(a5, a4);
            sort_indices<0,1,1,1,1,1>(i0data, odata, a5.size(), a4.size());
          }
