    ...
    for (int jst = 0; jst != nst; ++jst) {
      ...
      for (auto& i3 : active_) {
        ...
        if (is_same<DataType,complex<double>>::value)
          for (size_t i = 0; i != i3.size()*interm_size; ++i)
      ...
      for (auto& i3 : active_) {
        for (auto& i1 : active_) {
          ...
          if (is_same<DataType,complex<double>>::value)
            for (size_t i = 0; i != i1.size()*i3.size()*interm_size; ++i)
              transp2[i] = detail::conj(transp2[i]);
          ...
          for (auto& i0 : active_) {
            ...
            for (auto& i1 : virt_) {
              ...
              if (is_same<DataType,double>::value)
                ...
                sort_indices<3,2,0,1,0,1,2,1>(data0, data1, i2.size(), i3.size(), i0.size(), i1.size());
              // intermediate area
