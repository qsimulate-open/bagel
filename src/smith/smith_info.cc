...
template<>
shared_ptr<const Reference>  SMITH_Info<double>::extract_ref() const {
  return ref_;
}


template<>
shared_ptr<const Reference>  SMITH_Info<complex<double>>::extract_ref() const {
  shared_ptr<const Reference> out = ref_;
  if (!do_ms_ && !do_xms_ && in->nstate() != 1) {
