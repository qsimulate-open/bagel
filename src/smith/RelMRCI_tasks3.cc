...
void Task100::Task_local::compute() {
  ...
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x7, x0, x6, x5, x4, x1, x3, x2);
  {
    if (x3 == x5) {
...
void Task101::Task_local::compute() {
  ...
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x5, x0, x4, x2, x3, x1);
  {
...
void Task103::Task_local::compute() {
  ...
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x0, x7, x1, x6);
  // associated with merged
  std::unique_ptr<std::complex<double>[]> fdata = in(5)->get_block(x5, x4, x3, x2);
  if (x3 == x7 && x1 == x6 && x0 == x2) {
...
void Task107::Task_local::compute() {
  ...
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x7, x0, x6, x1);
  // associated with merged
...
void Task109::Task_local::compute() {
  ...
  {
    // tensor label: I0
    std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c2, c1, x1, x0);
    sort_indices<0,2,1,3,1,1,1,1>(i0data, odata, c2.size(), c1.size(), x1.size(), x0.size());
  }
...
void Task110::Task_local::compute() {
  ...
  for (auto& x3 : *range_[1]) {
    for (auto& x2 : *range_[1]) {
      ...
      // tensor label: I1
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c1, x3, x2, c2);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(c1, x3, x2, c2)]);
      sort_indices<1,2,0,3,0,1,1,1>(i1data, i1data_sorted, c1.size(), x3.size(), x2.size(), c2.size());
...
void Task111::Task_local::compute() {
  ...
  const Index c2 = b(3);
  // tensor label: I1
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c1, x3, x2, c2);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(c1, x3, x2, c2)]);
  ...
  for (auto& c3 : *range_[0]) {
    ...
    sort_indices<2,0,1,3,0,1,-2,1>(i1data, i1data_sorted, c1.size(), x3.size(), c3.size(), x2.size());
    zgemm3m_("T", "N", c2.size(), c1.size()*x3.size()*x2.size(), c3.size(),
           1.0, i0data_sorted, c3.size(), i1data_sorted, c3.size(),
           1.0, odata_sorted, c2.size());
  }
  sort_indices<1,2,3,0,1,1,1,1>(odata_sorted, odata, c2.size(), c1.size(), x3.size(), x2.size());
  out()->put_block(odata, c1, x3, x2, c2);
}
...
void Task112::Task_local::compute() {
  ...
  sort_indices<0,1,2,3,1,1,1,1>(odata_sorted, odata, c1.size(), x3.size(), x2.size(), c2.size());
  out()->put_block(odata, c1, x3, x2, c2);
}
...
void Task114::Task_local::compute() {
  ...
  sort_indices<0,1,3,2,1,1,1,1>(odata_sorted, odata, c1.size(), x3.size(), c2.size(), x2.size());
  out()->put_block(odata, c1, x3, x2, c2);
}
...
void Task115::Task_local::compute() {
  ...
  sort_indices<1,0,2,3,1,1,1,1>(odata_sorted, odata, c2.size(), c1.size(), x0.size(), x1.size());
  out()->put_block(odata, c1, c2, x0, x1);
}
...
void Task117::Task_local::compute() {
  ...
  for (auto& x3 : *range_[1]) {
    for (auto& x2 : *range_[1]) {
      ...
      // tensor label: I7
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x2, c1, c2, x3);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x2, c1, c2, x3)]);
      sort_indices<3,0,1,2,0,1,1,1>(i1data, i1data_sorted, x2.size(), c1.size(), c2.size(), x3.size());
      zgemm3m_("T", "N", x1.size()*x0.size(), c1.size()*c2.size(), x2.size()*x3.size(),
             1.0, i0data_sorted, x2.size()*x3.size(), i1data_sorted, x2.size()*x3.size(),
             1.0, odata_sorted, x1.size()*x0.size());
    }
  }
  sort_indices<2,3,1,0,1,1,1,1>(odata_sorted, odata, x1.size(), x0.size(), c1.size(), c2.size());
  out()->put_block(odata, c1, c2, x0, x1);
}
...
void Task118::Task_local::compute() {
  ...
  for (auto& a3 : *range_[2]) {
    ...
    zgemm3m_("T", "N", c1.size()*c2.size()*x3.size(), x2.size(), a3.size(),
...
void Task120::Task_local::compute() {
  ...
  sort_indices<3,2,0,1,1,1,1,1>(odata_sorted, odata, x0.size(), x1.size(), c2.size(), c1.size());
  out()->put_block(odata, c1, c2, x0, x1);
}
...
void Task121::Task_local::compute() {
  ...
  const Index x4 = b(5);
  // tensor label: I257
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c2, x3, x2, c1, x5, x4);
...
void Task126::Task_local::compute() {
  ...
  const Index x4 = b(5);
  // tensor label: I263
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x3, c2, x2, c1, x5, x4);
  ...
  out()->put_block(odata, x3, c2, x2, c1, x5, x4);
}
...
void Task127::Task_local::compute() {
  ...
  sort_indices<0,1,2,3,1,1,1,1>(odata_sorted, odata, c1.size(), c2.size(), x0.size(), x1.size());
  out()->put_block(odata, c1, c2, x0, x1);
}
...
void Task128::Task_local::compute() {
  ...
  const Index x1 = b(5);
  // tensor label: I272
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c2, x7, x6, x0, x5, x1);
  ...
  out()->put_block(odata, c2, x7, x6, x0, x5, x1);
}
...
void Task130::Task_local::compute() {
  ...
  sort_indices<0,1,3,2,1,1,1,1>(odata_sorted, odata, c1.size(), c2.size(), x1.size(), x0.size());
  out()->put_block(odata, c1, c2, x0, x1);
}
...
void Task131::Task_local::compute() {
  ...
  const Index x2 = b(3);
  // tensor label: I278
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c3, x1, x0, x2);
  ...
  out()->put_block(odata, c3, x1, x0, x2);
}
...
void Task133::Task_local::compute() {
  ...
  const Index x0 = b(3);
  // tensor label: I281
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(a3, x1, x5, x0);
  ...
  out()->put_block(odata, a3, x1, x5, x0);
}
    ...
    for (auto& x2 : *range_[1]) {
      for (auto& x4 : *range_[1]) {
        for (auto& x3 : *range_[1]) {
          ...
          std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x5, x2, x0, x4, x1, x3);
          std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(x5, x2, x0, x4, x1, x3)]);
          sort_indices<0,1,3,5,2,4,0,1,1,1>(i0data, i0data_sorted, x5.size(), x2.size(), x0.size(), x4.size(), x1.size(), x3.size());
          // tensor label: I299
          std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c2, x3, x2, x5, c1, x4);
          std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(c2, x3, x2, x5, c1, x4)]);
...
void Task136::Task_local::compute() {
  ...
  const Index x4 = b(5);
  // tensor label: I299
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c2, x3, x2, x5, c1, x4);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(c2, x3, x2, x5, c1, x4)]);
  ...
  for (auto& a3 : *range_[2]) {
    ...
    zgemm3m_("T", "N", x5.size()*c1.size()*x4.size(), c2.size()*x3.size()*x2.size(), a3.size(),
           1.0, i0data_sorted, a3.size(), i1data_sorted, a3.size(),
  ...
  out()->put_block(odata, c2, x3, x2, x5, c1, x4);
}
...
void Task137::Task_local::compute() {
  ...
  sort_indices<3,2,1,0,1,1,1,1>(odata_sorted, odata, x1.size(), x0.size(), c2.size(), c1.size());
  out()->put_block(odata, c1, c2, x0, x1);
}
...
void Task144::Task_local::compute() {
  ...
  for (auto& c2 : *range_[0]) {
    ...
    // tensor label: I13
    std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x2, x1, x0, c2);
    std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x2, x1, x0, c2)]);
...
void Task145::Task_local::compute() {
  ...
  const Index c2 = b(3);
  // tensor label: I13
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x2, x1, x0, c2);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(x2, x1, x0, c2)]);
  ...
  for (auto& x5 : *range_[1]) {
    for (auto& x4 : *range_[1]) {
      for (auto& x3 : *range_[1]) {
        ...
        zgemm3m_("T", "N", c2.size(), x2.size()*x1.size()*x0.size(), x5.size()*x4.size()*x3.size(),
               1.0, i0data_sorted, x5.size()*x4.size()*x3.size(), i1data_sorted, x5.size()*x4.size()*x3.size(),
               1.0, odata_sorted, c2.size());
      }
    }
