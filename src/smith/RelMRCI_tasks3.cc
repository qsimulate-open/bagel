...
void Task102::Task_local::compute() {
  ...
  const Index c2 = b(3);
  // tensor label: I0
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x0, x1, c1, c2);
  ...
  sort_indices<3,2,0,1,1,1,1,1>(odata_sorted, odata, c1.size(), c2.size(), x1.size(), x0.size());
...
void Task107::Task_local::compute() {
  ...
  for (auto& x3 : *range_[1]) {
    for (auto& x2 : *range_[1]) {
      ...
      // tensor label: I213
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x2, c2, c1, x3);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x2, c2, c1, x3)]);
      sort_indices<3,0,1,2,0,1,1,1>(i1data, i1data_sorted, x2.size(), c2.size(), c1.size(), x3.size());
      zgemm3m_("T", "N", x0.size()*x1.size(), c2.size()*c1.size(), x2.size()*x3.size(),
             1.0, i0data_sorted, x2.size()*x3.size(), i1data_sorted, x2.size()*x3.size(),
             1.0, odata_sorted, x0.size()*x1.size());
    }
  }
  sort_indices<0,1,3,2,1,1,1,1>(odata_sorted, odata, x0.size(), x1.size(), c2.size(), c1.size());
  out()->put_block(odata, x0, x1, c1, c2);
}
...
void Task108::Task_local::compute() {
  ...
  const Index x3 = b(3);
  // tensor label: I213
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x2, c2, c1, x3);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(x2, c2, c1, x3)]);
...
void Task111::Task_local::compute() {
  ...
  sort_indices<1,0,3,2,1,1,1,1>(odata_sorted, odata, x1.size(), x0.size(), c2.size(), c1.size());
  out()->put_block(odata, x0, x1, c1, c2);
}
