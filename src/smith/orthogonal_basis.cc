...
vector<double> f, shared_ptr<const Denom<DataType>> d, shared_ptr<const MultiTensor_<DataType>> tensor, const string type,
shared_ptr<Vec<Tensor_<DataType>>> g0, shared_ptr<Vec<Tensor_<DataType>>> g1, shared_ptr<Vec<Tensor_<DataType>>> g2,
shared_ptr<Vec<Tensor_<DataType>>> g3, shared_ptr<Vec<Tensor_<DataType>>> g4) : closed_(c), active_(a), virt_(v), eig_(f) {
  nact_ = info->nact();
