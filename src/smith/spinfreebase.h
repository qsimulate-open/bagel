...
class SpinFreeMethod {
  protected:
    ...
    std::shared_ptr<Vector_<DataType>> transform_to_orthogonal_amplitude(std::shared_ptr<const MultiTensor_<DataType>> tensor) const;
    std::shared_ptr<Vector_<DataType>> transform_to_orthogonal_amplitude(std::shared_ptr<const Tensor_<DataType>> tensor, const int ist) const;
