...
Task1156::Task1156(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range, const double e) {
  array<shared_ptr<const Tensor>,1> in = {{t[1]}};
...
Task1160::Task1160(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  ...
  for (auto& c2 : *range[0])
    for (auto& x5 : *range[1])
      for (auto& x4 : *range[1])
        for (auto& x3 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x3, x4, x5, c2}}, in, t[0], range));
}
...
Task1166::Task1166(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range, const double e) {
  array<shared_ptr<const Tensor>,1> in = {{t[1]}};
  subtasks_.reserve(range[1]->nblock()*range[0]->nblock()*range[1]->nblock()*range[1]->nblock());
  for (auto& x3 : *range[1])
    for (auto& c1 : *range[0])
...
Task1168::Task1168(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  ...
  for (auto& c1 : *range[0])
    for (auto& a2 : *range[2])
      for (auto& x5 : *range[1])
        for (auto& x4 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x4, x5, a2, c1}}, in, t[0], range));
}
...
Task1184::Task1184(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  array<shared_ptr<const Tensor>,1> in = {{t[1]}};
  subtasks_.reserve(range[0]->nblock()*range[1]->nblock());
  for (auto& c3 : *range[0])
    for (auto& x2 : *range[1])
...
Task1186::Task1186(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  array<shared_ptr<const Tensor>,1> in = {{t[1]}};
  subtasks_.reserve(range[0]->nblock()*range[1]->nblock());
  for (auto& c3 : *range[0])
    for (auto& x2 : *range[1])
