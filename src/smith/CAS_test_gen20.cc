...
#include <src/smith/CAS_test_tasks20.h>
...
Task952::Task952(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c1 : *range[0])
    for (auto& a3 : *range[2])
      for (auto& x0 : *range[1])
        for (auto& x1 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x1, x0, a3, c1}}, in, t[0], range));
}
...
Task968::Task968(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& a2 : *range[2])
    for (auto& x0 : *range[1])
      subtasks_.push_back(make_shared<Task_local>(array<const Index,2>{{x0, a2}}, in, t[0], range));
}
...
Task969::Task969(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c3 : *range[0])
    for (auto& a4 : *range[2])
      for (auto& c1 : *range[0])
        for (auto& x0 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x0, c1, a4, c3}}, in, t[0], range));
}
...
Task973::Task973(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c3 : *range[0])
    for (auto& a2 : *range[2])
      for (auto& c1 : *range[0])
        for (auto& x0 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x0, c1, a2, c3}}, in, t[0], range));
}
