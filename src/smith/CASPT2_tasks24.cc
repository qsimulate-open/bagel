...
void Task1154::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata = out()->move_block(a2, x3);
  {
    ...
    sort_indices<0,1,1,1,1,4>(i0data, odata, a2.size(), x3.size());
  }
...
void Task1156::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata = out()->move_block(x1, x0, c1, x2);
  {
    ...
    sort_indices<1,0,2,3,1,1,-1,4>(i0data, odata, x0.size(), x1.size(), c1.size(), x2.size());
  }
...
void Task1159::Task_local::compute() {
  ...
  for (auto& c2 : *range_[0]) {
    ...
    // tensor label: I1582
    std::unique_ptr<double[]> i1data = in(1)->get_block(x3, x4, x5, c2);
    std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x3, x4, x5, c2)]);
    sort_indices<3,0,1,2,0,1,1,1>(i1data, i1data_sorted, x3.size(), x4.size(), x5.size(), c2.size());
    dgemm_("T", "N", x0.size()*x1.size()*x2.size(), x3.size()*x4.size()*x5.size(), c2.size(),
...
void Task1160::Task_local::compute() {
  ...
  const Index c2 = b(3);
  // tensor label: I1582
  std::unique_ptr<double[]> odata = out()->move_block(x3, x4, x5, c2);
  std::unique_ptr<double[]> odata_sorted(new double[out()->get_size(x3, x4, x5, c2)]);
  std::fill_n(odata_sorted.get(), out()->get_size(x3, x4, x5, c2), 0.0);
  for (auto& c1 : *range_[0]) {
    // tensor label: f1
    std::unique_ptr<double[]> i0data = in(0)->get_block(c1, c2);
    std::unique_ptr<double[]> i0data_sorted(new double[in(0)->get_size(c1, c2)]);
    ...
    dgemm_("T", "N", c2.size(), x3.size()*x4.size()*x5.size(), c1.size(),
           1.0, i0data_sorted, c1.size(), i1data_sorted, c1.size(),
           1.0, odata_sorted, c2.size());
  }
  sort_indices<1,2,3,0,1,1,1,1>(odata_sorted, odata, c2.size(), x3.size(), x4.size(), x5.size());
  out()->put_block(odata, x3, x4, x5, c2);
}
...
void Task1161::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata = out()->move_block(x3, c1, x4, x5);
  {
    // tensor label: t2
    std::unique_ptr<double[]> i0data = in(0)->get_block(x5, x4, c1, x3);
    sort_indices<3,2,1,0,1,1,-1,4>(i0data, odata, x5.size(), x4.size(), c1.size(), x3.size());
  }
...
void Task1163::Task_local::compute() {
  ...
  for (auto& a2 : *range_[2]) {
    ...
    dgemm_("T", "N", c1.size()*x0.size()*x1.size(), x2.size(), a2.size(),
           1.0, i0data_sorted, a2.size(), i1data_sorted, a2.size(),
...
void Task1166::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata = out()->move_block(x4, x5, c1, x3);
  {
    ...
    sort_indices<1,0,2,3,1,1,-1,4>(i0data, odata, x5.size(), x4.size(), c1.size(), x3.size());
  }
...
void Task1167::Task_local::compute() {
  ...
  for (auto& a2 : *range_[2]) {
    ...
    sort_indices<2,0,1,3,0,1,1,1>(i1data, i1data_sorted, x4.size(), x5.size(), a2.size(), c1.size());
...
void Task1188::Task_local::compute() {
  ...
  const Index c3 = b(1);
  // tensor label: I1606
  std::unique_ptr<double[]> odata = out()->move_block(x3, c3);
  ...
  for (auto& c1 : *range_[0]) {
    for (auto& a2 : *range_[2]) {
      ...
      sort_indices<3,2,0,1,0,1,1,1>(i1data, i1data_sorted, x3.size(), c3.size(), a2.size(), c1.size());
      dgemm_("T", "N", 1, x3.size()*c3.size(), a2.size()*c1.size(),
             1.0, i0data_sorted, a2.size()*c1.size(), i1data_sorted, a2.size()*c1.size(),
             1.0, odata_sorted, 1);
    }
  }
  sort_indices<0,1,1,1,1,1>(odata_sorted, odata, x3.size(), c3.size());
  out()->put_block(odata, x3, c3);
}
...
void Task1191::Task_local::compute() {
  ...
  const Index c1 = b(1);
  // tensor label: I1610
  std::unique_ptr<double[]> odata = out()->move_block(x3, c1);
  ...
  for (auto& c3 : *range_[0]) {
    for (auto& a2 : *range_[2]) {
      ...
      std::unique_ptr<double[]> i1data = in(1)->get_block(x3, c3, a2, c1);
      std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x3, c3, a2, c1)]);
      sort_indices<1,2,0,3,0,1,1,1>(i1data, i1data_sorted, x3.size(), c3.size(), a2.size(), c1.size());
      dgemm_("T", "N", 1, x3.size()*c1.size(), c3.size()*a2.size(),
             1.0, i0data_sorted, c3.size()*a2.size(), i1data_sorted, c3.size()*a2.size(),
             1.0, odata_sorted, 1);
    }
  }
  sort_indices<0,1,1,1,1,1>(odata_sorted, odata, x3.size(), c1.size());
  out()->put_block(odata, x3, c1);
}
...
void Task1194::Task_local::compute() {
  ...
  for (auto& c3 : *range_[0]) {
    ...
    std::unique_ptr<double[]> i1data = in(1)->get_block(x3, c3, a2, c1);
    std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x3, c3, a2, c1)]);
    sort_indices<1,0,2,3,0,1,1,1>(i1data, i1data_sorted, x3.size(), c3.size(), a2.size(), c1.size());
    dgemm_("T", "N", x2.size(), x3.size()*a2.size()*c1.size(), c3.size(),
           1.0, i0data_sorted, c3.size(), i1data_sorted, c3.size(),
           1.0, odata_sorted, x2.size());
  }
  sort_indices<1,2,3,0,1,1,1,1>(odata_sorted, odata, x2.size(), x3.size(), a2.size(), c1.size());
