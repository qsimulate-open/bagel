...
Task753::Task753(vector<shared_ptr<Tensor>> t) {
  Den1_ =  t[0];
}
...
Task782::Task782(vector<shared_ptr<Tensor>> t) {
  deci_ =  t[0];
}
...
Task792::Task792(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range, const double e) {
  array<shared_ptr<const Tensor>,1> in = {{t[1]}};
  subtasks_.reserve(range[0]->nblock()*range[1]->nblock()*range[0]->nblock()*range[1]->nblock());
  for (auto& c1 : *range[0])
    for (auto& x0 : *range[1])
