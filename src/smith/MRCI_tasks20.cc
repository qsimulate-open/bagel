...
void Task952::Task_local::compute() {
  ...
  for (auto& x3 : *range_[1]) {
    for (auto& x2 : *range_[1]) {
      ...
      // tensor label: v2
      std::unique_ptr<double[]> i1data = in(1)->get_block(x3, x2, c1, a2);
      std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x3, x2, c1, a2)]);
      sort_indices<0,1,2,3,0,1,1,1>(i1data, i1data_sorted, x3.size(), x2.size(), c1.size(), a2.size());
      dgemm_("T", "N", x1.size()*x0.size(), c1.size()*a2.size(), x3.size()*x2.size(),
             1.0, i0data_sorted, x3.size()*x2.size(), i1data_sorted, x3.size()*x2.size(),
             1.0, odata_sorted, x1.size()*x0.size());
    }
...
void Task976::Task_local::compute() {
  ...
  for (auto& x1 : *range_[1]) {
    ...
    sort_indices<3,0,1,2,0,1,2,1>(i1data, i1data_sorted, c1.size(), a2.size(), c3.size(), x1.size());
    dgemm_("T", "N", x0.size(), c1.size()*a2.size()*c3.size(), x1.size(),
           1.0, i0data_sorted, x1.size(), i1data_sorted, x1.size(),
           1.0, odata_sorted, x0.size());
  }
  sort_indices<0,3,2,1,1,1,1,1>(odata_sorted, odata, x0.size(), c1.size(), a2.size(), c3.size());
  out()->put_block(odata, x0, c3, a2, c1);
}
