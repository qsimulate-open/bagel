...
Task804::Task804(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c2 : *range[0])
    for (auto& a3 : *range[2])
      for (auto& a1 : *range[2])
        for (auto& x3 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x3, a1, a3, c2}}, in, t[0], range));
}
...
Task821::Task821(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x3 : *range[1])
    for (auto& a2 : *range[2])
      for (auto& c3 : *range[0])
        for (auto& x4 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x4, c3, a2, x3}}, in, t[0], range));
}
...
Task823::Task823(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& a1 : *range[2])
    for (auto& a2 : *range[2])
      for (auto& x3 : *range[1])
        for (auto& a3 : *range[2])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{a3, x3, a2, a1}}, in, t[0], range));
}
...
Task830::Task830(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c2 : *range[0])
    for (auto& a3 : *range[2])
      for (auto& c4 : *range[0])
        for (auto& x3 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x3, c4, a3, c2}}, in, t[0], range));
}
...
Task834::Task834(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c2 : *range[0])
    for (auto& a3 : *range[2])
      for (auto& a1 : *range[2])
        for (auto& a4 : *range[2])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{a4, a1, a3, c2}}, in, t[0], range));
}
...
Task842::Task842(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c2 : *range[0])
    for (auto& x3 : *range[1])
      for (auto& c3 : *range[0])
        for (auto& x4 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x4, c3, x3, c2}}, in, t[0], range));
}
...
Task845::Task845(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x3 : *range[1])
    for (auto& x4 : *range[1])
      for (auto& c2 : *range[0])
        for (auto& x5 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x5, c2, x4, x3}}, in, t[0], range));
}
...
Task847::Task847(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c2 : *range[0])
    for (auto& x3 : *range[1])
      for (auto& x4 : *range[1])
        for (auto& x5 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x5, x4, x3, c2}}, in, t[0], range));
}
