...
void Task1211::Task_local::compute() {
  ...
  const Index x3 = b(3);
  // tensor label: I1668
  std::unique_ptr<double[]> odata = out()->move_block(x4, c2, x5, x3);
  ...
  for (auto& a1 : *range_[2]) {
    ...
    sort_indices<2,0,1,3,0,1,1,1>(i1data, i1data_sorted, x4.size(), c2.size(), a1.size(), x5.size());
    dgemm_("T", "N", x3.size(), x4.size()*c2.size()*x5.size(), a1.size(),
           1.0, i0data_sorted, a1.size(), i1data_sorted, a1.size(),
           1.0, odata_sorted, x3.size());
  }
  sort_indices<1,2,3,0,1,1,1,1>(odata_sorted, odata, x3.size(), x4.size(), c2.size(), x5.size());
  out()->put_block(odata, x4, c2, x5, x3);
}
...
void Task1230::Task_local::compute() {
  ...
  for (auto& c2 : *range_[0]) {
    for (auto& a3 : *range_[2]) {
      ...
      sort_indices<0,1,2,3,0,1,1,1>(i0data, i0data_sorted, c2.size(), a3.size(), c1.size(), x0.size());
      // tensor label: I1587
      ...
      dgemm_("T", "N", c1.size()*x0.size(), 1, a3.size()*c2.size(),
             1.0, i0data_sorted, a3.size()*c2.size(), i1data_sorted, a3.size()*c2.size(),
             1.0, odata_sorted, c1.size()*x0.size());
    }
  }
...
void Task1232::Task_local::compute() {
  ...
  for (auto& a3 : *range_[2]) {
    for (auto& c2 : *range_[0]) {
      ...
      sort_indices<1,2,0,3,0,1,1,1>(i0data, i0data_sorted, c1.size(), a3.size(), c2.size(), x0.size());
      // tensor label: I1591
      std::unique_ptr<double[]> i1data = in(1)->get_block(a3, c2);
      std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(a3, c2)]);
...
void Task1235::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata_sorted(new double[out()->get_size(x1, c2, a1, x0)]);
  std::fill_n(odata_sorted.get(), out()->get_size(x1, c2, a1, x0), 0.0);
  for (auto& c3 : *range_[0]) {
    // tensor label: t2
    ...
    sort_indices<2,0,1,3,0,1,1,1>(i0data, i0data_sorted, c2.size(), a1.size(), c3.size(), x0.size());
    // tensor label: I1677
    ...
    dgemm_("T", "N", c2.size()*a1.size()*x0.size(), x1.size(), c3.size(),
           1.0, i0data_sorted, c3.size(), i1data_sorted, c3.size(),
           1.0, odata_sorted, c2.size()*a1.size()*x0.size());
  }
  sort_indices<3,0,1,2,1,1,1,1>(odata_sorted, odata, c2.size(), a1.size(), x0.size(), x1.size());
...
void Task1237::Task_local::compute() {
  ...
  for (auto& a2 : *range_[2]) {
    for (auto& c1 : *range_[0]) {
      ...
      sort_indices<1,2,0,3,0,1,1,1>(i1data, i1data_sorted, x1.size(), a2.size(), c1.size(), x0.size());
      dgemm_("T", "N", x2.size()*x3.size(), x1.size()*x0.size(), a2.size()*c1.size(),
             1.0, i0data_sorted, a2.size()*c1.size(), i1data_sorted, a2.size()*c1.size(),
             1.0, odata_sorted, x2.size()*x3.size());
    }
  }
...
void Task1238::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata_sorted(new double[out()->get_size(x1, a2, c1, x0)]);
  std::fill_n(odata_sorted.get(), out()->get_size(x1, a2, c1, x0), 0.0);
  for (auto& c3 : *range_[0]) {
    // tensor label: t2
    ...
    std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x1, c3)]);
    sort_indices<1,0,0,1,1,1>(i1data, i1data_sorted, x1.size(), c3.size());
    dgemm_("T", "N", a2.size()*c1.size()*x0.size(), x1.size(), c3.size(),
           1.0, i0data_sorted, c3.size(), i1data_sorted, c3.size(),
           1.0, odata_sorted, a2.size()*c1.size()*x0.size());
  }
  sort_indices<3,0,1,2,1,1,1,1>(odata_sorted, odata, a2.size(), c1.size(), x0.size(), x1.size());
...
void Task1240::Task_local::compute() {
  ...
  for (auto& c3 : *range_[0]) {
    ...
    sort_indices<2,0,1,3,0,1,1,1>(i0data, i0data_sorted, c1.size(), a2.size(), c3.size(), x0.size());
    // tensor label: I1731
    std::unique_ptr<double[]> i1data = in(1)->get_block(x1, c3);
    std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x1, c3)]);
    sort_indices<1,0,0,1,1,1>(i1data, i1data_sorted, x1.size(), c3.size());
    dgemm_("T", "N", c1.size()*a2.size()*x0.size(), x1.size(), c3.size(),
           1.0, i0data_sorted, c3.size(), i1data_sorted, c3.size(),
           1.0, odata_sorted, c1.size()*a2.size()*x0.size());
  }
  sort_indices<3,1,0,2,1,1,1,1>(odata_sorted, odata, c1.size(), a2.size(), x0.size(), x1.size());
  out()->put_block(odata, x1, a2, c1, x0);
}
