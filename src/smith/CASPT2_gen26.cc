...
Task1278::Task1278(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  array<shared_ptr<const Tensor>,1> in = {{t[1]}};
  subtasks_.reserve(range[0]->nblock()*range[2]->nblock()*range[0]->nblock()*range[1]->nblock());
  for (auto& c1 : *range[0])
    for (auto& a2 : *range[2])
...
Task1280::Task1280(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  array<shared_ptr<const Tensor>,2> in = {{t[1], t[2]}};
...
Task1286::Task1286(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  ...
  for (auto& c4 : *range[0])
    for (auto& a2 : *range[2])
      for (auto& c3 : *range[0])
        for (auto& x1 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x1, c3, a2, c4}}, in, t[0], range));
}
...
Task1292::Task1292(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  ...
  for (auto& c4 : *range[0])
    for (auto& c1 : *range[0])
      for (auto& a2 : *range[2])
        for (auto& x1 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x1, a2, c1, c4}}, in, t[0], range));
}
