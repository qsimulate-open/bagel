...
template<typename DataType>
shared_ptr<Vector_<DataType>> SpinFreeMethod<DataType>::transform_to_orthogonal_amplitude(shared_ptr<const MultiTensor_<DataType>> tensor) const {
  const size_t nact = info_->nact();
  const size_t nclosed = info_->nclosed();
  const size_t nvirt = info_->nvirt();
  const size_t nocc = nact + nclosed;
  ...
  for (int ist = 0; ist != nst; ++ist) {
    if (tensor->at(ist)) {
      shared_ptr<Vector_<DataType>> vist = transform_to_orthogonal_amplitude(tensor->at(ist), ist);
      copy_n(vist->data(), vist->size(), out->data()+ioffset);
      ioffset += vist->size();
    }
  }
...
template<typename DataType>
shared_ptr<Vector_<DataType>> SpinFreeMethod<DataType>::transform_to_orthogonal_amplitude(shared_ptr<const Tensor_<DataType>> tensor, const int istate) const {
  // number of orthogonal basis functions: use numbers of denom.
  const size_t nact = info_->nact();
  const size_t nclosed = info_->nclosed();
  const size_t nvirt = info_->nvirt();
      ...
      for (auto& i3 : virt_)
        for (auto& i2 : closed_)
          for (auto& i1 : virt_) {
            ...
            unique_ptr<DataType[]> data0 = tensor->get_block(i0, i1, i2, i3);
            unique_ptr<DataType[]> data2(new DataType[blocksize]);
            sort_indices<0,1,2,3,0,1,1,1>(data0, data2, i0.size(), i1.size(), i2.size(), i3.size());
            if (is_same<DataType,double>::value) {
              const unique_ptr<DataType[]> data1 = tensor->get_block(i0, i3, i2, i1);
              sort_indices<0,3,2,1,2,3,1,3>(data1, data2, i0.size(), i3.size(), i2.size(), i1.size());
            } else {
      ...
      for (auto& i2 : active_) {
        ...
        for (auto& i1 : virt_)
          for (auto& i0 : closed_) {
            const size_t blocksize = tensor->get_size(i0, i1, i2, i3);
            unique_ptr<DataType[]> data0 = tensor->get_block(i0, i1, i2, i3);
            unique_ptr<DataType[]> data1 = tensor->get_block(i2, i1, i0, i3);
            unique_ptr<DataType[]> data2(new DataType[blocksize*2]);
            sort_indices<0,1,2,3,0,1,1,1>(data0.get(), data2.get()          , i0.size(), i1.size(), i2.size(), i3.size());
