...
void Task101::Task_local::compute() {
  ...
  {
    // tensor label: v2
    std::unique_ptr<std::complex<double>[]> i2data = in(1)->get_block(x3, x2, c2, a1);
...
void Task121::Task_local::compute() {
  ...
  }
  {
    ...
    sort_indices<0,2,1,3,1,1,-1,1>(i1data, odata, c1.size(), a2.size(), x3.size(), x2.size());
  }
  {
    // tensor label: v2
    std::unique_ptr<std::complex<double>[]> i2data = in(1)->get_block(c1, a2, x3, x2);
    sort_indices<0,2,1,3,1,1,1,2>(i2data, odata, c1.size(), a2.size(), x3.size(), x2.size());
  }
  {
    // tensor label: v2
    std::unique_ptr<std::complex<double>[]> i3data = in(1)->get_block(x3, a2, c1, x2);
    sort_indices<2,0,1,3,1,1,-1,2>(i3data, odata, x3.size(), a2.size(), c1.size(), x2.size());
  }
  {
    // tensor label: v2
    std::unique_ptr<std::complex<double>[]> i4data = in(1)->get_block(x3, x2, c1, a2);
    sort_indices<2,0,3,1,1,1,1,2>(i4data, odata, x3.size(), x2.size(), c1.size(), a2.size());
  }
...
void Task131::Task_local::compute() {
  ...
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c1, a2);
  {
    ...
    sort_indices<0,1,1,1,1,1>(i0data, odata, c1.size(), a2.size());
  }
...
void Task138::Task_local::compute() {
  ...
  {
    // tensor label: v2
    std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x5, x4, x3, a1);
