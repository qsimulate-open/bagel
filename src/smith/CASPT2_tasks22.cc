...
void Task1059::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata = out()->move_block(x1, a2, c1, x0);
  {
    ...
    sort_indices<0,1,2,3,1,1,-1,2>(i0data, odata, x1.size(), a2.size(), c1.size(), x0.size());
  }
...
void Task1074::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata = out()->move_block(c3, a2, c1, x1);
  {
    ...
    sort_indices<0,1,2,3,1,1,-1,2>(i0data, odata, c3.size(), a2.size(), c1.size(), x1.size());
  }
  {
    // tensor label: t2
...
void Task1077::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata = out()->move_block(x3, a1, c2, x2);
  {
    ...
    sort_indices<0,1,2,3,1,1,1,2>(i0data, odata, x3.size(), a1.size(), c2.size(), x2.size());
  }
...
void Task1099::Task_local::compute() {
  ...
  for (auto& c1 : *range_[0]) {
    for (auto& c2 : *range_[0]) {
      ...
      sort_indices<3,1,0,2,0,1,1,1>(i1data, i1data_sorted, x4.size(), c2.size(), x5.size(), c1.size());
