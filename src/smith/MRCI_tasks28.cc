...
void Task1370::Task_local::compute() {
  ...
  for (auto& x3 : *range_[1]) {
    for (auto& x2 : *range_[1]) {
      ...
      sort_indices<1,2,0,3,0,1,1,1>(i1data, i1data_sorted, c2.size(), x3.size(), x2.size(), a1.size());
      dgemm_("T", "N", x1.size()*x0.size(), c2.size()*a1.size(), x3.size()*x2.size(),
             1.0, i0data_sorted, x3.size()*x2.size(), i1data_sorted, x3.size()*x2.size(),
             1.0, odata_sorted, x1.size()*x0.size());
    }
...
void Task1371::Task_local::compute() {
  ...
  const Index a1 = b(3);
  // tensor label: I1753
  std::unique_ptr<double[]> odata = out()->move_block(c2, x3, x2, a1);
  {
    // tensor label: v2
    std::unique_ptr<double[]> i0data = in(0)->get_block(c2, x3, x2, a1);
    sort_indices<0,1,2,3,1,1,-1,2>(i0data, odata, c2.size(), x3.size(), x2.size(), a1.size());
  }
  out()->put_block(odata, c2, x3, x2, a1);
}
...
void Task1379::Task_local::compute() {
  ...
  for (auto& x2 : *range_[1]) {
    for (auto& x3 : *range_[1]) {
      ...
      sort_indices<2,1,0,3,0,1,1,1>(i1data, i1data_sorted, c1.size(), x3.size(), x2.size(), a2.size());
...
void Task1380::Task_local::compute() {
  ...
  const Index a2 = b(3);
  // tensor label: I1761
  std::unique_ptr<double[]> odata = out()->move_block(c1, x3, x2, a2);
  {
    // tensor label: v2
    std::unique_ptr<double[]> i0data = in(0)->get_block(c1, x3, x2, a2);
    sort_indices<0,1,2,3,1,1,1,2>(i0data, odata, c1.size(), x3.size(), x2.size(), a2.size());
  }
  out()->put_block(odata, c1, x3, x2, a2);
}
...
void Task1398::Task_local::compute() {
  ...
  for (auto& x1 : *range_[1]) {
    ...
    dgemm_("T", "N", x0.size(), c1.size()*c3.size()*a2.size(), x1.size(),
           1.0, i0data_sorted, x1.size(), i1data_sorted, x1.size(),
