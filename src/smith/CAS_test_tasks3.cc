...
#include <src/smith/CAS_test_tasks3.h>
...
void Task115::Task_local::compute() {
  ...
  {
    // tensor label: I31
    std::unique_ptr<double[]> i0data = in(0)->get_block(x0, c1, c3, a2);
    sort_indices<2,0,1,3,1,1,1,1>(i0data, odata, x0.size(), c1.size(), c3.size(), a2.size());
  }
...
void Task118::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata = out()->move_block(x1, x3, x0, x2);
  {
    ...
    sort_indices<0,1,2,3,1,1,-2,1>(i0data, odata, x1.size(), x3.size(), x0.size(), x2.size());
  }
