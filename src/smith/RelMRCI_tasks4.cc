...
void Task155::Task_local::compute() {
  ...
  for (auto& a3 : *range_[2]) {
    for (auto& c2 : *range_[0]) {
      ...
      // tensor label: v2
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(a3, x3, c1, c2);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(a3, x3, c1, c2)]);
      sort_indices<0,3,1,2,0,1,1,1>(i1data, i1data_sorted, a3.size(), x3.size(), c1.size(), c2.size());
      zgemm3m_("T", "N", x5.size()*x4.size(), x3.size()*c1.size(), a3.size()*c2.size(),
  ...
  for (auto& x5 : *range_[1]) {
    for (auto& x4 : *range_[1]) {
      for (auto& x3 : *range_[1]) {
        ...
        sort_indices<2,3,0,1,0,1,1,1>(i1data, i1data_sorted, x3.size(), c1.size(), x5.size(), x4.size());
        zgemm3m_("T", "N", x2.size()*x1.size()*x0.size(), c1.size(), x3.size()*x5.size()*x4.size(),
               1.0, i0data_sorted, x3.size()*x5.size()*x4.size(), i1data_sorted, x3.size()*x5.size()*x4.size(),
               1.0, odata_sorted, x2.size()*x1.size()*x0.size());
      }
...
void Task159::Task_local::compute() {
  ...
  sort_indices<3,2,0,1,1,1,1,1>(odata_sorted, odata, x5.size(), x4.size(), c1.size(), x3.size());
  out()->put_block(odata, x3, c1, x5, x4);
}
...
void Task161::Task_local::compute() {
  ...
  const Index c2 = b(3);
  // tensor label: I378
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(a3, x3, c1, c2);
  {
    // tensor label: v2
  ...
  {
    // tensor label: v2
    std::unique_ptr<std::complex<double>[]> i1data = in(0)->get_block(c1, x3, a3, c2);
    sort_indices<2,1,0,3,1,1,1,1>(i1data, odata, c1.size(), x3.size(), a3.size(), c2.size());
  }
...
void Task163::Task_local::compute() {
  ...
  const Index x6 = b(5);
  // tensor label: I308
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x5, x4, x3, c1, x7, x6);
...
void Task167::Task_local::compute() {
  ...
  const Index x5 = b(5);
  // tensor label: I317
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c1, x4, x3, x7, x6, x5);
...
void Task168::Task_local::compute() {
  ...
  const Index x3 = b(3);
  // tensor label: I318
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c1, c2, x4, x3);
  {
    // tensor label: v2
...
void Task173::Task_local::compute() {
  ...
  const Index x5 = b(5);
  // tensor label: I323
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x4, c1, x3, x7, x6, x5);
...
void Task175::Task_local::compute() {
  ...
  const Index x5 = b(3);
  // tensor label: I329
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x4, x3, c1, x5);
      ...
      for (auto& x6 : *range_[1]) {
        for (auto& x4 : *range_[1]) {
          for (auto& x3 : *range_[1]) {
            ...
            std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x7, x5, x2, x6, x4, x3, x1, x0);
            std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(x7, x5, x2, x6, x4, x3, x1, x0)]);
            ...
            // tensor label: I359
            std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x5, x4, x3, x7, c1, x6);
            std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x5, x4, x3, x7, c1, x6)]);
...
void Task185::Task_local::compute() {
  ...
  const Index x6 = b(5);
  // tensor label: I359
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x5, x4, x3, x7, c1, x6);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(x5, x4, x3, x7, c1, x6)]);
  ...
  for (auto& a2 : *range_[2]) {
    ...
    std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(x7, a2, c1, x6)]);
    sort_indices<1,0,2,3,0,1,1,1>(i0data, i0data_sorted, x7.size(), a2.size(), c1.size(), x6.size());
    // tensor label: v2
    std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(a2, x5, x4, x3);
    std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(a2, x5, x4, x3)]);
    sort_indices<0,1,2,3,0,1,-1,2>(i1data, i1data_sorted, a2.size(), x5.size(), x4.size(), x3.size());
    zgemm3m_("T", "N", x7.size()*c1.size()*x6.size(), x5.size()*x4.size()*x3.size(), a2.size(),
           1.0, i0data_sorted, a2.size(), i1data_sorted, a2.size(),
      ...
      for (auto& x6 : *range_[1]) {
        for (auto& x5 : *range_[1]) {
          for (auto& x4 : *range_[1]) {
            ...
            std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x7, x3, x2, x6, x5, x4, x1, x0);
            std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(x7, x3, x2, x6, x5, x4, x1, x0)]);
