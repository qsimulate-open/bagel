...
void Task1100::Task_local::compute() {
  ...
  const Index c1 = b(3);
  // tensor label: I1560
  std::unique_ptr<double[]> odata = out()->move_block(x4, c2, x5, c1);
  {
    // tensor label: t2
    std::unique_ptr<double[]> i0data = in(0)->get_block(c1, x5, c2, x4);
    sort_indices<3,2,1,0,1,1,1,2>(i0data, odata, c1.size(), x5.size(), c2.size(), x4.size());
  }
  out()->put_block(odata, x4, c2, x5, c1);
}
...
void Task1105::Task_local::compute() {
  ...
  for (auto& c1 : *range_[0]) {
    for (auto& c3 : *range_[0]) {
      ...
      // tensor label: I1563
      std::unique_ptr<double[]> i1data = in(1)->get_block(x2, x3, c1, c3);
      std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x2, x3, c1, c3)]);
      sort_indices<2,3,0,1,0,1,1,1>(i1data, i1data_sorted, x2.size(), x3.size(), c1.size(), c3.size());
      dgemm_("T", "N", x0.size()*x1.size(), x2.size()*x3.size(), c1.size()*c3.size(),
             1.0, i0data_sorted, c1.size()*c3.size(), i1data_sorted, c1.size()*c3.size(),
...
void Task1106::Task_local::compute() {
  ...
  const Index c3 = b(3);
  // tensor label: I1563
  std::unique_ptr<double[]> odata = out()->move_block(x2, x3, c1, c3);
  std::unique_ptr<double[]> odata_sorted(new double[out()->get_size(x2, x3, c1, c3)]);
  std::fill_n(odata_sorted.get(), out()->get_size(x2, x3, c1, c3), 0.0);
  for (auto& c2 : *range_[0]) {
    // tensor label: f1
    std::unique_ptr<double[]> i0data = in(0)->get_block(c2, c3);
    std::unique_ptr<double[]> i0data_sorted(new double[in(0)->get_size(c2, c3)]);
    ...
    sort_indices<1,0,2,3,0,1,1,1>(i1data, i1data_sorted, x2.size(), c2.size(), x3.size(), c1.size());
    dgemm_("T", "N", c3.size(), x2.size()*x3.size()*c1.size(), c2.size(),
           1.0, i0data_sorted, c2.size(), i1data_sorted, c2.size(),
           1.0, odata_sorted, c3.size());
  }
  sort_indices<1,2,3,0,1,1,1,1>(odata_sorted, odata, c3.size(), x2.size(), x3.size(), c1.size());
  out()->put_block(odata, x2, x3, c1, c3);
}
...
void Task1107::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata = out()->move_block(x2, c2, x3, c1);
  {
    ...
    sort_indices<3,2,1,0,1,1,-1,1>(i0data, odata, c1.size(), x3.size(), c2.size(), x2.size());
  }
...
void Task1111::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata = out()->move_block(x2, c2, x3, c1);
  {
    // tensor label: t2
    std::unique_ptr<double[]> i0data = in(0)->get_block(c1, x3, c2, x2);
    dscal_(x2.size()*c2.size()*x3.size()*c1.size(), e0_, i0data.get(), 1);
    sort_indices<3,2,1,0,1,1,-1,2>(i0data, odata, c1.size(), x3.size(), c2.size(), x2.size());
  }
  out()->put_block(odata, x2, c2, x3, c1);
}
...
void Task1120::Task_local::compute() {
  ...
  for (auto& c1 : *range_[0]) {
    ...
    sort_indices<2,0,1,3,0,1,1,1>(i0data, i0data_sorted, x5.size(), x4.size(), c1.size(), x3.size());
    // tensor label: I1575
    std::unique_ptr<double[]> i1data = in(1)->get_block(x2, c1, x0, x1);
    std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x2, c1, x0, x1)]);
    sort_indices<1,0,2,3,0,1,1,1>(i1data, i1data_sorted, x2.size(), c1.size(), x0.size(), x1.size());
...
void Task1121::Task_local::compute() {
  ...
  const Index x1 = b(3);
  // tensor label: I1575
  std::unique_ptr<double[]> odata = out()->move_block(x2, c1, x0, x1);
  std::unique_ptr<double[]> odata_sorted(new double[out()->get_size(x2, c1, x0, x1)]);
  std::fill_n(odata_sorted.get(), out()->get_size(x2, c1, x0, x1), 0.0);
  for (auto& c2 : *range_[0]) {
    // tensor label: t2
    std::unique_ptr<double[]> i0data = in(0)->get_block(c1, x0, c2, x1);
    std::unique_ptr<double[]> i0data_sorted(new double[in(0)->get_size(c1, x0, c2, x1)]);
    ...
    dgemm_("T", "N", c1.size()*x0.size()*x1.size(), x2.size(), c2.size(),
           1.0, i0data_sorted, c2.size(), i1data_sorted, c2.size(),
           1.0, odata_sorted, c1.size()*x0.size()*x1.size());
  }
  sort_indices<3,0,1,2,1,1,1,1>(odata_sorted, odata, c1.size(), x0.size(), x1.size(), x2.size());
  out()->put_block(odata, x2, c1, x0, x1);
}
...
void Task1122::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata = out()->move_block(x2, c2);
  {
    ...
    sort_indices<0,1,1,1,1,2>(i0data, odata, x2.size(), c2.size());
  }
...
void Task1130::Task_local::compute() {
  ...
  for (auto& c2 : *range_[0]) {
    for (auto& c1 : *range_[0]) {
      ...
      // tensor label: I1571
      std::unique_ptr<double[]> i1data = in(1)->get_block(x1, c1, c2, x0);
      std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x1, c1, c2, x0)]);
      sort_indices<2,1,0,3,0,1,1,1>(i1data, i1data_sorted, x1.size(), c1.size(), c2.size(), x0.size());
      dgemm_("T", "N", x2.size()*x3.size(), x1.size()*x0.size(), c1.size()*c2.size(),
             1.0, i0data_sorted, c1.size()*c2.size(), i1data_sorted, c1.size()*c2.size(),
...
void Task1131::Task_local::compute() {
  ...
  const Index x0 = b(3);
  // tensor label: I1571
  std::unique_ptr<double[]> odata = out()->move_block(x1, c1, c2, x0);
  std::unique_ptr<double[]> odata_sorted(new double[out()->get_size(x1, c1, c2, x0)]);
  std::fill_n(odata_sorted.get(), out()->get_size(x1, c1, c2, x0), 0.0);
  for (auto& a3 : *range_[2]) {
    // tensor label: t2
    ...
    sort_indices<1,0,2,3,0,1,1,1>(i0data, i0data_sorted, c1.size(), a3.size(), c2.size(), x0.size());
    // tensor label: I1572
    ...
    dgemm_("T", "N", c1.size()*c2.size()*x0.size(), x1.size(), a3.size(),
           1.0, i0data_sorted, a3.size(), i1data_sorted, a3.size(),
           1.0, odata_sorted, c1.size()*c2.size()*x0.size());
  }
  sort_indices<3,0,1,2,1,1,1,1>(odata_sorted, odata, c1.size(), c2.size(), x0.size(), x1.size());
  out()->put_block(odata, x1, c1, c2, x0);
}
...
void Task1134::Task_local::compute() {
  ...
  for (auto& a2 : *range_[2]) {
    ...
    sort_indices<2,0,1,3,0,1,1,1>(i1data, i1data_sorted, x3.size(), c3.size(), a2.size(), c1.size());
    dgemm_("T", "N", x2.size(), x3.size()*c3.size()*c1.size(), a2.size(),
           1.0, i0data_sorted, a2.size(), i1data_sorted, a2.size(),
           1.0, odata_sorted, x2.size());
  }
  sort_indices<1,2,3,0,1,1,1,1>(odata_sorted, odata, x2.size(), x3.size(), c3.size(), c1.size());
...
void Task1141::Task_local::compute() {
  ...
  const Index x3 = b(3);
  // tensor label: I1567
  std::unique_ptr<double[]> odata = out()->move_block(x4, x5, c1, x3);
  ...
  for (auto& c2 : *range_[0]) {
    ...
    // tensor label: I1568
    std::unique_ptr<double[]> i1data = in(1)->get_block(x4, c2, x5, c1);
    std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x4, c2, x5, c1)]);
    sort_indices<1,0,2,3,0,1,1,1>(i1data, i1data_sorted, x4.size(), c2.size(), x5.size(), c1.size());
    dgemm_("T", "N", x3.size(), x4.size()*x5.size()*c1.size(), c2.size(),
           1.0, i0data_sorted, c2.size(), i1data_sorted, c2.size(),
           1.0, odata_sorted, x3.size());
  }
  sort_indices<1,2,3,0,1,1,1,1>(odata_sorted, odata, x3.size(), x4.size(), x5.size(), c1.size());
  out()->put_block(odata, x4, x5, c1, x3);
}
...
void Task1146::Task_local::compute() {
  ...
  for (auto& c1 : *range_[0]) {
    ...
    sort_indices<2,0,1,3,0,1,1,1>(i0data, i0data_sorted, x0.size(), x1.size(), c1.size(), x2.size());
    // tensor label: I1579
    std::unique_ptr<double[]> i1data = in(1)->get_block(x5, c1, x6, x7);
    std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x5, c1, x6, x7)]);
    sort_indices<1,0,2,3,0,1,1,1>(i1data, i1data_sorted, x5.size(), c1.size(), x6.size(), x7.size());
...
void Task1147::Task_local::compute() {
  ...
  const Index x7 = b(3);
  // tensor label: I1579
  std::unique_ptr<double[]> odata = out()->move_block(x5, c1, x6, x7);
  {
    // tensor label: t2
    std::unique_ptr<double[]> i0data = in(0)->get_block(x7, x6, c1, x5);
    sort_indices<3,2,1,0,1,1,1,4>(i0data, odata, x7.size(), x6.size(), c1.size(), x5.size());
  }
  out()->put_block(odata, x5, c1, x6, x7);
}
