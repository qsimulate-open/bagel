...
void Task921::Task_local::compute() {
  ...
  for (auto& a6 : *range_[2]) {
    for (auto& a5 : *range_[2]) {
      ...
      sort_indices<0,2,1,3,0,1,8,1>(i1data, i1data_sorted, a6.size(), a2.size(), a5.size(), a4.size());
...
void Task922::Task_local::compute() {
  ...
  for (auto& a5 : *range_[2]) {
    for (auto& a6 : *range_[2]) {
      ...
      std::unique_ptr<double[]> i1data = in(1)->get_block(a6, a2, a5, a4);
      std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(a6, a2, a5, a4)]);
      sort_indices<2,0,1,3,0,1,-4,1>(i1data, i1data_sorted, a6.size(), a2.size(), a5.size(), a4.size());
      dgemm_("T", "N", c1.size()*c3.size(), a2.size()*a4.size(), a6.size()*a5.size(),
             1.0, i0data_sorted, a6.size()*a5.size(), i1data_sorted, a6.size()*a5.size(),
             1.0, odata_sorted, c1.size()*c3.size());
    }
...
void Task946::Task_local::compute() {
  ...
  for (auto& x3 : *range_[1]) {
    for (auto& x2 : *range_[1]) {
      ...
      // tensor label: v2
      std::unique_ptr<double[]> i1data = in(1)->get_block(x3, x2, c2, a1);
      std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x3, x2, c2, a1)]);
      sort_indices<0,1,2,3,0,1,-1,2>(i1data, i1data_sorted, x3.size(), x2.size(), c2.size(), a1.size());
