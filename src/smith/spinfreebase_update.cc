...
void SpinFreeMethod::update_amplitude(shared_ptr<Tensor> t, shared_ptr<const Tensor> r, shared_ptr<const Denom> denom) const {
    ...
    for (auto& i0 : active_) {
      ...
      for (int j2 = i2.offset(), k = 0; j2 != i2.offset()+i2.size(); ++j2)
        for (int j0 = i0.offset(); j0 != i0.offset()+i0.size(); ++j0, ++k)
          copy_n(denom->shalf_xx()->element_ptr(0,(j0-nclo)+(j2-nclo)*nact), nact*nact, transp.get()+nact*nact*k);
    ...
    for (auto& i1 : active_) {
      ...
      for (int j3 = i3.offset(), k = 0; j3 != i3.offset()+i3.size(); ++j3)
        for (int j1 = i1.offset(); j1 != i1.offset()+i1.size(); ++j1, ++k)
          copy_n(denom->shalf_hh()->element_ptr(0,(j1-nclo)+(j3-nclo)*nact), nact*nact, transp.get()+nact*nact*k);
  ...
  for (auto& i3 : active_) {
    for (auto& i2 : active_) {
      assert(denom->shalf_xh());
      const int nact = ref_->nact();
      const int nclo = ref_->nclosed();
      unique_ptr<double[]> transp(new double[i2.size()*i3.size()*nact*nact*4]);
      for (int j3 = i3.offset(), k = 0; j3 != i3.offset()+i3.size(); ++j3)
        for (int j2 = i2.offset(); j2 != i2.offset()+i2.size(); ++j2, ++k) {
          copy_n(denom->shalf_xh()->element_ptr(0,             (j2-nclo)+(j3-nclo)*nact), nact*nact*2, transp.get()+nact*nact*2*k);
          copy_n(denom->shalf_xh()->element_ptr(0, nact*nact + (j2-nclo)+(j3-nclo)*nact), nact*nact*2, transp.get()+nact*nact*2*(k+i2.size()*i3.size()));
        }
    ...
    for (auto& i2 : active_) {
      for (auto& i0 : active_) {
        ...
        for (int j3 = i3.offset(), k = 0; j3 != i3.offset()+i3.size(); ++j3)
          for (int j2 = i2.offset(); j2 != i2.offset()+i2.size(); ++j2)
            for (int j0 = i0.offset(); j0 != i0.offset()+i0.size(); ++j0, ++k)
              copy_n(denom->shalf_xhh()->element_ptr(0,j0-nclo+nact*(j2-nclo+nact*(j3-nclo))), nact*nact*nact, transp.get()+nact*nact*nact*k);
    ...
    for (auto& i1 : active_) {
      for (auto& i0 : active_) {
        ...
        for (int j3 = i3.offset(), k = 0; j3 != i3.offset()+i3.size(); ++j3)
          for (int j1 = i1.offset(); j1 != i1.offset()+i1.size(); ++j1)
            for (int j0 = i0.offset(); j0 != i0.offset()+i0.size(); ++j0, ++k)
              copy_n(denom->shalf_xxh()->element_ptr(0,j0-nclo+nact*(j1-nclo+nact*(j3-nclo))), nact*nact*nact, transp.get()+nact*nact*nact*k);
