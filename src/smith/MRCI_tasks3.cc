...
void Task144::compute_() {
  if (!ta0_->initialized())
    ta0_->fill_local(0.0);
  (*ta0_)("c1, x2, x1, x0") += (*ta1_)("c1, c2") * (*ta2_)("c2, x2, x1, x0");
}
...
void Task145::compute_() {
  if (!ta0_->initialized())
    ta0_->fill_local(0.0);
  ta1_->init();
  (*ta0_)("c2, x2, x1, x0") += (*ta1_)("x5, x4, x2, x3, x1, x0") * (*ta2_)("x5, x4, c2, x3") * (-1);
}
