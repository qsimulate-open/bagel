...
void Task122::Task_local::compute() {
  ...
  const Index x3 = b(3);
  // tensor label: I8
  std::unique_ptr<double[]> odata = out()->move_block(c1, a3, c2, x3);
  {
    // tensor label: t2
    std::unique_ptr<double[]> i0data = in(0)->get_block(c1, a3, c2, x3);
    sort_indices<0,1,2,3,1,1,-1,1>(i0data, odata, c1.size(), a3.size(), c2.size(), x3.size());
  }
  out()->put_block(odata, c1, a3, c2, x3);
}
