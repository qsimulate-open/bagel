...
Task1660::Task1660(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  ...
  for (auto& x7 : *range[1])
    for (auto& a1 : *range[2])
      for (auto& x6 : *range[1])
        for (auto& x5 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x5, x6, a1, x7}}, in, t[0], range));
}
...
Task1669::Task1669(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range, const double e) {
  array<shared_ptr<const Tensor>,1> in = {{t[1]}};
...
Task1673::Task1673(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  ...
  for (auto& a2 : *range[2])
    for (auto& x5 : *range[1])
      for (auto& x4 : *range[1])
        for (auto& x3 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x3, x4, x5, a2}}, in, t[0], range));
}
...
Task1676::Task1676(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  ...
  for (auto& x1 : *range[1])
    for (auto& a1 : *range[2])
      for (auto& x0 : *range[1])
        for (auto& x2 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x2, x0, a1, x1}}, in, t[0], range));
}
...
Task1679::Task1679(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range, const double e) {
  array<shared_ptr<const Tensor>,1> in = {{t[1]}};
  subtasks_.reserve(range[1]->nblock()*range[1]->nblock()*range[2]->nblock()*range[1]->nblock());
  for (auto& x3 : *range[1])
    for (auto& x5 : *range[1])
