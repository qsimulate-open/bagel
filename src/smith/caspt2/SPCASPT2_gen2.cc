...
Task87::Task87(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& a3 : *range[2])
    for (auto& c2 : *range[0])
      if (t[0]->is_local(c2, a3))
        subtasks_.push_back(make_shared<Task_local>(array<const Index,2>{{c2, a3}}, in, t[0], range));
}
...
Task98::Task98(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x3 : *range[1])
    for (auto& x5 : *range[1])
      for (auto& x4 : *range[1])
        for (auto& c1 : *range[0])
          if (t[0]->is_local(c1, x4, x5, x3))
            subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c1, x4, x5, x3}}, in, t[0], range));
}
