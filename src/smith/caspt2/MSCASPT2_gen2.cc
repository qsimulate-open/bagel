...
Task60::Task60(vector<shared_ptr<Tensor>> t, const bool reset) : reset_(reset) {
  den2_ =  t[0];
}
...
Task84::Task84(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& a3 : *range[2])
    for (auto& x2 : *range[1])
      if (t[0]->is_local(x2, a3))
        subtasks_.push_back(make_shared<Task_local>(array<const Index,2>{{x2, a3}}, in, t[0], range));
}
...
Task95::Task95(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x3 : *range[1])
    for (auto& c2 : *range[0])
      if (t[0]->is_local(c2, x3))
        subtasks_.push_back(make_shared<Task_local>(array<const Index,2>{{c2, x3}}, in, t[0], range));
}
...
Task99::Task99(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x4 : *range[1])
    for (auto& x3 : *range[1])
      for (auto& x5 : *range[1])
        for (auto& a1 : *range[2])
          if (t[0]->is_local(a1, x5, x3, x4))
            subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{a1, x5, x3, x4}}, in, t[0], range));
}
