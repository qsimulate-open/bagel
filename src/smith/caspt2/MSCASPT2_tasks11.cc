...
void Task501::Task_local::compute() {
  ...
  for (auto& c2 : *range_[0]) {
    for (auto& a1 : *range_[2]) {
      ...
      // tensor label: I822
      std::unique_ptr<double[]> i1data = in(1)->get_block(a1, x0, x1, c2);
      std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(a1, x0, x1, c2)]);
      sort_indices<3,0,1,2,0,1,1,1>(i1data, i1data_sorted, a1.size(), x0.size(), x1.size(), c2.size());
...
void Task502::Task_local::compute() {
  ...
  const Index c2 = b(3);
  // tensor label: I822
  std::unique_ptr<double[]> odata(new double[out()->get_size(a1, x0, x1, c2)]);
  std::fill_n(odata.get(), out()->get_size(a1, x0, x1, c2), 0.0);
  std::unique_ptr<double[]> odata_sorted(new double[out()->get_size(a1, x0, x1, c2)]);
  ...
  for (auto& c3 : *range_[0]) {
    ...
    // tensor label: t2
    std::unique_ptr<double[]> i1data = in(1)->get_block(c3, a1, x0, x1);
    std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(c3, a1, x0, x1)]);
    sort_indices<0,1,2,3,0,1,1,1>(i1data, i1data_sorted, c3.size(), a1.size(), x0.size(), x1.size());
    dgemm_("T", "N", c2.size(), a1.size()*x0.size()*x1.size(), c3.size(),
...
void Task503::Task_local::compute() {
  ...
  for (auto& a3 : *range_[2]) {
    ...
    // tensor label: t2
    std::unique_ptr<double[]> i1data = in(1)->get_block(c2, a3, x0, x1);
    std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(c2, a3, x0, x1)]);
    sort_indices<1,0,2,3,0,1,-1,1>(i1data, i1data_sorted, c2.size(), a3.size(), x0.size(), x1.size());
    dgemm_("T", "N", a1.size(), c2.size()*x0.size()*x1.size(), a3.size(),
           1.0, i0data_sorted, a3.size(), i1data_sorted, a3.size(),
...
void Task504::Task_local::compute() {
  ...
  for (auto& a3 : *range_[2]) {
    ...
    sort_indices<1,0,2,3,0,1,-1,1>(i1data, i1data_sorted, x0.size(), a3.size(), c2.size(), a1.size());
    dgemm_("T", "N", x1.size(), x0.size()*c2.size()*a1.size(), a3.size(),
           1.0, i0data_sorted, a3.size(), i1data_sorted, a3.size(),
           1.0, odata_sorted, x1.size());
  }
...
void Task505::Task_local::compute() {
  ...
  for (auto& a2 : *range_[2]) {
    for (auto& c1 : *range_[0]) {
      ...
      // tensor label: I865
      std::unique_ptr<double[]> i1data = in(1)->get_block(x0, a2, x1, c1);
      std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x0, a2, x1, c1)]);
      sort_indices<1,3,0,2,0,1,1,1>(i1data, i1data_sorted, x0.size(), a2.size(), x1.size(), c1.size());
...
void Task506::Task_local::compute() {
  ...
  const Index c1 = b(3);
  // tensor label: I865
  std::unique_ptr<double[]> odata(new double[out()->get_size(x0, a2, x1, c1)]);
  std::fill_n(odata.get(), out()->get_size(x0, a2, x1, c1), 0.0);
  std::unique_ptr<double[]> odata_sorted(new double[out()->get_size(x0, a2, x1, c1)]);
  std::fill_n(odata_sorted.get(), out()->get_size(x0, a2, x1, c1), 0.0);
  for (auto& c3 : *range_[0]) {
    // tensor label: f1
    ...
    // tensor label: I866
    std::unique_ptr<double[]> i1data = in(1)->get_block(x0, a2, c3, x1);
    std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x0, a2, c3, x1)]);
    sort_indices<2,0,1,3,0,1,1,1>(i1data, i1data_sorted, x0.size(), a2.size(), c3.size(), x1.size());
    dgemm_("T", "N", c1.size(), x0.size()*a2.size()*x1.size(), c3.size(),
           1.0, i0data_sorted, c3.size(), i1data_sorted, c3.size(),
           1.0, odata_sorted, c1.size());
  }
  sort_indices<1,2,3,0,1,1,1,1>(odata_sorted, odata, c1.size(), x0.size(), a2.size(), x1.size());
  out()->add_block(odata, x0, a2, x1, c1);
}
...
void Task507::Task_local::compute() {
  ...
  const Index x1 = b(3);
  // tensor label: I866
  std::unique_ptr<double[]> odata(new double[out()->get_size(x0, a2, c3, x1)]);
  std::fill_n(odata.get(), out()->get_size(x0, a2, c3, x1), 0.0);
  {
    // tensor label: t2
    std::unique_ptr<double[]> i0data = in(0)->get_block(x0, a2, c3, x1);
    sort_indices<0,1,2,3,1,1,1,1>(i0data, odata, x0.size(), a2.size(), c3.size(), x1.size());
  }
  {
    // tensor label: t2
    std::unique_ptr<double[]> i1data = in(0)->get_block(c3, a2, x0, x1);
    sort_indices<2,1,0,3,1,1,-2,1>(i1data, odata, c3.size(), a2.size(), x0.size(), x1.size());
  }
  out()->add_block(odata, x0, a2, c3, x1);
}
...
void Task508::Task_local::compute() {
  ...
  for (auto& a3 : *range_[2]) {
    ...
    // tensor label: I870
    std::unique_ptr<double[]> i1data = in(1)->get_block(x0, a3, c1, x1);
    std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x0, a3, c1, x1)]);
    sort_indices<1,0,2,3,0,1,1,1>(i1data, i1data_sorted, x0.size(), a3.size(), c1.size(), x1.size());
    dgemm_("T", "N", a2.size(), x0.size()*c1.size()*x1.size(), a3.size(),
           1.0, i0data_sorted, a3.size(), i1data_sorted, a3.size(),
           1.0, odata_sorted, a2.size());
  }
  sort_indices<1,0,3,2,1,1,1,1>(odata_sorted, odata, a2.size(), x0.size(), c1.size(), x1.size());
  out()->add_block(odata, x0, a2, x1, c1);
}
...
void Task509::Task_local::compute() {
  ...
  const Index x1 = b(3);
  // tensor label: I870
  std::unique_ptr<double[]> odata(new double[out()->get_size(x0, a3, c1, x1)]);
  std::fill_n(odata.get(), out()->get_size(x0, a3, c1, x1), 0.0);
  {
    // tensor label: t2
    std::unique_ptr<double[]> i0data = in(0)->get_block(x0, a3, c1, x1);
    sort_indices<0,1,2,3,1,1,-1,1>(i0data, odata, x0.size(), a3.size(), c1.size(), x1.size());
  }
  {
    // tensor label: t2
    std::unique_ptr<double[]> i1data = in(0)->get_block(c1, a3, x0, x1);
    sort_indices<2,1,0,3,1,1,2,1>(i1data, odata, c1.size(), a3.size(), x0.size(), x1.size());
  }
  out()->add_block(odata, x0, a3, c1, x1);
}
...
void Task510::Task_local::compute() {
  ...
  for (auto& a3 : *range_[2]) {
    ...
    sort_indices<0,1,0,1,1,1>(i0data, i0data_sorted, a3.size(), x1.size());
    // tensor label: I897
    std::unique_ptr<double[]> i1data = in(1)->get_block(x0, a3, c1, a2);
    std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x0, a3, c1, a2)]);
    sort_indices<1,0,2,3,0,1,1,1>(i1data, i1data_sorted, x0.size(), a3.size(), c1.size(), a2.size());
    dgemm_("T", "N", x1.size(), x0.size()*c1.size()*a2.size(), a3.size(),
           1.0, i0data_sorted, a3.size(), i1data_sorted, a3.size(),
           1.0, odata_sorted, x1.size());
  }
  sort_indices<1,3,0,2,1,1,1,1>(odata_sorted, odata, x1.size(), x0.size(), c1.size(), a2.size());
  out()->add_block(odata, x0, a2, x1, c1);
}
...
void Task511::Task_local::compute() {
  ...
  const Index a2 = b(3);
  // tensor label: I897
  std::unique_ptr<double[]> odata(new double[out()->get_size(x0, a3, c1, a2)]);
  std::fill_n(odata.get(), out()->get_size(x0, a3, c1, a2), 0.0);
  {
    // tensor label: t2
    std::unique_ptr<double[]> i0data = in(0)->get_block(x0, a3, c1, a2);
    sort_indices<0,1,2,3,1,1,2,1>(i0data, odata, x0.size(), a3.size(), c1.size(), a2.size());
  }
  {
    // tensor label: t2
    std::unique_ptr<double[]> i1data = in(0)->get_block(x0, a2, c1, a3);
    sort_indices<0,3,2,1,1,1,-1,1>(i1data, odata, x0.size(), a2.size(), c1.size(), a3.size());
  }
  out()->add_block(odata, x0, a3, c1, a2);
}
...
void Task513::Task_local::compute() {
  ...
  const Index x2 = b(3);
  // tensor label: I969
  std::unique_ptr<double[]> odata(new double[out()->get_size(a3, c2, x3, x2)]);
  std::fill_n(odata.get(), out()->get_size(a3, c2, x3, x2), 0.0);
  ...
  for (auto& a1 : *range_[2]) {
    ...
    sort_indices<1,0,2,3,0,1,-1,1>(i1data, i1data_sorted, x3.size(), a1.size(), c2.size(), a3.size());
  ...
  sort_indices<3,2,1,0,1,1,1,1>(odata_sorted, odata, x2.size(), x3.size(), c2.size(), a3.size());
  out()->add_block(odata, a3, c2, x3, x2);
}
...
void Task514::Task_local::compute() {
  ...
  for (auto& c2 : *range_[0]) {
    for (auto& a3 : *range_[2]) {
      ...
      sort_indices<0,1,2,3,0,1,1,1>(i0data, i0data_sorted, c2.size(), a3.size(), x0.size(), x1.size());
      // tensor label: I977
      std::unique_ptr<double[]> i1data = in(1)->get_block(a3, c2, x3, x2);
      std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(a3, c2, x3, x2)]);
      sort_indices<1,0,2,3,0,1,1,1>(i1data, i1data_sorted, a3.size(), c2.size(), x3.size(), x2.size());
      dgemm_("T", "N", x0.size()*x1.size(), x3.size()*x2.size(), a3.size()*c2.size(),
             1.0, i0data_sorted, a3.size()*c2.size(), i1data_sorted, a3.size()*c2.size(),
             1.0, odata_sorted, x0.size()*x1.size());
    }
...
void Task515::Task_local::compute() {
  ...
  for (auto& a1 : *range_[2]) {
    ...
    std::unique_ptr<double[]> i1data = in(1)->get_block(x3, a1, c2, a3);
    std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x3, a1, c2, a3)]);
    sort_indices<1,0,2,3,0,1,2,1>(i1data, i1data_sorted, x3.size(), a1.size(), c2.size(), a3.size());
    dgemm_("T", "N", x2.size(), a3.size()*c2.size()*x3.size(), a1.size(),
           1.0, i0data_sorted, a1.size(), i1data_sorted, a1.size(),
           1.0, odata_sorted, x2.size());
  }
...
void Task517::Task_local::compute() {
  ...
  std::fill_n(odata.get(), out()->get_size(c2, a1, x3, x2), 0.0);
  {
    ...
    sort_indices<2,1,0,3,1,1,1,1>(i0data, odata, x3.size(), a1.size(), c2.size(), x2.size());
  }
...
void Task518::Task_local::compute() {
  ...
  sort_indices<3,2,1,0,1,1,1,1>(odata_sorted, odata, x2.size(), x3.size(), a1.size(), c2.size());
  out()->add_block(odata, c2, a1, x3, x2);
}
...
void Task521::Task_local::compute() {
  ...
  for (auto& a2 : *range_[2]) {
    for (auto& c1 : *range_[0]) {
      ...
      dscal_(x2.size()*x3.size()*a2.size()*c1.size(), e0_, i1data.get(), 1);
...
void Task527::Task_local::compute() {
  ...
  const Index x2 = b(3);
  // tensor label: I904
  std::unique_ptr<double[]> odata(new double[out()->get_size(x0, a1, x1, x2)]);
  std::fill_n(odata.get(), out()->get_size(x0, a1, x1, x2), 0.0);
  std::unique_ptr<double[]> odata_sorted(new double[out()->get_size(x0, a1, x1, x2)]);
  std::fill_n(odata_sorted.get(), out()->get_size(x0, a1, x1, x2), 0.0);
  for (auto& c2 : *range_[0]) {
    // tensor label: f1
    ...
    std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x0, a1, c2, x1)]);
    sort_indices<2,0,1,3,0,1,1,1>(i1data, i1data_sorted, x0.size(), a1.size(), c2.size(), x1.size());
    dgemm_("T", "N", x2.size(), x0.size()*a1.size()*x1.size(), c2.size(),
           1.0, i0data_sorted, c2.size(), i1data_sorted, c2.size(),
           1.0, odata_sorted, x2.size());
  }
  sort_indices<1,2,3,0,1,1,1,1>(odata_sorted, odata, x2.size(), x0.size(), a1.size(), x1.size());
  out()->add_block(odata, x0, a1, x1, x2);
}
