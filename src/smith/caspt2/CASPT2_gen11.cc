...
Task506::Task506(vector<shared_ptr<Tensor>> t, const bool reset) : reset_(reset) {
  den1_ =  t[0];
}
...
Task514::Task514(vector<shared_ptr<Tensor>> t, const bool reset) : reset_(reset) {
  Den1_ =  t[0];
}
...
Task537::Task537(vector<shared_ptr<Tensor>> t, const bool reset) : reset_(reset) {
  deci_ =  t[0];
}
...
Task544::Task544(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range, const double e) {
  array<shared_ptr<const Tensor>,1> in = {{t[1]}};
  out_ = t[0];
  in_ = in;
  subtasks_.reserve(range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock());
