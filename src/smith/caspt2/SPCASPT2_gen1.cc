    ...
    for (auto& x4 : *range[1])
      for (auto& x1 : *range[1])
        for (auto& x0 : *range[1])
          for (auto& x3 : *range[1])
            for (auto& x2 : *range[1])
              if (t[0]->is_local(x5, x4, x1, x0, x3, x2))
    ...
    for (auto& x5 : *range[1])
      for (auto& x1 : *range[1])
        for (auto& x0 : *range[1])
          for (auto& x3 : *range[1])
            for (auto& x4 : *range[1])
              if (t[0]->is_local(x2, x5, x1, x0, x3, x4))
                subtasks_.push_back(make_shared<Task_local>(array<const Index,6>{{x2, x5, x1, x0, x3, x4}}, in, t[0], range));
}
    ...
    for (auto& x0 : *range[1])
      for (auto& x2 : *range[1])
        for (auto& x1 : *range[1])
          for (auto& x3 : *range[1])
            for (auto& x4 : *range[1])
              if (t[0]->is_local(x5, x0, x2, x1, x3, x4))
                subtasks_.push_back(make_shared<Task_local>(array<const Index,6>{{x5, x0, x2, x1, x3, x4}}, in, t[0], range));
}
    ...
    for (auto& x4 : *range[1])
      for (auto& x2 : *range[1])
        for (auto& x1 : *range[1])
          for (auto& x3 : *range[1])
            for (auto& x0 : *range[1])
              if (t[0]->is_local(x5, x4, x2, x1, x3, x0))
                subtasks_.push_back(make_shared<Task_local>(array<const Index,6>{{x5, x4, x2, x1, x3, x0}}, in, t[0], range));
}
        ...
        for (auto& x5 : *range[1])
          for (auto& x1 : *range[1])
            for (auto& x0 : *range[1])
              for (auto& x4 : *range[1])
                for (auto& x3 : *range[1])
                  if (t[0]->is_local(x7, x6, x2, x5, x1, x0, x4, x3))
        ...
        for (auto& x5 : *range[1])
          for (auto& x2 : *range[1])
            for (auto& x1 : *range[1])
              for (auto& x4 : *range[1])
                for (auto& x3 : *range[1])
                  if (t[0]->is_local(x7, x0, x6, x5, x2, x1, x4, x3))
    ...
    for (auto& x4 : *range[1])
      for (auto& x1 : *range[1])
        for (auto& x0 : *range[1])
          for (auto& x2 : *range[1])
            for (auto& x3 : *range[1])
              if (t[0]->is_local(x5, x4, x1, x0, x2, x3))
                subtasks_.push_back(make_shared<Task_local>(array<const Index,6>{{x5, x4, x1, x0, x2, x3}}, in, t[0], range));
}
...
Task18::Task18(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x1 : *range[1])
    for (auto& x0 : *range[1])
      for (auto& x2 : *range[1])
        for (auto& x3 : *range[1])
          if (t[0]->is_local(x1, x0, x2, x3))
            subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x1, x0, x2, x3}}, in, t[0], range));
}
    ...
    for (auto& x4 : *range[1])
      for (auto& x1 : *range[1])
        for (auto& x0 : *range[1])
          for (auto& x5 : *range[1])
            for (auto& x3 : *range[1])
              if (t[0]->is_local(x2, x4, x1, x0, x5, x3))
                subtasks_.push_back(make_shared<Task_local>(array<const Index,6>{{x2, x4, x1, x0, x5, x3}}, in, t[0], range));
}
    ...
    for (auto& x0 : *range[1])
      for (auto& x2 : *range[1])
        for (auto& x1 : *range[1])
          for (auto& x4 : *range[1])
            for (auto& x3 : *range[1])
              if (t[0]->is_local(x5, x0, x2, x1, x4, x3))
                subtasks_.push_back(make_shared<Task_local>(array<const Index,6>{{x5, x0, x2, x1, x4, x3}}, in, t[0], range));
}
...
Task23::Task23(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x0 : *range[1])
    for (auto& x2 : *range[1])
      for (auto& x1 : *range[1])
        for (auto& x3 : *range[1])
          if (t[0]->is_local(x0, x2, x1, x3))
            subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x0, x2, x1, x3}}, in, t[0], range));
}
...
Task32::Task32(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x0 : *range[1])
    for (auto& x2 : *range[1])
      for (auto& x3 : *range[1])
        for (auto& x1 : *range[1])
          if (t[0]->is_local(x0, x2, x3, x1))
            subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x0, x2, x3, x1}}, in, t[0], range));
}
...
Task35::Task35(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x1 : *range[1])
    for (auto& x2 : *range[1])
      for (auto& x3 : *range[1])
        for (auto& x0 : *range[1])
          if (t[0]->is_local(x1, x2, x3, x0))
            subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x1, x2, x3, x0}}, in, t[0], range));
}
    ...
    for (auto& x3 : *range[1])
      for (auto& x1 : *range[1])
        for (auto& x0 : *range[1])
          for (auto& x5 : *range[1])
            for (auto& x2 : *range[1])
              if (t[0]->is_local(x4, x3, x1, x0, x5, x2))
                subtasks_.push_back(make_shared<Task_local>(array<const Index,6>{{x4, x3, x1, x0, x5, x2}}, in, t[0], range));
}
    ...
    for (auto& x3 : *range[1])
      for (auto& x2 : *range[1])
        for (auto& x1 : *range[1])
          for (auto& x5 : *range[1])
            for (auto& x0 : *range[1])
              if (t[0]->is_local(x4, x3, x2, x1, x5, x0))
                subtasks_.push_back(make_shared<Task_local>(array<const Index,6>{{x4, x3, x2, x1, x5, x0}}, in, t[0], range));
}

Task40::Task40(vector<shared_ptr<Tensor>> t, const bool reset) : reset_(reset) {
  den2_ =  t[0];
}
