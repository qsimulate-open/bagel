...
void Task450::Task_local::compute() {
  ...
  for (auto& c1 : *range_[0]) {
    ...
    // tensor label: I722
    std::unique_ptr<double[]> i1data = in(1)->get_block(x0, c1, x1, x2);
    std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x0, c1, x1, x2)]);
    sort_indices<1,0,2,3,0,1,1,1>(i1data, i1data_sorted, x0.size(), c1.size(), x1.size(), x2.size());
...
void Task451::Task_local::compute() {
  ...
  const Index x2 = b(3);
  // tensor label: I722
  std::unique_ptr<double[]> odata(new double[out()->get_size(x0, c1, x1, x2)]);
  std::fill_n(odata.get(), out()->get_size(x0, c1, x1, x2), 0.0);
  std::unique_ptr<double[]> odata_sorted(new double[out()->get_size(x0, c1, x1, x2)]);
  std::fill_n(odata_sorted.get(), out()->get_size(x0, c1, x1, x2), 0.0);
  for (auto& a2 : *range_[2]) {
    ...
    sort_indices<1,0,2,3,0,1,-1,1>(i1data, i1data_sorted, x0.size(), a2.size(), c1.size(), x1.size());
    dgemm_("T", "N", x2.size(), x0.size()*c1.size()*x1.size(), a2.size(),
           1.0, i0data_sorted, a2.size(), i1data_sorted, a2.size(),
           1.0, odata_sorted, x2.size());
  }
  sort_indices<1,2,3,0,1,1,1,1>(odata_sorted, odata, x2.size(), x0.size(), c1.size(), x1.size());
  out()->add_block(odata, x0, c1, x1, x2);
}
...
void Task459::Task_local::compute() {
  ...
  for (auto& a1 : *range_[2]) {
    for (auto& c2 : *range_[0]) {
      ...
      sort_indices<1,2,0,3,0,1,1,1>(i1data, i1data_sorted, x5.size(), a1.size(), c2.size(), x4.size());
...
void Task466::Task_local::compute() {
  ...
  std::fill_n(odata.get(), out()->get_size(c2, a1, x0, x1), 0.0);
  {
    ...
    sort_indices<2,1,0,3,1,1,-1,1>(i0data, odata, x0.size(), a1.size(), c2.size(), x1.size());
  }
...
void Task468::Task_local::compute() {
  ...
  for (auto& c2 : *range_[0]) {
    for (auto& a1 : *range_[2]) {
      ...
      // tensor label: I811
      std::unique_ptr<double[]> i1data = in(1)->get_block(x0, a1, x1, c2);
      std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x0, a1, x1, c2)]);
      sort_indices<3,1,0,2,0,1,1,1>(i1data, i1data_sorted, x0.size(), a1.size(), x1.size(), c2.size());
...
void Task469::Task_local::compute() {
  ...
  const Index c2 = b(3);
  // tensor label: I811
  std::unique_ptr<double[]> odata(new double[out()->get_size(x0, a1, x1, c2)]);
  std::fill_n(odata.get(), out()->get_size(x0, a1, x1, c2), 0.0);
  std::unique_ptr<double[]> odata_sorted(new double[out()->get_size(x0, a1, x1, c2)]);
  std::fill_n(odata_sorted.get(), out()->get_size(x0, a1, x1, c2), 0.0);
  for (auto& c3 : *range_[0]) {
    // tensor label: f1
    std::unique_ptr<double[]> i0data = in(0)->get_block(c2, c3);
    std::unique_ptr<double[]> i0data_sorted(new double[in(0)->get_size(c2, c3)]);
    sort_indices<1,0,0,1,1,1>(i0data, i0data_sorted, c2.size(), c3.size());
    // tensor label: t2
    std::unique_ptr<double[]> i1data = in(1)->get_block(x0, a1, c3, x1);
    std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x0, a1, c3, x1)]);
    sort_indices<2,0,1,3,0,1,-1,1>(i1data, i1data_sorted, x0.size(), a1.size(), c3.size(), x1.size());
    dgemm_("T", "N", c2.size(), x0.size()*a1.size()*x1.size(), c3.size(),
           1.0, i0data_sorted, c3.size(), i1data_sorted, c3.size(),
           1.0, odata_sorted, c2.size());
  }
  sort_indices<1,2,3,0,1,1,1,1>(odata_sorted, odata, c2.size(), x0.size(), a1.size(), x1.size());
  out()->add_block(odata, x0, a1, x1, c2);
}
...
void Task470::Task_local::compute() {
  ...
  for (auto& a3 : *range_[2]) {
    ...
    sort_indices<0,1,0,1,1,1>(i0data, i0data_sorted, a3.size(), a1.size());
    // tensor label: t2
    std::unique_ptr<double[]> i1data = in(1)->get_block(x0, a3, c2, x1);
    std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x0, a3, c2, x1)]);
    sort_indices<1,0,2,3,0,1,1,1>(i1data, i1data_sorted, x0.size(), a3.size(), c2.size(), x1.size());
    dgemm_("T", "N", a1.size(), x0.size()*c2.size()*x1.size(), a3.size(),
           1.0, i0data_sorted, a3.size(), i1data_sorted, a3.size(),
           1.0, odata_sorted, a1.size());
  }
  sort_indices<1,0,3,2,1,1,1,1>(odata_sorted, odata, a1.size(), x0.size(), c2.size(), x1.size());
  out()->add_block(odata, x0, a1, x1, c2);
}
...
void Task471::Task_local::compute() {
  ...
  for (auto& a3 : *range_[2]) {
    ...
    std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x0, a1, c2, a3)]);
    sort_indices<3,0,1,2,0,1,1,1>(i1data, i1data_sorted, x0.size(), a1.size(), c2.size(), a3.size());
    dgemm_("T", "N", x1.size(), x0.size()*a1.size()*c2.size(), a3.size(),
           1.0, i0data_sorted, a3.size(), i1data_sorted, a3.size(),
           1.0, odata_sorted, x1.size());
  }
  sort_indices<1,2,0,3,1,1,1,1>(odata_sorted, odata, x1.size(), x0.size(), a1.size(), c2.size());
  out()->add_block(odata, x0, a1, x1, c2);
}
...
void Task480::Task_local::compute() {
  ...
  for (auto& a2 : *range_[2]) {
    for (auto& c1 : *range_[0]) {
      ...
      dgemm_("T", "N", x0.size()*x1.size(), x4.size()*x5.size(), a2.size()*c1.size(),
             1.0, i0data_sorted, a2.size()*c1.size(), i1data_sorted, a2.size()*c1.size(),
             1.0, odata_sorted, x0.size()*x1.size());
    }
  }
...
void Task481::Task_local::compute() {
  ...
  for (auto& c1 : *range_[0]) {
    for (auto& a2 : *range_[2]) {
      ...
      std::unique_ptr<double[]> i1data = in(1)->get_block(c1, a2, x5, x4);
      std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(c1, a2, x5, x4)]);
      sort_indices<0,1,2,3,0,1,2,1>(i1data, i1data_sorted, c1.size(), a2.size(), x5.size(), x4.size());
      dgemm_("T", "N", x0.size()*x1.size(), x4.size()*x5.size(), a2.size()*c1.size(),
             1.0, i0data_sorted, a2.size()*c1.size(), i1data_sorted, a2.size()*c1.size(),
             1.0, odata_sorted, x0.size()*x1.size());
    }
