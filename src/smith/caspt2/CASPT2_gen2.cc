      ...
      for (auto& x0 : *range[1])
        for (auto& x3 : *range[1])
          for (auto& x4 : *range[1])
            for (auto& x2 : *range[1])
              for (auto& x1 : *range[1])
                if (t[0]->is_local(ci0, x5, x0, x3, x4, x2, x1))
      ...
      for (auto& x4 : *range[1])
        for (auto& x3 : *range[1])
          for (auto& x0 : *range[1])
            for (auto& x2 : *range[1])
              for (auto& x1 : *range[1])
                if (t[0]->is_local(ci0, x5, x4, x3, x0, x2, x1))
          ...
          for (auto& x5 : *range[1])
            for (auto& x2 : *range[1])
              for (auto& x1 : *range[1])
                for (auto& x4 : *range[1])
                  for (auto& x3 : *range[1])
                    if (t[0]->is_local(ci0, x7, x0, x6, x5, x2, x1))
      ...
      for (auto& x0 : *range[1])
        for (auto& x4 : *range[1])
          for (auto& x3 : *range[1])
            for (auto& x2 : *range[1])
              for (auto& x1 : *range[1])
                if (t[0]->is_local(ci0, x5, x0, x4, x3, x2, x1))
  ...
  for (auto& ci0 : *range[3])
    for (auto& x3 : *range[1])
      for (auto& x0 : *range[1])
        for (auto& x2 : *range[1])
          for (auto& x1 : *range[1])
            if (t[0]->is_local(ci0, x3, x0, x2, x1))
      ...
      for (auto& x0 : *range[1])
        for (auto& x4 : *range[1])
          for (auto& x1 : *range[1])
            for (auto& x3 : *range[1])
              for (auto& x2 : *range[1])
                if (t[0]->is_local(ci0, x5, x0, x4, x1))
      ...
      for (auto& x5 : *range[1])
        for (auto& x4 : *range[1])
          for (auto& x3 : *range[1])
            for (auto& x1 : *range[1])
              for (auto& x0 : *range[1])
                if (t[0]->is_local(ci0, x2, x5, x4, x3, x1, x0))
      ...
      for (auto& x4 : *range[1])
        for (auto& x0 : *range[1])
          for (auto& x3 : *range[1])
            for (auto& x2 : *range[1])
              for (auto& x1 : *range[1])
                if (t[0]->is_local(ci0, x5, x4, x0, x3, x2, x1))
...
Task72::Task72(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range, const double e) {
  array<shared_ptr<const Tensor>,2> in = {{t[1], t[2]}};
  out_ = t[0];
  in_ = in;
  subtasks_.reserve(range[1]->nblock()*range[1]->nblock()*range[0]->nblock()*range[0]->nblock());
  for (auto& x1 : *range[1])
    for (auto& x0 : *range[1])
      for (auto& c2 : *range[0])
        for (auto& c1 : *range[0])
          if (t[0]->is_local(c1, c2, x0, x1))
            subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c1, c2, x0, x1}}, in, t[0], range, e));
}
...
Task74::Task74(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c2 : *range[0])
    for (auto& x1 : *range[1])
      for (auto& x0 : *range[1])
        for (auto& c1 : *range[0])
          if (t[0]->is_local(c1, x0, x1, c2))
            subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c1, x0, x1, c2}}, in, t[0], range));
}
...
Task75::Task75(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x1 : *range[1])
    for (auto& x0 : *range[1])
      for (auto& c3 : *range[0])
        for (auto& c1 : *range[0])
          if (t[0]->is_local(c1, c3, x0, x1))
            subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c1, c3, x0, x1}}, in, t[0], range));
}
