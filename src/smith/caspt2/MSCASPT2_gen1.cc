...
Task28::Task28(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  array<shared_ptr<const Tensor>,5> in = {{t[1], t[2], t[3], t[4], t[5]}};
  out_ = t[0];
  in_ = in;
  subtasks_.reserve(range[3]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock());
      ...
      for (auto& x5 : *range[1])
        for (auto& x1 : *range[1])
          for (auto& x4 : *range[1])
            for (auto& x3 : *range[1])
              for (auto& x2 : *range[1])
                if (t[0]->is_local(ci0, x0, x5, x1, x4))
                  subtasks_.push_back(make_shared<Task_local>(array<const Index,7>{{ci0, x0, x5, x1, x4, x3, x2}}, in, t[0], range));
}

Task29::Task29(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  array<shared_ptr<const Tensor>,3> in = {{t[1], t[2], t[3]}};
  out_ = t[0];
  in_ = in;
  subtasks_.reserve(range[3]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock());
  for (auto& ci0 : *range[3])
    for (auto& x0 : *range[1])
      for (auto& x3 : *range[1])
        for (auto& x1 : *range[1])
          for (auto& x2 : *range[1])
            if (t[0]->is_local(ci0, x0, x3, x1, x2))
              subtasks_.push_back(make_shared<Task_local>(array<const Index,5>{{ci0, x0, x3, x1, x2}}, in, t[0], range));
}

Task30::Task30(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  array<shared_ptr<const Tensor>,3> in = {{t[1], t[2], t[3]}};
  out_ = t[0];
  in_ = in;
  subtasks_.reserve(range[3]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock());
      ...
      for (auto& x4 : *range[1])
        for (auto& x0 : *range[1])
          for (auto& x3 : *range[1])
            for (auto& x1 : *range[1])
              for (auto& x2 : *range[1])
                if (t[0]->is_local(ci0, x5, x4, x0, x3, x1, x2))
                  subtasks_.push_back(make_shared<Task_local>(array<const Index,7>{{ci0, x5, x4, x0, x3, x1, x2}}, in, t[0], range));
}

Task31::Task31(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  array<shared_ptr<const Tensor>,3> in = {{t[1], t[2], t[3]}};
  out_ = t[0];
  in_ = in;
  subtasks_.reserve(range[3]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock());
  for (auto& ci0 : *range[3])
    for (auto& x1 : *range[1])
      for (auto& x3 : *range[1])
        for (auto& x0 : *range[1])
          for (auto& x2 : *range[1])
            if (t[0]->is_local(ci0, x1, x3, x0, x2))
              subtasks_.push_back(make_shared<Task_local>(array<const Index,5>{{ci0, x1, x3, x0, x2}}, in, t[0], range));
}

Task32::Task32(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  array<shared_ptr<const Tensor>,3> in = {{t[1], t[2], t[3]}};
  out_ = t[0];
  in_ = in;
  subtasks_.reserve(range[3]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock());
      ...
      for (auto& x5 : *range[1])
        for (auto& x3 : *range[1])
          for (auto& x4 : *range[1])
            for (auto& x1 : *range[1])
              for (auto& x0 : *range[1])
                if (t[0]->is_local(ci0, x2, x5, x3, x4, x1, x0))
                  subtasks_.push_back(make_shared<Task_local>(array<const Index,7>{{ci0, x2, x5, x3, x4, x1, x0}}, in, t[0], range));
}

Task33::Task33(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  array<shared_ptr<const Tensor>,5> in = {{t[1], t[2], t[3], t[4], t[5]}};
  out_ = t[0];
  in_ = in;
  subtasks_.reserve(range[3]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock());
  for (auto& ci0 : *range[3])
    for (auto& x7 : *range[1])
      for (auto& x6 : *range[1])
        for (auto& x2 : *range[1])
          for (auto& x5 : *range[1])
            for (auto& x1 : *range[1])
              for (auto& x0 : *range[1])
                for (auto& x4 : *range[1])
                  for (auto& x3 : *range[1])
                    if (t[0]->is_local(ci0, x7, x6, x2, x5, x1, x0))
                      subtasks_.push_back(make_shared<Task_local>(array<const Index,9>{{ci0, x7, x6, x2, x5, x1, x0, x4, x3}}, in, t[0], range));
}

Task34::Task34(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  array<shared_ptr<const Tensor>,3> in = {{t[1], t[2], t[3]}};
  out_ = t[0];
  in_ = in;
  subtasks_.reserve(range[3]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock());
      ...
      for (auto& x4 : *range[1])
        for (auto& x2 : *range[1])
          for (auto& x3 : *range[1])
            for (auto& x1 : *range[1])
              for (auto& x0 : *range[1])
                if (t[0]->is_local(ci0, x5, x4, x2, x3, x1, x0))
                  subtasks_.push_back(make_shared<Task_local>(array<const Index,7>{{ci0, x5, x4, x2, x3, x1, x0}}, in, t[0], range));
}

Task35::Task35(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  array<shared_ptr<const Tensor>,2> in = {{t[1], t[2]}};
  out_ = t[0];
  in_ = in;
  subtasks_.reserve(range[3]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock());
  for (auto& ci0 : *range[3])
    for (auto& x2 : *range[1])
      for (auto& x3 : *range[1])
        for (auto& x1 : *range[1])
          for (auto& x0 : *range[1])
            if (t[0]->is_local(ci0, x2, x3, x1, x0))
              subtasks_.push_back(make_shared<Task_local>(array<const Index,5>{{ci0, x2, x3, x1, x0}}, in, t[0], range));
}

Task36::Task36(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,4> range) {
  array<shared_ptr<const Tensor>,3> in = {{t[1], t[2], t[3]}};
  out_ = t[0];
  in_ = in;
  subtasks_.reserve(range[3]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock()*range[1]->nblock());
      ...
      for (auto& x3 : *range[1])
        for (auto& x2 : *range[1])
          for (auto& x4 : *range[1])
            for (auto& x1 : *range[1])
              for (auto& x0 : *range[1])
                if (t[0]->is_local(ci0, x5, x3, x2, x4, x1, x0))
  ...
  for (auto& ci0 : *range[3])
    for (auto& x3 : *range[1])
      for (auto& x2 : *range[1])
        for (auto& x0 : *range[1])
          for (auto& x1 : *range[1])
            if (t[0]->is_local(ci0, x3, x2, x0, x1))
  ...
  for (auto& ci0 : *range[3])
    for (auto& x3 : *range[1])
      for (auto& x1 : *range[1])
        for (auto& x0 : *range[1])
          for (auto& x2 : *range[1])
            if (t[0]->is_local(ci0, x3, x1, x0, x2))
      ...
      for (auto& x4 : *range[1])
        for (auto& x1 : *range[1])
          for (auto& x3 : *range[1])
            for (auto& x2 : *range[1])
              for (auto& x0 : *range[1])
                if (t[0]->is_local(ci0, x5, x4, x1, x3, x2, x0))
  ...
  for (auto& ci0 : *range[3])
    for (auto& x1 : *range[1])
      for (auto& x3 : *range[1])
        for (auto& x2 : *range[1])
          for (auto& x0 : *range[1])
            if (t[0]->is_local(ci0, x1, x3, x2, x0))
      ...
      for (auto& x0 : *range[1])
        for (auto& x1 : *range[1])
          for (auto& x4 : *range[1])
            for (auto& x3 : *range[1])
              for (auto& x2 : *range[1])
                if (t[0]->is_local(ci0, x5, x0, x1, x4))
  ...
  for (auto& ci0 : *range[3])
    for (auto& x3 : *range[1])
      for (auto& x0 : *range[1])
        for (auto& x1 : *range[1])
          for (auto& x2 : *range[1])
            if (t[0]->is_local(ci0, x3, x0, x1, x2))
      ...
      for (auto& x4 : *range[1])
        for (auto& x1 : *range[1])
          for (auto& x0 : *range[1])
            for (auto& x3 : *range[1])
              for (auto& x2 : *range[1])
                if (t[0]->is_local(ci0, x5, x4, x1, x0))
  ...
  for (auto& ci0 : *range[3])
    for (auto& x3 : *range[1])
      for (auto& x2 : *range[1])
        for (auto& x1 : *range[1])
          for (auto& x0 : *range[1])
            if (t[0]->is_local(ci0, x3, x2, x1, x0))
      ...
      for (auto& x0 : *range[1])
        for (auto& x4 : *range[1])
          for (auto& x3 : *range[1])
            for (auto& x1 : *range[1])
              for (auto& x2 : *range[1])
                if (t[0]->is_local(ci0, x5, x0, x4, x3, x1, x2))
      ...
      for (auto& x2 : *range[1])
        for (auto& x4 : *range[1])
          for (auto& x3 : *range[1])
            for (auto& x1 : *range[1])
              for (auto& x0 : *range[1])
                if (t[0]->is_local(ci0, x5, x2, x4, x3, x1, x0))
