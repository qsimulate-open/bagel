    ...
    for (auto& x0 : *range[1])
      for (auto& x4 : *range[1])
        for (auto& x3 : *range[1])
          for (auto& x2 : *range[1])
            for (auto& x1 : *range[1])
              if (t[0]->is_local(x1, x2, x3, x4, x0, x5))
...
Task284::Task284(vector<shared_ptr<Tensor>> t, const bool reset) : reset_(reset) {
  den2_ =  t[0];
}
