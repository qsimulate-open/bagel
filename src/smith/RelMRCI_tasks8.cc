...
void Task350::Task_local::compute() {
  ...
  const Index x3 = b(3);
  // tensor label: I76
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x2, a1, c2, x3);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(x2, a1, c2, x3)]);
  std::fill_n(odata_sorted.get(), out()->get_size(x2, a1, c2, x3), 0.0);
  for (auto& c4 : *range_[0]) {
    for (auto& c3 : *range_[0]) {
      // tensor label: t2
      std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c4, a1, c3, x3);
      std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(c4, a1, c3, x3)]);
      sort_indices<0,2,1,3,0,1,1,1>(i0data, i0data_sorted, c4.size(), a1.size(), c3.size(), x3.size());
      // tensor label: v2
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x2, c4, c2, c3);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x2, c4, c2, c3)]);
      sort_indices<1,3,0,2,0,1,-1,1>(i1data, i1data_sorted, x2.size(), c4.size(), c2.size(), c3.size());
      zgemm3m_("T", "N", a1.size()*x3.size(), x2.size()*c2.size(), c4.size()*c3.size(),
...
void Task351::Task_local::compute() {
  ...
  for (auto& c4 : *range_[0]) {
    for (auto& a3 : *range_[2]) {
      ...
      // tensor label: v2
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x2, c4, a3, a1);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x2, c4, a3, a1)]);
      sort_indices<1,2,0,3,0,1,1,1>(i1data, i1data_sorted, x2.size(), c4.size(), a3.size(), a1.size());
      zgemm3m_("T", "N", c2.size()*x3.size(), x2.size()*a1.size(), c4.size()*a3.size(),
...
void Task352::Task_local::compute() {
  ...
  for (auto& c3 : *range_[0]) {
    for (auto& a4 : *range_[2]) {
      // tensor label: t2
      std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c3, a4, c2, x3);
      std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(c3, a4, c2, x3)]);
      sort_indices<0,1,2,3,0,1,1,1>(i0data, i0data_sorted, c3.size(), a4.size(), c2.size(), x3.size());
      // tensor label: v2
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x2, a1, a4, c3);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x2, a1, a4, c3)]);
      sort_indices<3,2,0,1,0,1,-1,1>(i1data, i1data_sorted, x2.size(), a1.size(), a4.size(), c3.size());
      zgemm3m_("T", "N", c2.size()*x3.size(), x2.size()*a1.size(), a4.size()*c3.size(),
...
void Task353::Task_local::compute() {
  ...
  for (auto& a4 : *range_[2]) {
    for (auto& c3 : *range_[0]) {
      // tensor label: t2
      std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c2, a4, c3, x3);
      std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(c2, a4, c3, x3)]);
...
void Task354::Task_local::compute() {
  ...
  for (auto& a4 : *range_[2]) {
    for (auto& c3 : *range_[0]) {
      ...
      std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(c2, a4, c3, a1)]);
      ...
      zgemm3m_("T", "N", c2.size()*a1.size(), x3.size()*x2.size(), a4.size()*c3.size(),
             1.0, i0data_sorted, a4.size()*c3.size(), i1data_sorted, a4.size()*c3.size(),
...
void Task355::Task_local::compute() {
  ...
  for (auto& x2 : *range_[1]) {
    for (auto& x3 : *range_[1]) {
      ...
      // tensor label: I79
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x2, c2, a1, x3);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x2, c2, a1, x3)]);
      sort_indices<0,3,1,2,0,1,1,1>(i1data, i1data_sorted, x2.size(), c2.size(), a1.size(), x3.size());
      zgemm3m_("T", "N", x1.size()*x0.size(), c2.size()*a1.size(), x2.size()*x3.size(),
...
void Task356::Task_local::compute() {
  ...
  const Index x3 = b(3);
  // tensor label: I79
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x2, c2, a1, x3);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(x2, c2, a1, x3)]);
  std::fill_n(odata_sorted.get(), out()->get_size(x2, c2, a1, x3), 0.0);
  for (auto& c3 : *range_[0]) {
    // tensor label: t2
    std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c2, a1, c3, x3);
    std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(c2, a1, c3, x3)]);
    ...
    zgemm3m_("T", "N", c2.size()*a1.size()*x3.size(), x2.size(), c3.size(),
...
void Task357::Task_local::compute() {
  ...
  for (auto& c3 : *range_[0]) {
    for (auto& c4 : *range_[0]) {
      // tensor label: t2
      std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c3, a1, c4, x3);
      std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(c3, a1, c4, x3)]);
...
void Task359::Task_local::compute() {
  ...
  for (auto& c3 : *range_[0]) {
    for (auto& a4 : *range_[2]) {
      // tensor label: t2
      std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c2, a1, c3, a4);
      std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(c2, a1, c3, a4)]);
...
void Task361::Task_local::compute() {
  ...
  for (auto& x3 : *range_[1]) {
    for (auto& x2 : *range_[1]) {
      ...
      // tensor label: I84
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c2, x3, a1, x2);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(c2, x3, a1, x2)]);
      sort_indices<1,3,0,2,0,1,1,1>(i1data, i1data_sorted, c2.size(), x3.size(), a1.size(), x2.size());
      zgemm3m_("T", "N", x0.size()*x1.size(), c2.size()*a1.size(), x3.size()*x2.size(),
...
void Task362::Task_local::compute() {
  ...
  const Index x2 = b(3);
  // tensor label: I84
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c2, x3, a1, x2);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(c2, x3, a1, x2)]);
  std::fill_n(odata_sorted.get(), out()->get_size(c2, x3, a1, x2), 0.0);
  for (auto& c3 : *range_[0]) {
    // tensor label: t2
    std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x3, a1, c3, x2);
    std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(x3, a1, c3, x2)]);
    ...
    zgemm3m_("T", "N", x3.size()*a1.size()*x2.size(), c2.size(), c3.size(),
           1.0, i0data_sorted, c3.size(), i1data_sorted, c3.size(),
...
void Task363::Task_local::compute() {
  ...
  for (auto& a3 : *range_[2]) {
    ...
    std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(x3, a3, c2, x2)]);
    sort_indices<1,0,2,3,0,1,1,1>(i0data, i0data_sorted, x3.size(), a3.size(), c2.size(), x2.size());
    // tensor label: h1
    std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(a3, a1);
    std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(a3, a1)]);
    sort_indices<0,1,0,1,1,1>(i1data, i1data_sorted, a3.size(), a1.size());
    zgemm3m_("T", "N", x3.size()*c2.size()*x2.size(), a1.size(), a3.size(),
...
void Task364::Task_local::compute() {
  ...
  for (auto& a3 : *range_[2]) {
    ...
    zgemm3m_("T", "N", x3.size()*a1.size()*c2.size(), x2.size(), a3.size(),
           1.0, i0data_sorted, a3.size(), i1data_sorted, a3.size(),
...
void Task365::Task_local::compute() {
  ...
  for (auto& a3 : *range_[2]) {
    for (auto& c4 : *range_[0]) {
      ...
      // tensor label: v2
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c2, c4, a3, a1);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(c2, c4, a3, a1)]);
      sort_indices<2,1,0,3,0,1,-1,1>(i1data, i1data_sorted, c2.size(), c4.size(), a3.size(), a1.size());
      zgemm3m_("T", "N", x3.size()*x2.size(), c2.size()*a1.size(), c4.size()*a3.size(),
             1.0, i0data_sorted, c4.size()*a3.size(), i1data_sorted, c4.size()*a3.size(),
...
void Task366::Task_local::compute() {
  ...
  for (auto& a3 : *range_[2]) {
    for (auto& c4 : *range_[0]) {
      ...
      zgemm3m_("T", "N", c2.size()*a1.size(), x3.size()*x2.size(), c4.size()*a3.size(),
             1.0, i0data_sorted, c4.size()*a3.size(), i1data_sorted, c4.size()*a3.size(),
             1.0, odata_sorted, c2.size()*a1.size());
    }
  }
...
void Task367::Task_local::compute() {
  ...
  for (auto& c3 : *range_[0]) {
    for (auto& a4 : *range_[2]) {
      ...
      zgemm3m_("T", "N", x3.size()*a1.size(), x2.size()*c2.size(), a4.size()*c3.size(),
             1.0, i0data_sorted, a4.size()*c3.size(), i1data_sorted, a4.size()*c3.size(),
...
void Task368::Task_local::compute() {
  ...
  const Index c3 = b(3);
  // tensor label: I835
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(a4, x2, c2, c3);
  {
    // tensor label: v2
...
void Task369::Task_local::compute() {
  ...
  for (auto& a4 : *range_[2]) {
    for (auto& c3 : *range_[0]) {
      ...
      zgemm3m_("T", "N", x3.size()*a1.size(), c2.size()*x2.size(), a4.size()*c3.size(),
             1.0, i0data_sorted, a4.size()*c3.size(), i1data_sorted, a4.size()*c3.size(),
             1.0, odata_sorted, x3.size()*a1.size());
    }
  }
...
void Task370::Task_local::compute() {
  ...
  for (auto& a3 : *range_[2]) {
    for (auto& a4 : *range_[2]) {
      ...
      // tensor label: v2
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(a4, x2, a3, a1);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(a4, x2, a3, a1)]);
      sort_indices<2,0,1,3,0,1,1,1>(i1data, i1data_sorted, a4.size(), x2.size(), a3.size(), a1.size());
      zgemm3m_("T", "N", x3.size()*c2.size(), x2.size()*a1.size(), a4.size()*a3.size(),
...
void Task373::Task_local::compute() {
  ...
  const Index x2 = b(3);
  // tensor label: I92
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c2, a1, x3, x2);
  ...
  for (auto& c3 : *range_[0]) {
    // tensor label: t2
    std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c3, a1, x3, x2);
    std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(c3, a1, x3, x2)]);
    sort_indices<0,1,2,3,0,1,1,1>(i0data, i0data_sorted, c3.size(), a1.size(), x3.size(), x2.size());
    // tensor label: h1
    std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c2, c3);
    std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(c2, c3)]);
    sort_indices<1,0,0,1,1,1>(i1data, i1data_sorted, c2.size(), c3.size());
    zgemm3m_("T", "N", a1.size()*x3.size()*x2.size(), c2.size(), c3.size(),
           1.0, i0data_sorted, c3.size(), i1data_sorted, c3.size(),
...
void Task374::Task_local::compute() {
  ...
  for (auto& a3 : *range_[2]) {
    // tensor label: t2
    std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c2, a3, x3, x2);
    std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(c2, a3, x3, x2)]);
    sort_indices<1,0,2,3,0,1,1,1>(i0data, i0data_sorted, c2.size(), a3.size(), x3.size(), x2.size());
    // tensor label: h1
    std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(a3, a1);
    std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(a3, a1)]);
    sort_indices<0,1,0,1,-1,1>(i1data, i1data_sorted, a3.size(), a1.size());
    zgemm3m_("T", "N", c2.size()*x3.size()*x2.size(), a1.size(), a3.size(),
           1.0, i0data_sorted, a3.size(), i1data_sorted, a3.size(),
...
void Task375::Task_local::compute() {
  ...
  for (auto& a3 : *range_[2]) {
    ...
    std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(a3, x2)]);
    sort_indices<0,1,0,1,-1,1>(i1data, i1data_sorted, a3.size(), x2.size());
    zgemm3m_("T", "N", x3.size()*c2.size()*a1.size(), x2.size(), a3.size(),
           1.0, i0data_sorted, a3.size(), i1data_sorted, a3.size(),
...
void Task376::Task_local::compute() {
  ...
  for (auto& a4 : *range_[2]) {
    for (auto& c3 : *range_[0]) {
      ...
      // tensor label: v2
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c2, a1, a4, c3);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(c2, a1, a4, c3)]);
      sort_indices<2,3,0,1,0,1,1,1>(i1data, i1data_sorted, c2.size(), a1.size(), a4.size(), c3.size());
      zgemm3m_("T", "N", x3.size()*x2.size(), c2.size()*a1.size(), a4.size()*c3.size(),
             1.0, i0data_sorted, a4.size()*c3.size(), i1data_sorted, a4.size()*c3.size(),
             1.0, odata_sorted, x3.size()*x2.size());
    }
  }
...
void Task389::Task_local::compute() {
  ...
  const Index a1 = b(1);
  // tensor label: I101
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c2, a1);
  ...
  for (auto& a4 : *range_[2]) {
    for (auto& c3 : *range_[0]) {
      ...
      zgemm3m_("T", "N", c2.size()*a1.size(), 1, a4.size()*c3.size(),
             1.0, i0data_sorted, a4.size()*c3.size(), i1data_sorted, a4.size()*c3.size(),
             1.0, odata_sorted, c2.size()*a1.size());
    }
  }
  ...
  for (auto& c3 : *range_[0]) {
    for (auto& a4 : *range_[2]) {
      for (auto& c5 : *range_[0]) {
        // tensor label: t2
        std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c3, a4, c5, a1);
        std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(c3, a4, c5, a1)]);
        sort_indices<0,1,2,3,0,1,1,1>(i0data, i0data_sorted, c3.size(), a4.size(), c5.size(), a1.size());
        // tensor label: v2
        std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c2, c5, a4, c3);
        std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(c2, c5, a4, c3)]);
        sort_indices<3,2,1,0,0,1,2,1>(i1data, i1data_sorted, c2.size(), c5.size(), a4.size(), c3.size());
        zgemm3m_("T", "N", a1.size(), c2.size(), c5.size()*a4.size()*c3.size(),
...
void Task392::Task_local::compute() {
  ...
  for (auto& c3 : *range_[0]) {
    for (auto& c5 : *range_[0]) {
      for (auto& a4 : *range_[2]) {
        // tensor label: t2
        std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c3, a1, c5, a4);
        std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(c3, a1, c5, a4)]);
...
void Task393::Task_local::compute() {
  ...
  for (auto& a5 : *range_[2]) {
    for (auto& c3 : *range_[0]) {
      for (auto& a4 : *range_[2]) {
        // tensor label: t2
        std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c2, a5, c3, a4);
        std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(c2, a5, c3, a4)]);
        sort_indices<1,2,3,0,0,1,1,1>(i0data, i0data_sorted, c2.size(), a5.size(), c3.size(), a4.size());
        // tensor label: v2
        std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(a5, a1, a4, c3);
        std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(a5, a1, a4, c3)]);
        sort_indices<0,3,2,1,0,1,-2,1>(i1data, i1data_sorted, a5.size(), a1.size(), a4.size(), c3.size());
        zgemm3m_("T", "N", c2.size(), a1.size(), a5.size()*a4.size()*c3.size(),
...
void Task394::Task_local::compute() {
  ...
  for (auto& a4 : *range_[2]) {
    for (auto& c3 : *range_[0]) {
      for (auto& a5 : *range_[2]) {
        // tensor label: t2
        std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c2, a4, c3, a5);
        std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(c2, a4, c3, a5)]);
...
void Task395::Task_local::compute() {
  ...
  for (auto& x3 : *range_[1]) {
    for (auto& x2 : *range_[1]) {
      for (auto& c3 : *range_[0]) {
        // tensor label: v2
        std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x3, a1, x2, c3);
        std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(x3, a1, x2, c3)]);
        sort_indices<0,2,3,1,0,1,1,1>(i0data, i0data_sorted, x3.size(), a1.size(), x2.size(), c3.size());
        // tensor label: I662
        std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c2, c3, x1, x2, x3, x0);
        std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(c2, c3, x1, x2, x3, x0)]);
        sort_indices<4,3,1,0,2,5,0,1,1,1>(i1data, i1data_sorted, c2.size(), c3.size(), x1.size(), x2.size(), x3.size(), x0.size());
        zgemm3m_("T", "N", a1.size(), c2.size()*x1.size()*x0.size(), c3.size()*x2.size()*x3.size(),
...
void Task396::Task_local::compute() {
  ...
  const Index x0 = b(5);
  // tensor label: I662
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c2, c3, x1, x2, x3, x0);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(c2, c3, x1, x2, x3, x0)]);
  std::fill_n(odata_sorted.get(), out()->get_size(c2, c3, x1, x2, x3, x0), 0.0);
  for (auto& x5 : *range_[1]) {
    for (auto& x4 : *range_[1]) {
      // tensor label: Gamma219
      std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x1, x5, x2, x4, x3, x0);
      std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(x1, x5, x2, x4, x3, x0)]);
      sort_indices<1,3,0,2,4,5,0,1,1,1>(i0data, i0data_sorted, x1.size(), x5.size(), x2.size(), x4.size(), x3.size(), x0.size());
      // tensor label: t2
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c2, x5, c3, x4);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(c2, x5, c3, x4)]);
      sort_indices<1,3,0,2,0,1,2,1>(i1data, i1data_sorted, c2.size(), x5.size(), c3.size(), x4.size());
      zgemm3m_("T", "N", x1.size()*x2.size()*x3.size()*x0.size(), c2.size()*c3.size(), x5.size()*x4.size(),
             1.0, i0data_sorted, x5.size()*x4.size(), i1data_sorted, x5.size()*x4.size(),
...
void Task397::Task_local::compute() {
  ...
  for (auto& x4 : *range_[1]) {
    for (auto& x3 : *range_[1]) {
      for (auto& x2 : *range_[1]) {
        // tensor label: v2
        std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x4, a1, x3, x2);
        std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(x4, a1, x3, x2)]);
        sort_indices<0,2,3,1,0,1,1,1>(i0data, i0data_sorted, x4.size(), a1.size(), x3.size(), x2.size());
        // tensor label: I665
        std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c2, x1, x4, x0, x3, x2);
        std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(c2, x1, x4, x0, x3, x2)]);
        sort_indices<2,4,5,0,1,3,0,1,1,1>(i1data, i1data_sorted, c2.size(), x1.size(), x4.size(), x0.size(), x3.size(), x2.size());
        zgemm3m_("T", "N", a1.size(), c2.size()*x1.size()*x0.size(), x4.size()*x3.size()*x2.size(),
...
void Task398::Task_local::compute() {
  ...
  const Index x2 = b(5);
  // tensor label: I665
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c2, x1, x4, x0, x3, x2);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(c2, x1, x4, x0, x3, x2)]);
  std::fill_n(odata_sorted.get(), out()->get_size(c2, x1, x4, x0, x3, x2), 0.0);
  for (auto& x7 : *range_[1]) {
    for (auto& x6 : *range_[1]) {
      for (auto& x5 : *range_[1]) {
        // tensor label: Gamma220
        std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x7, x6, x1, x5, x4, x0, x3, x2);
        std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(x7, x6, x1, x5, x4, x0, x3, x2)]);
        ...
        zgemm3m_("T", "N", x1.size()*x4.size()*x0.size()*x3.size()*x2.size(), c2.size(), x7.size()*x6.size()*x5.size(),
...
void Task399::Task_local::compute() {
  ...
  for (auto& x4 : *range_[1]) {
    for (auto& x3 : *range_[1]) {
      for (auto& x2 : *range_[1]) {
        // tensor label: v2
        std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x4, x3, x2, a1);
        std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(x4, x3, x2, a1)]);
        sort_indices<0,1,2,3,0,1,1,1>(i0data, i0data_sorted, x4.size(), x3.size(), x2.size(), a1.size());
        // tensor label: I668
        std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c2, x1, x4, x3, x2, x0);
        std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(c2, x1, x4, x3, x2, x0)]);
