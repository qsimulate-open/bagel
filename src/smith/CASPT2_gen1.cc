...
Task16::Task16(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range, const double e) {
  array<shared_ptr<const Tensor>,2> in = {{t[1], t[2]}};
  subtasks_.reserve(range[2]->nblock()*range[0]->nblock()*range[2]->nblock()*range[0]->nblock());
  for (auto& a2 : *range[2])
    for (auto& c3 : *range[0])
...
Task27::Task27(vector<shared_ptr<Tensor>> t) {
  d_ =  t[0];
}
...
Task40::Task40(vector<shared_ptr<Tensor>> t) {
  d2_ =  t[0];
}
