...
#include <src/smith/CAS_test_tasks2.h>
...
void Task73::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata = out()->move_block(x0, x5, x1, x4);
  {
    ...
    sort_indices<0,1,2,3,1,1,2,1>(i0data, odata, x0.size(), x5.size(), x1.size(), x4.size());
  }
...
void Task75::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata = out()->move_block(x0, x3, x1, x2);
  {
    ...
    sort_indices<0,1,2,3,1,1,-2,1>(i0data, odata, x0.size(), x3.size(), x1.size(), x2.size());
  }
...
void Task78::Task_local::compute() {
  ...
  {
    // tensor label: I2
    std::unique_ptr<double[]> i0data = in(0)->get_block(x1, x0, c2, c1);
    sort_indices<2,0,3,1,1,1,1,1>(i0data, odata, x1.size(), x0.size(), c2.size(), c1.size());
  }
  out()->put_block(odata, c2, x1, c1, x0);
}
...
void Task91::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata = out()->move_block(x2, x5, x3, x4, x1, x0);
  {
    ...
    sort_indices<0,1,2,3,4,5,1,1,2,1>(i0data, odata, x2.size(), x5.size(), x3.size(), x4.size(), x1.size(), x0.size());
  }
