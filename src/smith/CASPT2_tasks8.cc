...
void Task363::Task_local::compute() {
  ...
  for (auto& a2 : *range_[2]) {
    for (auto& c1 : *range_[0]) {
      ...
      sort_indices<3,0,1,2,0,1,1,1>(i1data, i1data_sorted, c1.size(), x3.size(), x2.size(), a2.size());
...
void Task364::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata = out()->move_block(c1, x3, x2, a2);
  {
    ...
    sort_indices<0,1,2,3,1,1,1,4>(i0data, odata, c1.size(), x3.size(), x2.size(), a2.size());
  }
...
void Task381::Task_local::compute() {
  ...
  std::unique_ptr<double[]> odata = out()->move_block(c2, a1);
  {
    ...
    sort_indices<0,1,1,1,-1,2>(i0data, odata, c2.size(), a1.size());
  }
