...
void Task151::Task_local::compute() {
  ...
  sort_indices<2,1,0,3,1,1,1,1>(odata_sorted, odata, c1.size(), a4.size(), c3.size(), a2.size());
...
void Task153::Task_local::compute() {
  ...
  sort_indices<2,3,0,1,1,1,1,1>(odata_sorted, odata, c1.size(), a2.size(), c3.size(), a4.size());
  out()->add_block(odata, c3, a4, c1, a2);
}
...
void Task155::Task_local::compute() {
  ...
  sort_indices<2,0,1,3,1,1,1,1>(odata_sorted, odata, a4.size(), c1.size(), c3.size(), a2.size());
  out()->add_block(odata, c3, a4, c1, a2);
}
...
void Task157::Task_local::compute() {
  ...
  {
    // tensor label: I160
    std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x0, c2, a3, a1);
...
void Task158::Task_local::compute() {
  ...
  const Index a1 = b(3);
  // tensor label: I160
  std::unique_ptr<std::complex<double>[]> odata(new std::complex<double>[out()->get_size(x0, c2, a3, a1)]);
  std::fill_n(odata.get(), out()->get_size(x0, c2, a3, a1), 0.0);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(x0, c2, a3, a1)]);
  ...
  out()->add_block(odata, x0, c2, a3, a1);
}
...
void Task168::Task_local::compute() {
  ...
  sort_indices<3,2,1,0,1,1,1,1>(odata_sorted, odata, a1.size(), a3.size(), c2.size(), x0.size());
  out()->add_block(odata, x0, c2, a3, a1);
}
...
void Task171::Task_local::compute() {
  ...
  sort_indices<3,2,0,1,1,1,1,1>(odata_sorted, odata, a3.size(), a1.size(), c2.size(), x0.size());
  out()->add_block(odata, x0, c2, a3, a1);
}
