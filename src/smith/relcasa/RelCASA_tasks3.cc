...
void Task109::Task_local::compute() {
  ...
  sort_indices<1,0,2,3,1,1,1,1>(odata_sorted, odata, c1.size(), a2.size(), x1.size(), x0.size());
  out()->add_block(odata, a2, c1, x1, x0);
}
...
void Task111::Task_local::compute() {
  ...
  zgemm3m_("T", "N", x1.size()*x0.size(), c1.size()*a2.size(), 1,
         ...
         1.0, odata_sorted, x1.size()*x0.size());
  sort_indices<3,2,0,1,1,1,1,1>(odata_sorted, odata, x1.size(), x0.size(), c1.size(), a2.size());
  out()->add_block(odata, a2, c1, x1, x0);
}
...
void Task117::Task_local::compute() {
  ...
  sort_indices<0,1,2,3,1,1,1,1>(odata_sorted, odata, a2.size(), c1.size(), x1.size(), x0.size());
  out()->add_block(odata, a2, c1, x1, x0);
}
...
void Task140::Task_local::compute() {
  ...
  {
    // tensor label: I132
    std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c3, a4, c1, a2);
...
void Task141::Task_local::compute() {
  ...
  std::unique_ptr<std::complex<double>[]> odata(new std::complex<double>[out()->get_size(c3, a4, c1, a2)]);
  std::fill_n(odata.get(), out()->get_size(c3, a4, c1, a2), 0.0);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(c3, a4, c1, a2)]);
...
void Task145::Task_local::compute() {
  ...
  sort_indices<0,2,3,1,1,1,1,1>(odata_sorted, odata, c3.size(), a2.size(), a4.size(), c1.size());
  out()->add_block(odata, c3, a4, c1, a2);
}
...
void Task148::Task_local::compute() {
  ...
  sort_indices<0,1,3,2,1,1,1,1>(odata_sorted, odata, c3.size(), a4.size(), a2.size(), c1.size());
  out()->add_block(odata, c3, a4, c1, a2);
}
