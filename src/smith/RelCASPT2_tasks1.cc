...
void Task35::Task_local::compute() {
  ...
  const Index x2 = b(3);
  // tensor label: I277
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c1, x3, c2, x2);
  {
    // tensor label: t2
    std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c1, x3, c2, x2);
    zscal_(c1.size()*x3.size()*c2.size()*x2.size(), e0_, i0data.get(), 1);
    sort_indices<0,1,2,3,1,1,-2,1>(i0data, odata, c1.size(), x3.size(), c2.size(), x2.size());
  }
  {
    // tensor label: v2
    std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c1, x3, c2, x2);
    sort_indices<0,1,2,3,1,1,1,1>(i1data, odata, c1.size(), x3.size(), c2.size(), x2.size());
  }
...
void Task48::Task_local::compute() {
  ...
  }
  {
    // tensor label: v2
    std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x5, x4, c1, x3);
    sort_indices<2,0,1,3,1,1,1,2>(i1data, odata, x5.size(), x4.size(), c1.size(), x3.size());
  }
