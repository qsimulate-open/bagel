...
#include <src/smith/casa/CASA_tasks3.h>
...
void Task123::Task_local::compute() {
  ...
  {
    // tensor label: I108
    std::unique_ptr<double[]> i0data = in(0)->get_block(a2, x1, x0, c1);
...
void Task124::Task_local::compute() {
  ...
  const Index c1 = b(3);
  // tensor label: I108
  std::unique_ptr<double[]> odata(new double[out()->get_size(a2, x1, x0, c1)]);
  std::fill_n(odata.get(), out()->get_size(a2, x1, x0, c1), 0.0);
  std::unique_ptr<double[]> odata_sorted(new double[out()->get_size(a2, x1, x0, c1)]);
