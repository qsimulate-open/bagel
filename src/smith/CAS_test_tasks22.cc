...
#include <src/smith/CAS_test_tasks22.h>
    ...
    for (auto& x0 : *range_[1]) {
      for (auto& c2 : *range_[0]) {
        for (auto& x1 : *range_[1]) {
          ...
          // tensor label: I1197
          std::unique_ptr<double[]> i1data = in(1)->get_block(ci0, x0, x1, c1, c2);
          std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(ci0, x0, x1, c1, c2)]);
          sort_indices<3,1,4,2,0,0,1,1,1>(i1data, i1data_sorted, ci0.size(), x0.size(), x1.size(), c1.size(), c2.size());
          dgemm_("T", "N", 1, ci0.size(), x0.size()*x1.size()*c1.size()*c2.size(),
...
void Task1097::Task_local::compute() {
  ...
  for (auto& x4 : *range_[1]) {
    for (auto& x5 : *range_[1]) {
      ...
      sort_indices<4,2,0,1,3,0,1,1,1>(i1data, i1data_sorted, ci0.size(), x0.size(), x5.size(), x1.size(), x4.size());
      dgemm_("T", "N", c1.size()*c2.size(), ci0.size()*x0.size()*x1.size(), x5.size()*x4.size(),
...
void Task1099::Task_local::compute() {
  ...
  for (auto& c3 : *range_[0]) {
    ...
    // tensor label: I1201
    std::unique_ptr<double[]> i1data = in(1)->get_block(ci0, x0, x1, c1, c3);
    std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(ci0, x0, x1, c1, c3)]);
    sort_indices<4,0,1,2,3,0,1,1,1>(i1data, i1data_sorted, ci0.size(), x0.size(), x1.size(), c1.size(), c3.size());
    dgemm_("T", "N", c2.size(), ci0.size()*x0.size()*x1.size()*c1.size(), c3.size(),
