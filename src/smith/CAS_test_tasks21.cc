  ...
  for (auto& x0 : *range_[1]) {
    for (auto& c2 : *range_[0]) {
      for (auto& a3 : *range_[2]) {
        ...
        sort_indices<1,3,2,0,0,1,1,1>(i1data, i1data_sorted, x1.size(), x0.size(), a3.size(), c2.size());
        dgemm_("T", "N", a1.size(), x1.size(), x0.size()*a3.size()*c2.size(),
               1.0, i0data_sorted, x0.size()*a3.size()*c2.size(), i1data_sorted, x0.size()*a3.size()*c2.size(),
  ...
  for (auto& x0 : *range_[1]) {
    for (auto& a1 : *range_[2]) {
      for (auto& c2 : *range_[0]) {
        ...
        sort_indices<0,2,3,1,0,1,1,1>(i1data, i1data_sorted, x0.size(), x1.size(), a1.size(), c2.size());
        dgemm_("T", "N", a3.size(), x1.size(), x0.size()*a1.size()*c2.size(),
               1.0, i0data_sorted, x0.size()*a1.size()*c2.size(), i1data_sorted, x0.size()*a1.size()*c2.size(),
...
void Task1012::Task_local::compute() {
  ...
  for (auto& x0 : *range_[1]) {
    for (auto& a3 : *range_[2]) {
      ...
      std::unique_ptr<double[]> i0data = in(0)->get_block(x0, a1, c2, a3);
      std::unique_ptr<double[]> i0data_sorted(new double[in(0)->get_size(x0, a1, c2, a3)]);
      sort_indices<0,3,1,2,0,1,1,1>(i0data, i0data_sorted, x0.size(), a1.size(), c2.size(), a3.size());
      // tensor label: I1112
      ...
      dgemm_("T", "N", c2.size()*a1.size(), 1, x0.size()*a3.size(),
             1.0, i0data_sorted, x0.size()*a3.size(), i1data_sorted, x0.size()*a3.size(),
             1.0, odata_sorted, c2.size()*a1.size());
    }
  }
...
void Task1016::Task_local::compute() {
  ...
  for (auto& x0 : *range_[1]) {
    for (auto& a1 : *range_[2]) {
      ...
      dgemm_("T", "N", a3.size()*c2.size(), 1, x0.size()*a1.size(),
             1.0, i0data_sorted, x0.size()*a1.size(), i1data_sorted, x0.size()*a1.size(),
             1.0, odata_sorted, a3.size()*c2.size());
    }
  }
  sort_indices<0,1,1,1,1,1>(odata_sorted, odata, a3.size(), c2.size());
  out()->put_block(odata, a3, c2);
}
