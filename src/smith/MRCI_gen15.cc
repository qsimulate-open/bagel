...
Task705::Task705(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  array<shared_ptr<const Tensor>,1> in = {{t[1]}};
  subtasks_.reserve(range[0]->nblock()*range[2]->nblock()*range[0]->nblock()*range[0]->nblock());
  for (auto& c3 : *range[0])
    for (auto& a4 : *range[2])
      for (auto& c5 : *range[0])
        for (auto& c1 : *range[0])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c1, c5, a4, c3}}, in, t[0], range));
}
...
Task709::Task709(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  array<shared_ptr<const Tensor>,1> in = {{t[1]}};
  subtasks_.reserve(range[0]->nblock()*range[2]->nblock()*range[2]->nblock()*range[2]->nblock());
  for (auto& c3 : *range[0])
    for (auto& a4 : *range[2])
      for (auto& a2 : *range[2])
        for (auto& a5 : *range[2])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{a5, a2, a4, c3}}, in, t[0], range));
}
...
Task714::Task714(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x4 : *range[1])
    for (auto& c3 : *range[0])
      for (auto& x5 : *range[1])
        for (auto& c1 : *range[0])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c1, x5, c3, x4}}, in, t[0], range));
}
    ...
    for (auto& x1 : *range[1])
      for (auto& x2 : *range[1])
        for (auto& x3 : *range[1])
          for (auto& x4 : *range[1])
            for (auto& c1 : *range[0])
              subtasks_.push_back(make_shared<Task_local>(array<const Index,6>{{c1, x4, x3, x2, x1, x0}}, in, t[0], range));
}
    ...
    for (auto& x1 : *range[1])
      for (auto& x3 : *range[1])
        for (auto& x4 : *range[1])
          for (auto& x2 : *range[1])
            for (auto& c1 : *range[0])
              subtasks_.push_back(make_shared<Task_local>(array<const Index,6>{{c1, x2, x4, x3, x1, x0}}, in, t[0], range));
}
    ...
    for (auto& c1 : *range[0])
      for (auto& x5 : *range[1])
        for (auto& x2 : *range[1])
          for (auto& x3 : *range[1])
            for (auto& a2 : *range[2])
              subtasks_.push_back(make_shared<Task_local>(array<const Index,6>{{a2, x3, x2, x5, c1, x4}}, in, t[0], range));
}
