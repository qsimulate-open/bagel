...
Task254::Task254(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& a2 : *range[2])
    for (auto& a4 : *range[2])
      subtasks_.push_back(make_shared<Task_local>(array<const Index,2>{{a4, a2}}, in, t[0], range));
}
...
Task260::Task260(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c4 : *range[0])
    for (auto& c3 : *range[0])
      for (auto& c5 : *range[0])
        for (auto& c1 : *range[0])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c1, c5, c3, c4}}, in, t[0], range));
}
...
Task264::Task264(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& a2 : *range[2])
    for (auto& a4 : *range[2])
      for (auto& c5 : *range[0])
        for (auto& c1 : *range[0])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c1, c5, a4, a2}}, in, t[0], range));
}
...
Task266::Task266(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c4 : *range[0])
    for (auto& a5 : *range[2])
      for (auto& a2 : *range[2])
        for (auto& c1 : *range[0])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c1, a2, a5, c4}}, in, t[0], range));
}
...
Task272::Task272(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& a2 : *range[2])
    for (auto& a4 : *range[2])
      for (auto& c5 : *range[0])
        for (auto& c3 : *range[0])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c3, c5, a4, a2}}, in, t[0], range));
}
...
Task274::Task274(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c4 : *range[0])
    for (auto& a5 : *range[2])
      for (auto& a2 : *range[2])
        for (auto& c3 : *range[0])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c3, a2, a5, c4}}, in, t[0], range));
}
...
Task288::Task288(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& a2 : *range[2])
    for (auto& a4 : *range[2])
      for (auto& x1 : *range[1])
        for (auto& a5 : *range[2])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{a5, x1, a4, a2}}, in, t[0], range));
}
