...
void Task160::Task_local::compute() {
  ...
  for (auto& c5 : *range_[0]) {
    ...
    sort_indices<1,0,0,1,2,1>(i1data, i1data_sorted, c3.size(), c5.size());
...
void Task162::Task_local::compute() {
  ...
  for (auto& a5 : *range_[2]) {
    ...
    sort_indices<0,1,0,1,-2,1>(i1data, i1data_sorted, a5.size(), a4.size());
...
void Task169::Task_local::compute() {
  ...
  {
    // tensor label: v2
    std::unique_ptr<std::complex<double>[]> i2data = in(1)->get_block(c1, a4, c3, a2);
    sort_indices<0,1,2,3,1,1,-1,1>(i2data, odata, c1.size(), a4.size(), c3.size(), a2.size());
  }
  {
    // tensor label: v2
    std::unique_ptr<std::complex<double>[]> i3data = in(1)->get_block(c1, a2, c3, a4);
