...
Task452::Task452(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x1 : *range[1])
    for (auto& a4 : *range[2])
      for (auto& x2 : *range[1])
        for (auto& c1 : *range[0])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c1, x2, a4, x1}}, in, t[0], range));
}
...
Task455::Task455(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x1 : *range[1])
    for (auto& a4 : *range[2])
      for (auto& x2 : *range[1])
        for (auto& c3 : *range[0])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c3, x2, a4, x1}}, in, t[0], range));
}
...
Task471::Task471(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c3 : *range[0])
    for (auto& c2 : *range[0])
      for (auto& c4 : *range[0])
        for (auto& x2 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x2, c4, c2, c3}}, in, t[0], range));
}
...
Task473::Task473(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& a1 : *range[2])
    for (auto& a3 : *range[2])
      for (auto& c4 : *range[0])
        for (auto& x2 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x2, c4, a3, a1}}, in, t[0], range));
}
...
Task475::Task475(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c3 : *range[0])
    for (auto& a4 : *range[2])
      for (auto& a1 : *range[2])
        for (auto& x2 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x2, a1, a4, c3}}, in, t[0], range));
}
...
Task479::Task479(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c3 : *range[0])
    for (auto& x2 : *range[1])
      for (auto& x3 : *range[1])
        for (auto& a4 : *range[2])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{a4, x3, x2, c3}}, in, t[0], range));
}
