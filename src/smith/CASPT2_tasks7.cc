...
void Task330::Task_local::compute() {
  ...
  }
  {
    // tensor label: v2
    std::unique_ptr<double[]> i1data = in(1)->get_block(x3, a1, x2, a2);
    sort_indices<0,1,2,3,1,1,2,1>(i1data, odata, x3.size(), a1.size(), x2.size(), a2.size());
  }
...
void Task331::Task_local::compute() {
  ...
  sort_indices<0,1,2,3,0,1,1,1>(i0data, i0data_sorted, x0.size(), x5.size(), x1.size(), x4.size());
  // tensor label: I349
...
void Task334::Task_local::compute() {
  ...
  sort_indices<0,1,0,1,1,1>(i1data, i1data_sorted, c2.size(), c3.size());
  target_ += ddot_(c2.size()*c3.size(), i0data_sorted, 1, i1data_sorted, 1);
}
...
void Task344::Task_local::compute() {
  ...
  target_ += ddot_(x2.size()*c2.size(), i0data_sorted, 1, i1data_sorted, 1);
}
