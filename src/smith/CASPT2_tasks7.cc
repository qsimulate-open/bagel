...
void Task323::Task_local::compute() {
  ...
  target_ += ddot_(x5.size()*x4.size()*x3.size()*c1.size(), i0data_sorted, 1, i1data_sorted, 1);
}
...
void Task324::Task_local::compute() {
  ...
  const Index c1 = b(3);
  // tensor label: I338
  std::unique_ptr<double[]> odata = out()->move_block(x5, x4, x3, c1);
  ...
  for (auto& x2 : *range_[1]) {
    for (auto& x1 : *range_[1]) {
      for (auto& x0 : *range_[1]) {
        ...
        sort_indices<0,4,5,1,2,3,0,1,1,1>(i1data, i1data_sorted, x2.size(), x5.size(), x4.size(), x3.size(), x1.size(), x0.size());
        dgemm_("T", "N", c1.size(), x5.size()*x4.size()*x3.size(), x2.size()*x1.size()*x0.size(),
  ...
  out()->put_block(odata, x5, x4, x3, c1);
}
  ...
  for (auto& c3 : *range_[0]) {
    for (auto& a2 : *range_[2]) {
      for (auto& c1 : *range_[0]) {
        ...
        sort_indices<0,3,2,1,0,1,1,1>(i1data, i1data_sorted, c3.size(), x1.size(), c1.size(), a2.size());
        dgemm_("T", "N", x0.size(), x1.size(), c3.size()*c1.size()*a2.size(),
               1.0, i0data_sorted, c3.size()*c1.size()*a2.size(), i1data_sorted, c3.size()*c1.size()*a2.size(),
...
void Task331::Task_local::compute() {
  ...
  const Index a2 = b(3);
  // tensor label: I345
  std::unique_ptr<double[]> odata = out()->move_block(c3, x1, c1, a2);
  {
    // tensor label: v2
...
void Task342::Task_local::compute() {
  ...
  }
  {
    ...
    sort_indices<0,3,2,1,1,1,1,1>(i1data, odata, x3.size(), x2.size(), c1.size(), a2.size());
  }
