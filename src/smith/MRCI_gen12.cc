    ...
    for (auto& x3 : *range[1])
      for (auto& x0 : *range[1])
        for (auto& x4 : *range[1])
          for (auto& x1 : *range[1])
            for (auto& c2 : *range[0])
              subtasks_.push_back(make_shared<Task_local>(array<const Index,6>{{c2, x1, x4, x0, x3, x2}}, in, t[0], range));
}
...
Task551::Task551(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x5 : *range[1])
    for (auto& c2 : *range[0])
      for (auto& x6 : *range[1])
        for (auto& x7 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x7, x6, c2, x5}}, in, t[0], range));
}
...
Task583::Task583(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& a1 : *range[2])
    for (auto& x2 : *range[1])
      for (auto& x3 : *range[1])
        for (auto& a3 : *range[2])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{a3, x3, x2, a1}}, in, t[0], range));
}
...
Task586::Task586(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x2 : *range[1])
    for (auto& a3 : *range[2])
      for (auto& a1 : *range[2])
        for (auto& x3 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x3, a1, a3, x2}}, in, t[0], range));
}
