...
Task631::Task631(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x6 : *range[1])
    for (auto& c2 : *range[0])
      for (auto& a1 : *range[2])
        for (auto& x7 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x7, a1, c2, x6}}, in, t[0], range));
}
...
Task633::Task633(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x6 : *range[1])
    for (auto& x7 : *range[1])
      for (auto& a1 : *range[2])
        for (auto& c2 : *range[0])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c2, a1, x7, x6}}, in, t[0], range));
}
...
Task644::Task644(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c3 : *range[0])
    for (auto& c1 : *range[0])
      for (auto& c4 : *range[0])
        for (auto& x2 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x2, c4, c1, c3}}, in, t[0], range));
}
