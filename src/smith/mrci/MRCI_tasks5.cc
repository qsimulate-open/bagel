...
void Task217::Task_local::compute() {
  ...
  for (auto& a5 : *range_[2]) {
    for (auto& c4 : *range_[0]) {
      ...
      sort_indices<0,3,1,2,0,1,-4,1>(i1data, i1data_sorted, a5.size(), x1.size(), c1.size(), c4.size());
...
void Task220::Task_local::compute() {
  ...
  for (auto& a5 : *range_[2]) {
    for (auto& c4 : *range_[0]) {
      // tensor label: v2
      std::unique_ptr<double[]> i0data = in(0)->get_block(c1, x1, a5, c4);
      std::unique_ptr<double[]> i0data_sorted(new double[in(0)->get_size(c1, x1, a5, c4)]);
      sort_indices<2,3,0,1,0,1,1,1>(i0data, i0data_sorted, c1.size(), x1.size(), a5.size(), c4.size());
      // tensor label: t2
      std::unique_ptr<double[]> i1data = in(1)->get_block(c3, a5, c4, a2);
      std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(c3, a5, c4, a2)]);
      sort_indices<1,2,0,3,0,1,2,1>(i1data, i1data_sorted, c3.size(), a5.size(), c4.size(), a2.size());
      dgemm_("T", "N", c1.size()*x1.size(), c3.size()*a2.size(), a5.size()*c4.size(),
             1.0, i0data_sorted, a5.size()*c4.size(), i1data_sorted, a5.size()*c4.size(),
             1.0, odata_sorted, c1.size()*x1.size());
    }
