...
void Task287::Task_local::compute() {
  ...
  for (auto& x2 : *range_[1]) {
    for (auto& x1 : *range_[1]) {
      ...
      sort_indices<2,3,0,1,0,1,-1,2>(i1data, i1data_sorted, a4.size(), a2.size(), x2.size(), x1.size());
...
void Task290::Task_local::compute() {
  ...
  for (auto& x2 : *range_[1]) {
    for (auto& x1 : *range_[1]) {
      // tensor label: v2
      std::unique_ptr<double[]> i0data = in(0)->get_block(x2, x1, a4, a2);
      std::unique_ptr<double[]> i0data_sorted(new double[in(0)->get_size(x2, x1, a4, a2)]);
      sort_indices<0,1,2,3,0,1,1,1>(i0data, i0data_sorted, x2.size(), x1.size(), a4.size(), a2.size());
      // tensor label: Gamma197
      ...
      sort_indices<2,3,0,1,0,1,-1,2>(i1data, i1data_sorted, x0.size(), x3.size(), x2.size(), x1.size());
      dgemm_("T", "N", a4.size()*a2.size(), x0.size()*x3.size(), x2.size()*x1.size(),
             1.0, i0data_sorted, x2.size()*x1.size(), i1data_sorted, x2.size()*x1.size(),
             1.0, odata_sorted, a4.size()*a2.size());
    }
  }
  sort_indices<0,1,2,3,1,1,1,1>(odata_sorted, odata, a4.size(), a2.size(), x0.size(), x3.size());
  out()->put_block(odata, a4, a2, x0, x3);
}
