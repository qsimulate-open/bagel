...
void Task251::Task_local::compute() {
  ...
  const Index x0 = b(3);
  // tensor label: I402
  std::unique_ptr<double[]> odata(new double[out()->get_size(c1, c4, x1, x0)]);
...
void Task253::Task_local::compute() {
  ...
  const Index x1 = b(3);
  // tensor label: I405
  std::unique_ptr<double[]> odata(new double[out()->get_size(c3, x0, x2, x1)]);
...
void Task255::Task_local::compute() {
  ...
  const Index x1 = b(3);
  // tensor label: I408
  std::unique_ptr<double[]> odata(new double[out()->get_size(c1, x0, x2, x1)]);
...
void Task257::Task_local::compute() {
  ...
  const Index x2 = b(3);
  // tensor label: I411
  std::unique_ptr<double[]> odata(new double[out()->get_size(c3, x0, x1, x2)]);
  ...
  for (auto& x3 : *range_[1]) {
    for (auto& x2 : *range_[1]) {
      for (auto& x1 : *range_[1]) {
        ...
        std::unique_ptr<double[]> i1data = in(1)->get_block(x3, x2, c4, x1);
        std::unique_ptr<double[]> i1data_sorted(new double[in(1)->get_size(x3, x2, c4, x1)]);
        sort_indices<0,1,3,2,0,1,-2,1>(i1data, i1data_sorted, x3.size(), x2.size(), c4.size(), x1.size());
        dgemm_("T", "N", x0.size(), c4.size(), x3.size()*x2.size()*x1.size(),
               1.0, i0data_sorted, x3.size()*x2.size()*x1.size(), i1data_sorted, x3.size()*x2.size()*x1.size(),
               1.0, odata_sorted, x0.size());
      }
...
void Task273::Task_local::compute() {
  ...
  const Index x3 = b(3);
  // tensor label: I435
  std::unique_ptr<double[]> odata(new double[out()->get_size(c1, c4, x0, x3)]);
...
void Task274::Task_local::compute() {
  ...
  const Index x1 = b(3);
  // tensor label: I436
  std::unique_ptr<double[]> odata(new double[out()->get_size(c1, c4, x2, x1)]);
...
void Task282::Task_local::compute() {
  ...
  const Index x3 = b(3);
  // tensor label: I441
  std::unique_ptr<double[]> odata(new double[out()->get_size(c3, c4, x0, x3)]);
...
void Task283::Task_local::compute() {
  ...
  const Index x1 = b(3);
  // tensor label: I442
  std::unique_ptr<double[]> odata(new double[out()->get_size(c3, c4, x2, x1)]);
...
void Task287::Task_local::compute() {
  ...
  const Index x3 = b(3);
  // tensor label: I444
  std::unique_ptr<double[]> odata(new double[out()->get_size(a4, a2, x0, x3)]);
...
void Task288::Task_local::compute() {
  ...
  const Index x1 = b(3);
  // tensor label: I445
  std::unique_ptr<double[]> odata(new double[out()->get_size(a4, a2, x2, x1)]);
