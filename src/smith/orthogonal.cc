...
vector<double> f, vector<double> e0, shared_ptr<const Matrix> fact, shared_ptr<const Denom<double>> d, const bool residual) : closed_(c), active_(a), virt_(v), eig_(f), e0all_(e0) {
  ...
  if (sssr_) {
    ...
    for (int iext = Excitations::arbs; iext != Excitations::aibj; ++iext) {
      size_t ndim = 0;
      for (int istate = 0; istate != nstates_; ++istate) {
        ndim += d->shalf(to_denom_.at(iext), istate).ndim();
      }
      const int maxtile = max(((int)(ndim / 255) + 1), info->maxtile());
      interm_.push_back(IndexRange(ndim, maxtile, keyoffset));
      keyoffset += interm_[iext].nblock();
...
void Orthogonal_Basis::transform_to_orthogonal(shared_ptr<const MultiTensor_<double>> t, int istate) {
  if (sssr_) {
    transform_to_orthogonal_sssr(t, istate);
  } else {
    transform_to_orthogonal_msmr(t, istate);
  }
}


void Orthogonal_Basis::transform_to_orthogonal_sssr(shared_ptr<const MultiTensor_<double>> t, int istate) {
...
void Orthogonal_Basis::transform_to_orthogonal_msmr(shared_ptr<const MultiTensor_<double>> t, int istate) {
}


shared_ptr<MultiTensor_<double>> Orthogonal_Basis::transform_to_redundant(const int istate) const {
  shared_ptr<MultiTensor_<double>> out;
  if (sssr_) {
    out = transform_to_redundant_sssr(istate);
  } else {
    out = transform_to_redundant_msmr(istate);
  }
  return out;
}


shared_ptr<MultiTensor_<double>> Orthogonal_Basis::transform_to_redundant_sssr(const int istate) const {
  // we put the transformed data in out.
  auto out = make_shared<MultiTensor_<double>>(nstates_);
  (*out)[istate] = init_amplitude();
        ...
        for (auto& i3 : virt_)
          for (auto& i2 : closed_)
            for (auto& i1 : virt_)
              for (auto& i0 : closed_) {
                if (!tensor->is_local(i0, i1, i2, i3)) continue;
                unique_ptr<double[]> data0 = data_[istate]->at(iext)->get_block(i0, i1, i2, i3);
                if (imag_) {
                  const unique_ptr<double[]> denom = denom_[istate]->at(iext)->get_block(i0,i1, i2, i3);
                  const size_t blocksize = data_[istate]->at(iext)->get_size(i0, i1, i2, i3);
                  for (size_t i = 0; i != blocksize; ++i) {
                    data0[i] *= denom[i];
                  }
                }
                out->at(istate)->add_block(data0, i0, i1, i2, i3);
              }
        break;
    }
  }
...
shared_ptr<MultiTensor_<double>> Orthogonal_Basis::transform_to_redundant_msmr(const int istate) const {
  return shared_ptr<MultiTensor_<double>>();
}
