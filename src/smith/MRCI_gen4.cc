...
Task159::Task159(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c2 : *range[0])
    for (auto& c1 : *range[0])
      for (auto& c3 : *range[0])
        for (auto& x3 : *range[1])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{x3, c3, c1, c2}}, in, t[0], range));
}
...
Task168::Task168(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c2 : *range[0])
    for (auto& a3 : *range[2])
      for (auto& x3 : *range[1])
        for (auto& c1 : *range[0])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c1, x3, a3, c2}}, in, t[0], range));
}
...
Task170::Task170(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c2 : *range[0])
    for (auto& c1 : *range[0])
      for (auto& x3 : *range[1])
        for (auto& a3 : *range[2])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{a3, x3, c1, c2}}, in, t[0], range));
}
...
Task177::Task177(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& c2 : *range[0])
    for (auto& a3 : *range[2])
      for (auto& c4 : *range[0])
        for (auto& c1 : *range[0])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c1, c4, a3, c2}}, in, t[0], range));
}
...
Task181::Task181(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  array<shared_ptr<const Tensor>,1> in = {{t[1]}};
  subtasks_.reserve(range[0]->nblock()*range[2]->nblock()*range[1]->nblock()*range[2]->nblock());
  for (auto& c2 : *range[0])
    for (auto& a3 : *range[2])
      for (auto& x3 : *range[1])
        for (auto& a4 : *range[2])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{a4, x3, a3, c2}}, in, t[0], range));
}
...
Task190::Task190(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x3 : *range[1])
    for (auto& a2 : *range[2])
      for (auto& x4 : *range[1])
        for (auto& a3 : *range[2])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{a3, x4, a2, x3}}, in, t[0], range));
}
