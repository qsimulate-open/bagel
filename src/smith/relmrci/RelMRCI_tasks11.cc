...
void Task501::Task_local::compute() {
  ...
  const Index a2 = b(1);
  // tensor label: I852
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c5, a2);
...
void Task503::Task_local::compute() {
  ...
  const Index a5 = b(1);
  // tensor label: I855
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c1, a5);
...
void Task514::Task_local::compute() {
  ...
  for (auto& a6 : *range_[2]) {
    for (auto& c5 : *range_[0]) {
      // tensor label: v2
      std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c3, a2, a6, c5);
      std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(c3, a2, a6, c5)]);
      sort_indices<2,3,0,1,0,1,1,1>(i0data, i0data_sorted, c3.size(), a2.size(), a6.size(), c5.size());
      // tensor label: t2
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c1, a6, c5, a4);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(c1, a6, c5, a4)]);
      sort_indices<1,2,0,3,0,1,2,1>(i1data, i1data_sorted, c1.size(), a6.size(), c5.size(), a4.size());
      zgemm3m_("T", "N", c3.size()*a2.size(), c1.size()*a4.size(), a6.size()*c5.size(),
...
void Task542::Task_local::compute() {
  ...
  const Index x0 = b(3);
  // tensor label: I135
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c2, a3, x1, x0);
  ...
  out()->put_block(odata, c2, a3, x1, x0);
}
