...
void Task351::Task_local::compute() {
  ...
  for (auto& x4 : *range_[1]) {
    for (auto& x3 : *range_[1]) {
      for (auto& x2 : *range_[1]) {
        // tensor label: v2
        std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x4, x3, x2, c3);
        std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(x4, x3, x2, c3)]);
        sort_indices<0,1,2,3,0,1,1,1>(i0data, i0data_sorted, x4.size(), x3.size(), x2.size(), c3.size());
        ...
        sort_indices<2,3,0,1,4,5,0,1,1,2>(i1data, i1data_sorted, x2.size(), x5.size(), x4.size(), x3.size(), x1.size(), x0.size());
        zgemm3m_("T", "N", c3.size(), x5.size()*x1.size()*x0.size(), x2.size()*x4.size()*x3.size(),
               1.0, i0data_sorted, x2.size()*x4.size()*x3.size(), i1data_sorted, x2.size()*x4.size()*x3.size(),
               1.0, odata_sorted, c3.size());
      }
    }
  }
  sort_indices<0,1,2,3,1,1,1,1>(odata_sorted, odata, c3.size(), x5.size(), x1.size(), x0.size());
  out()->put_block(odata, c3, x5, x1, x0);
}
...
void Task356::Task_local::compute() {
  ...
  for (auto& x3 : *range_[1]) {
    for (auto& x2 : *range_[1]) {
      // tensor label: v2
      std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x3, x2, c1, c3);
      std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(x3, x2, c1, c3)]);
      sort_indices<0,1,2,3,0,1,1,1>(i0data, i0data_sorted, x3.size(), x2.size(), c1.size(), c3.size());
      // tensor label: Gamma193
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x5, x4, x3, x2, x1, x0);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x5, x4, x3, x2, x1, x0)]);
      sort_indices<2,3,0,1,4,5,0,1,-1,2>(i1data, i1data_sorted, x5.size(), x4.size(), x3.size(), x2.size(), x1.size(), x0.size());
      zgemm3m_("T", "N", c1.size()*c3.size(), x5.size()*x4.size()*x1.size()*x0.size(), x3.size()*x2.size(),
             1.0, i0data_sorted, x3.size()*x2.size(), i1data_sorted, x3.size()*x2.size(),
             1.0, odata_sorted, c1.size()*c3.size());
    }
  }
  sort_indices<0,1,2,3,4,5,1,1,1,1>(odata_sorted, odata, c1.size(), c3.size(), x5.size(), x4.size(), x1.size(), x0.size());
  out()->put_block(odata, c1, c3, x5, x4, x1, x0);
}
  ...
  for (auto& a3 : *range_[2]) {
    for (auto& x3 : *range_[1]) {
      for (auto& x2 : *range_[1]) {
        ...
        std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(a3, a2, x3, x2)]);
        sort_indices<0,2,3,1,0,1,1,1>(i0data, i0data_sorted, a3.size(), a2.size(), x3.size(), x2.size());
        // tensor label: I591
        std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c1, a3, x3, x2, x1, x0);
        std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(c1, a3, x3, x2, x1, x0)]);
        sort_indices<1,2,3,0,4,5,0,1,1,1>(i1data, i1data_sorted, c1.size(), a3.size(), x3.size(), x2.size(), x1.size(), x0.size());
        zgemm3m_("T", "N", a2.size(), c1.size()*x1.size()*x0.size(), a3.size()*x3.size()*x2.size(),
...
void Task358::Task_local::compute() {
  ...
  const Index x0 = b(5);
  // tensor label: I591
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c1, a3, x3, x2, x1, x0);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(c1, a3, x3, x2, x1, x0)]);
  std::fill_n(odata_sorted.get(), out()->get_size(c1, a3, x3, x2, x1, x0), 0.0);
  for (auto& x5 : *range_[1]) {
    for (auto& x4 : *range_[1]) {
      // tensor label: Gamma193
      std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x5, x4, x3, x2, x1, x0);
      ...
      // tensor label: t2
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c1, a3, x5, x4);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(c1, a3, x5, x4)]);
      sort_indices<2,3,0,1,0,1,1,2>(i1data, i1data_sorted, c1.size(), a3.size(), x5.size(), x4.size());
      zgemm3m_("T", "N", x3.size()*x2.size()*x1.size()*x0.size(), c1.size()*a3.size(), x5.size()*x4.size(),
             1.0, i0data_sorted, x5.size()*x4.size(), i1data_sorted, x5.size()*x4.size(),
             1.0, odata_sorted, x3.size()*x2.size()*x1.size()*x0.size());
    }
  }
  sort_indices<4,5,0,1,2,3,1,1,1,1>(odata_sorted, odata, x3.size(), x2.size(), x1.size(), x0.size(), c1.size(), a3.size());
  out()->put_block(odata, c1, a3, x3, x2, x1, x0);
}
...
void Task361::Task_local::compute() {
  ...
  for (auto& x3 : *range_[1]) {
    for (auto& a3 : *range_[2]) {
      for (auto& x2 : *range_[1]) {
        // tensor label: v2
        std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x3, a2, a3, x2);
        std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(x3, a2, a3, x2)]);
        sort_indices<0,2,3,1,0,1,1,1>(i0data, i0data_sorted, x3.size(), a2.size(), a3.size(), x2.size());
        // tensor label: I603
        std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c1, a3, x3, x2, x1, x0);
        std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(c1, a3, x3, x2, x1, x0)]);
        sort_indices<2,1,3,0,4,5,0,1,1,1>(i1data, i1data_sorted, c1.size(), a3.size(), x3.size(), x2.size(), x1.size(), x0.size());
        zgemm3m_("T", "N", a2.size(), c1.size()*x1.size()*x0.size(), a3.size()*x3.size()*x2.size(),
               1.0, i0data_sorted, a3.size()*x3.size()*x2.size(), i1data_sorted, a3.size()*x3.size()*x2.size(),
               1.0, odata_sorted, a2.size());
      }
...
void Task363::Task_local::compute() {
  ...
  for (auto& x3 : *range_[1]) {
    for (auto& x2 : *range_[1]) {
      for (auto& a3 : *range_[2]) {
        // tensor label: v2
        std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x3, x2, a3, a2);
        std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(x3, x2, a3, a2)]);
        sort_indices<0,1,2,3,0,1,1,1>(i0data, i0data_sorted, x3.size(), x2.size(), a3.size(), a2.size());
        // tensor label: I609
        std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x3, x2, x1, x0, c1, a3);
        std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x3, x2, x1, x0, c1, a3)]);
        sort_indices<0,1,5,2,3,4,0,1,1,1>(i1data, i1data_sorted, x3.size(), x2.size(), x1.size(), x0.size(), c1.size(), a3.size());
        zgemm3m_("T", "N", a2.size(), x1.size()*x0.size()*c1.size(), x3.size()*x2.size()*a3.size(),
               1.0, i0data_sorted, x3.size()*x2.size()*a3.size(), i1data_sorted, x3.size()*x2.size()*a3.size(),
               1.0, odata_sorted, a2.size());
      }
    }
  }
  sort_indices<3,1,2,0,1,1,1,1>(odata_sorted, odata, a2.size(), x1.size(), x0.size(), c1.size());
  out()->put_block(odata, c1, x1, x0, a2);
}
...
void Task364::Task_local::compute() {
  ...
  const Index a3 = b(5);
  // tensor label: I609
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x3, x2, x1, x0, c1, a3);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(x3, x2, x1, x0, c1, a3)]);
  std::fill_n(odata_sorted.get(), out()->get_size(x3, x2, x1, x0, c1, a3), 0.0);
  for (auto& x5 : *range_[1]) {
    for (auto& x4 : *range_[1]) {
      // tensor label: t2
      std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c1, a3, x5, x4);
      std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(c1, a3, x5, x4)]);
      sort_indices<2,3,0,1,0,1,1,1>(i0data, i0data_sorted, c1.size(), a3.size(), x5.size(), x4.size());
      // tensor label: Gamma193
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x5, x4, x3, x2, x1, x0);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x5, x4, x3, x2, x1, x0)]);
      sort_indices<0,1,2,3,4,5,0,1,1,2>(i1data, i1data_sorted, x5.size(), x4.size(), x3.size(), x2.size(), x1.size(), x0.size());
      zgemm3m_("T", "N", c1.size()*a3.size(), x3.size()*x2.size()*x1.size()*x0.size(), x5.size()*x4.size(),
             1.0, i0data_sorted, x5.size()*x4.size(), i1data_sorted, x5.size()*x4.size(),
             1.0, odata_sorted, c1.size()*a3.size());
    }
  }
  sort_indices<2,3,4,5,0,1,1,1,1,1>(odata_sorted, odata, c1.size(), a3.size(), x3.size(), x2.size(), x1.size(), x0.size());
  out()->put_block(odata, x3, x2, x1, x0, c1, a3);
}
...
void Task365::Task_local::compute() {
  ...
  for (auto& a4 : *range_[2]) {
    for (auto& c3 : *range_[0]) {
      // tensor label: v2
      std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c1, a2, a4, c3);
      std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(c1, a2, a4, c3)]);
      sort_indices<2,3,0,1,0,1,1,1>(i0data, i0data_sorted, c1.size(), a2.size(), a4.size(), c3.size());
      // tensor label: I615
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x1, x0, c3, a4);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x1, x0, c3, a4)]);
      sort_indices<3,2,0,1,0,1,1,1>(i1data, i1data_sorted, x1.size(), x0.size(), c3.size(), a4.size());
      zgemm3m_("T", "N", c1.size()*a2.size(), x1.size()*x0.size(), c3.size()*a4.size(),
...
void Task366::Task_local::compute() {
  ...
  const Index a4 = b(3);
  // tensor label: I615
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x1, x0, c3, a4);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(x1, x0, c3, a4)]);
  std::fill_n(odata_sorted.get(), out()->get_size(x1, x0, c3, a4), 0.0);
  for (auto& x3 : *range_[1]) {
    for (auto& x2 : *range_[1]) {
      // tensor label: t2
      ...
      sort_indices<2,3,0,1,0,1,1,1>(i0data, i0data_sorted, c3.size(), a4.size(), x3.size(), x2.size());
      ...
      zgemm3m_("T", "N", c3.size()*a4.size(), x1.size()*x0.size(), x3.size()*x2.size(),
             1.0, i0data_sorted, x3.size()*x2.size(), i1data_sorted, x3.size()*x2.size(),
             1.0, odata_sorted, c3.size()*a4.size());
    }
  }
  sort_indices<2,3,0,1,1,1,1,1>(odata_sorted, odata, c3.size(), a4.size(), x1.size(), x0.size());
  out()->put_block(odata, x1, x0, c3, a4);
}
...
void Task374::Task_local::compute() {
  ...
  for (auto& a4 : *range_[2]) {
    for (auto& c3 : *range_[0]) {
      ...
      // tensor label: I648
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x1, x0, a4, c3);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x1, x0, a4, c3)]);
      sort_indices<2,3,0,1,0,1,1,1>(i1data, i1data_sorted, x1.size(), x0.size(), a4.size(), c3.size());
      zgemm3m_("T", "N", c1.size()*a2.size(), x1.size()*x0.size(), a4.size()*c3.size(),
...
void Task375::Task_local::compute() {
  ...
  const Index c3 = b(3);
  // tensor label: I648
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x1, x0, a4, c3);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(x1, x0, a4, c3)]);
  std::fill_n(odata_sorted.get(), out()->get_size(x1, x0, a4, c3), 0.0);
  for (auto& x3 : *range_[1]) {
    for (auto& x2 : *range_[1]) {
      // tensor label: v2
      std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x3, x2, a4, c3);
      std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(x3, x2, a4, c3)]);
      sort_indices<0,1,2,3,0,1,1,1>(i0data, i0data_sorted, x3.size(), x2.size(), a4.size(), c3.size());
      // tensor label: Gamma24
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x3, x2, x1, x0);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x3, x2, x1, x0)]);
      sort_indices<0,1,2,3,0,1,-1,1>(i1data, i1data_sorted, x3.size(), x2.size(), x1.size(), x0.size());
      zgemm3m_("T", "N", a4.size()*c3.size(), x1.size()*x0.size(), x3.size()*x2.size(),
             1.0, i0data_sorted, x3.size()*x2.size(), i1data_sorted, x3.size()*x2.size(),
             1.0, odata_sorted, a4.size()*c3.size());
    }
  }
  sort_indices<2,3,0,1,1,1,1,1>(odata_sorted, odata, a4.size(), c3.size(), x1.size(), x0.size());
  out()->put_block(odata, x1, x0, a4, c3);
}
...
void Task376::Task_local::compute() {
  ...
  for (auto& x5 : *range_[1]) {
    for (auto& a3 : *range_[2]) {
      ...
      // tensor label: I666
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x5, x1, x0, a3);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x5, x1, x0, a3)]);
      sort_indices<0,3,1,2,0,1,1,1>(i1data, i1data_sorted, x5.size(), x1.size(), x0.size(), a3.size());
      zgemm3m_("T", "N", c1.size()*a2.size(), x1.size()*x0.size(), x5.size()*a3.size(),
             1.0, i0data_sorted, x5.size()*a3.size(), i1data_sorted, x5.size()*a3.size(),
             1.0, odata_sorted, c1.size()*a2.size());
    }
  }
  sort_indices<0,2,3,1,1,1,1,1>(odata_sorted, odata, c1.size(), a2.size(), x1.size(), x0.size());
...
void Task377::Task_local::compute() {
  ...
  const Index a3 = b(3);
  // tensor label: I666
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(x5, x1, x0, a3);
  std::unique_ptr<std::complex<double>[]> odata_sorted(new std::complex<double>[out()->get_size(x5, x1, x0, a3)]);
  std::fill_n(odata_sorted.get(), out()->get_size(x5, x1, x0, a3), 0.0);
  for (auto& x4 : *range_[1]) {
    for (auto& x3 : *range_[1]) {
      for (auto& x2 : *range_[1]) {
        // tensor label: v2
        std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(a3, x4, x3, x2);
        std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(a3, x4, x3, x2)]);
        sort_indices<1,2,3,0,0,1,1,1>(i0data, i0data_sorted, a3.size(), x4.size(), x3.size(), x2.size());
        // tensor label: Gamma193
        std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x5, x4, x3, x2, x1, x0);
        std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x5, x4, x3, x2, x1, x0)]);
        sort_indices<1,2,3,0,4,5,0,1,1,2>(i1data, i1data_sorted, x5.size(), x4.size(), x3.size(), x2.size(), x1.size(), x0.size());
        zgemm3m_("T", "N", a3.size(), x5.size()*x1.size()*x0.size(), x4.size()*x3.size()*x2.size(),
               1.0, i0data_sorted, x4.size()*x3.size()*x2.size(), i1data_sorted, x4.size()*x3.size()*x2.size(),
               1.0, odata_sorted, a3.size());
      }
    }
  }
  sort_indices<1,2,3,0,1,1,1,1>(odata_sorted, odata, a3.size(), x5.size(), x1.size(), x0.size());
  out()->put_block(odata, x5, x1, x0, a3);
}
...
void Task378::Task_local::compute() {
  ...
  for (auto& x4 : *range_[1]) {
    for (auto& x3 : *range_[1]) {
      for (auto& x2 : *range_[1]) {
        // tensor label: v2
        std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x4, x3, a3, x2);
        std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(x4, x3, a3, x2)]);
        sort_indices<0,1,3,2,0,1,1,1>(i0data, i0data_sorted, x4.size(), x3.size(), a3.size(), x2.size());
        // tensor label: Gamma26
        std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x5, x2, x4, x3, x1, x0);
        std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x5, x2, x4, x3, x1, x0)]);
        sort_indices<2,3,1,0,4,5,0,1,1,2>(i1data, i1data_sorted, x5.size(), x2.size(), x4.size(), x3.size(), x1.size(), x0.size());
        zgemm3m_("T", "N", a3.size(), x5.size()*x1.size()*x0.size(), x2.size()*x4.size()*x3.size(),
               1.0, i0data_sorted, x2.size()*x4.size()*x3.size(), i1data_sorted, x2.size()*x4.size()*x3.size(),
               1.0, odata_sorted, a3.size());
      }
    }
...
void Task379::Task_local::compute() {
  ...
  for (auto& x5 : *range_[1]) {
    for (auto& a3 : *range_[2]) {
      ...
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x5, x1, x0, a3)]);
      sort_indices<0,3,1,2,0,1,1,1>(i1data, i1data_sorted, x5.size(), x1.size(), x0.size(), a3.size());
      zgemm3m_("T", "N", a2.size()*c1.size(), x1.size()*x0.size(), x5.size()*a3.size(),
             1.0, i0data_sorted, x5.size()*a3.size(), i1data_sorted, x5.size()*a3.size(),
...
void Task381::Task_local::compute() {
  ...
  sort_indices<0,1,2,3,1,1,1,1>(odata_sorted, odata, x5.size(), x1.size(), x0.size(), a3.size());
  out()->put_block(odata, x5, x1, x0, a3);
}
