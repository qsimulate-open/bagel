...
Task305::Task305(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x3 : *range[1])
    for (auto& c1 : *range[0])
      for (auto& a3 : *range[2])
        for (auto& c4 : *range[0])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c4, a3, c1, x3}}, in, t[0], range));
}
...
Task307::Task307(vector<shared_ptr<Tensor>> t, array<shared_ptr<const IndexRange>,3> range) {
  ...
  for (auto& x3 : *range[1])
    for (auto& c1 : *range[0])
      for (auto& a4 : *range[2])
        for (auto& c3 : *range[0])
          subtasks_.push_back(make_shared<Task_local>(array<const Index,4>{{c3, a4, c1, x3}}, in, t[0], range));
}
