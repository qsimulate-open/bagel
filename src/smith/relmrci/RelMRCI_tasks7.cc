...
void Task304::Task_local::compute() {
  ...
  for (auto& c4 : *range_[0]) {
    for (auto& a3 : *range_[2]) {
      // tensor label: v2
      std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x2, c4, a3, a2);
      std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(x2, c4, a3, a2)]);
      sort_indices<1,2,0,3,0,1,1,1>(i0data, i0data_sorted, x2.size(), c4.size(), a3.size(), a2.size());
      // tensor label: I577
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c4, a3, c1, x3);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(c4, a3, c1, x3)]);
      sort_indices<0,1,2,3,0,1,1,1>(i1data, i1data_sorted, c4.size(), a3.size(), c1.size(), x3.size());
      zgemm3m_("T", "N", x2.size()*a2.size(), c1.size()*x3.size(), c4.size()*a3.size(),
             1.0, i0data_sorted, c4.size()*a3.size(), i1data_sorted, c4.size()*a3.size(),
             1.0, odata_sorted, x2.size()*a2.size());
    }
  }
  sort_indices<0,1,2,3,1,1,1,1>(odata_sorted, odata, x2.size(), a2.size(), c1.size(), x3.size());
  out()->put_block(odata, x2, a2, c1, x3);
}
...
void Task305::Task_local::compute() {
  ...
  const Index x3 = b(3);
  // tensor label: I577
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c4, a3, c1, x3);
  {
    // tensor label: t2
    std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c4, a3, c1, x3);
    sort_indices<0,1,2,3,1,1,-1,1>(i0data, odata, c4.size(), a3.size(), c1.size(), x3.size());
  }
  {
    // tensor label: t2
    std::unique_ptr<std::complex<double>[]> i1data = in(0)->get_block(c1, a3, c4, x3);
    sort_indices<2,1,0,3,1,1,1,1>(i1data, odata, c1.size(), a3.size(), c4.size(), x3.size());
  }
  out()->put_block(odata, c4, a3, c1, x3);
}
...
void Task306::Task_local::compute() {
  ...
  for (auto& a4 : *range_[2]) {
    for (auto& c3 : *range_[0]) {
      // tensor label: v2
      std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(x2, a2, a4, c3);
      std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(x2, a2, a4, c3)]);
      sort_indices<2,3,0,1,0,1,1,1>(i0data, i0data_sorted, x2.size(), a2.size(), a4.size(), c3.size());
      // tensor label: I580
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c3, a4, c1, x3);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(c3, a4, c1, x3)]);
      sort_indices<1,0,2,3,0,1,1,1>(i1data, i1data_sorted, c3.size(), a4.size(), c1.size(), x3.size());
      zgemm3m_("T", "N", x2.size()*a2.size(), c1.size()*x3.size(), c3.size()*a4.size(),
...
void Task307::Task_local::compute() {
  ...
  const Index x3 = b(3);
  // tensor label: I580
  std::unique_ptr<std::complex<double>[]> odata = out()->move_block(c3, a4, c1, x3);
  {
    // tensor label: t2
    std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(c3, a4, c1, x3);
    sort_indices<0,1,2,3,1,1,1,1>(i0data, odata, c3.size(), a4.size(), c1.size(), x3.size());
  }
  {
    // tensor label: t2
    std::unique_ptr<std::complex<double>[]> i1data = in(0)->get_block(c1, a4, c3, x3);
    sort_indices<2,1,0,3,1,1,-1,1>(i1data, odata, c1.size(), a4.size(), c3.size(), x3.size());
  }
  out()->put_block(odata, c3, a4, c1, x3);
}
...
void Task313::Task_local::compute() {
  ...
  sort_indices<1,2,0,3,1,1,1,1>(odata_sorted, odata, x3.size(), c1.size(), a2.size(), x2.size());
  out()->put_block(odata, c1, a2, x3, x2);
}
...
void Task314::Task_local::compute() {
  ...
  sort_indices<2,1,0,3,1,1,1,1>(odata_sorted, odata, x3.size(), a2.size(), c1.size(), x2.size());
  out()->put_block(odata, c1, a2, x3, x2);
}
...
void Task321::Task_local::compute() {
  ...
  for (auto& a4 : *range_[2]) {
    for (auto& c3 : *range_[0]) {
      ...
      std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(a4, x2, c1, c3)]);
      sort_indices<0,3,1,2,0,1,1,1>(i0data, i0data_sorted, a4.size(), x2.size(), c1.size(), c3.size());
      // tensor label: t2
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(x3, a4, c3, a2);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(x3, a4, c3, a2)]);
      sort_indices<1,2,0,3,0,1,-1,1>(i1data, i1data_sorted, x3.size(), a4.size(), c3.size(), a2.size());
      zgemm3m_("T", "N", x2.size()*c1.size(), x3.size()*a2.size(), a4.size()*c3.size(),
             1.0, i0data_sorted, a4.size()*c3.size(), i1data_sorted, a4.size()*c3.size(),
             1.0, odata_sorted, x2.size()*c1.size());
    }
  }
  sort_indices<1,3,2,0,1,1,1,1>(odata_sorted, odata, x2.size(), c1.size(), x3.size(), a2.size());
  out()->put_block(odata, c1, a2, x3, x2);
}
...
void Task324::Task_local::compute() {
  ...
  for (auto& a4 : *range_[2]) {
    for (auto& c3 : *range_[0]) {
      ...
      // tensor label: v2
      std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c1, x2, a4, c3);
      std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(c1, x2, a4, c3)]);
      sort_indices<2,3,0,1,0,1,1,1>(i1data, i1data_sorted, c1.size(), x2.size(), a4.size(), c3.size());
      zgemm3m_("T", "N", x3.size()*a2.size(), c1.size()*x2.size(), a4.size()*c3.size(),
             1.0, i0data_sorted, a4.size()*c3.size(), i1data_sorted, a4.size()*c3.size(),
             1.0, odata_sorted, x3.size()*a2.size());
    }
  }
...
void Task334::Task_local::compute() {
  ...
  for (auto& a5 : *range_[2]) {
    for (auto& a4 : *range_[2]) {
      for (auto& c3 : *range_[0]) {
        // tensor label: v2
        std::unique_ptr<std::complex<double>[]> i0data = in(0)->get_block(a5, a2, a4, c3);
        std::unique_ptr<std::complex<double>[]> i0data_sorted(new std::complex<double>[in(0)->get_size(a5, a2, a4, c3)]);
        sort_indices<0,2,3,1,0,1,1,1>(i0data, i0data_sorted, a5.size(), a2.size(), a4.size(), c3.size());
        // tensor label: t2
        std::unique_ptr<std::complex<double>[]> i1data = in(1)->get_block(c1, a5, c3, a4);
        std::unique_ptr<std::complex<double>[]> i1data_sorted(new std::complex<double>[in(1)->get_size(c1, a5, c3, a4)]);
        sort_indices<1,3,2,0,0,1,2,1>(i1data, i1data_sorted, c1.size(), a5.size(), c3.size(), a4.size());
        zgemm3m_("T", "N", a2.size(), c1.size(), a5.size()*c3.size()*a4.size(),
               1.0, i0data_sorted, a5.size()*c3.size()*a4.size(), i1data_sorted, a5.size()*c3.size()*a4.size(),
               1.0, odata_sorted, a2.size());
      }
    }
  }
