...
ASDCASSCF::ASDCASSCF(std::shared_ptr<const PTree> idat, const shared_ptr<const Geometry> geom, const shared_ptr<const Reference> re) 
  : Method(idat, geom, re), hcore_(make_shared<Hcore>(geom)) {
  cout << "ASDCASSCF object constructed" << endl;
  if (!ref_) { //ref_ from method class
    cout << "ASDCASSCF ctor: no reference given, create reference.." << endl;
...
/*
ASDCASSCF::ASDCASSCF(std::shared_ptr<const PTree> idat, const shared_ptr<const Geometry> geom, const shared_ptr<const Reference> ref, 
...
void ASDCASSCF::common_init() {
  // at the moment I only care about C1 symmetry, with dynamics in mind
  if (dimer_->sgeom()->nirrep() > 1) throw runtime_error("ASDCASSCF: C1 only at the moment.");
  print_header();
...
//REMOVED
  // ASDCASSCF methods should have FCI member. Inserting "ncore" and "norb" keyword for closed and total orbitals.
//mute_stdcout();
...
void ASDCASSCF::resume_stdcout() {
  cout.rdbuf(backup_stream_);
  delete ofs_;
}


shared_ptr<Matrix> ASDCASSCF::ao_rdm1(shared_ptr<const RDM<1>> rdm1, const bool inactive_only) const {
...
shared_ptr<Matrix> ASDCASSCF::Qvec(const int n, const int m, shared_ptr<const Matrix> coeff, const size_t nclosed) const {
  //orig: asd_nevpt2_mat.cc
  //use half_ (see form_natorb)
  assert(n == coeff->mdim());

  cout << "ASDCASSCF-Qvec: check 1" << endl;
...
shared_ptr<const Coeff> ASDCASSCF::semi_canonical_orb() const {
  cout << "ASDCASSCF: semi_canonical_orb: not implemented" << endl;
  assert(false);
#if 0
...
shared_ptr<const Reference> ASDCASSCF::conv_to_ref() const {
  cout << "ASDCASSCF: conv_to_ref: not implemented" << endl;
  assert(false);
/*
...
void ASDCASSCF::print_natocc() const {
...
pair<shared_ptr<Matrix>, vector<double>> ASDCASSCF::natorb_convert() {
  cout << "ASDCASSCF: natorb_convert.." << endl;
...
//pair<shared_ptr<Matrix>, VectorB> natorb = rdm1_->generate_natural_orbitals();
  auto rdm1_mat = make_shared<Matrix>(*(rdm1_->rdm1_mat(/*closed*/0)));
  pair<shared_ptr<Matrix>, vector<double>> natorb = generate_natural_orbitals(rdm1_mat);
  for (int i = 0; i != nact_; ++i) {
...
void ASDCASSCF::update_rdms(const shared_ptr<Matrix>& coeff) {
  cout << "ASDCASSCF: update_rdms.." << endl;
...
pair<shared_ptr<Matrix>, vector<double>> ASDCASSCF::generate_natural_orbitals(shared_ptr<Matrix> rdm1) {
  ...
  cout << nactA_ << nactB_ << nact_ << endl;
//shared_ptr<Matrix> rdm1 = rdm1_av_->rdm1_mat(/*closed*/0);
  //subspace rdm
  vector<shared_ptr<Matrix>> srdm;
  array<int,2> act = { nactA_, nactB_ };
  srdm.push_back(make_shared<Matrix>(*rdm1->get_submatrix(0,0,nactA_,nactA_)));
  srdm.push_back(make_shared<Matrix>(*rdm1->get_submatrix(nactA_,nactA_,nactB_,nactB_)));
  cout << "check.." << endl;
  auto outm = make_shared<Matrix>(nact_,nact_,true);
  outm->unit();
  vector<double> outv(nact_);

  for(int ispace = 0; ispace != 2; ++ispace) {
    cout << "ispace = " << ispace << endl;
    auto buf = make_shared<Matrix>(act[ispace],act[ispace],true);
    buf->add_diag(2.0);
    daxpy_(act[ispace]*act[ispace], -1.0, srdm[ispace]->data(), 1, buf->data(), 1);
 
    VectorB vec(act[ispace]);
    buf->diagonalize(vec);
    for (int i = 0; i != act[ispace]; ++i) {
    ...
    for (int i = 0; i != act[ispace]; ++i) {
      ...
      // copy to the target
      copy_n(buf->element_ptr(0,get<0>(max)), act[ispace], buf2->element_ptr(0,i));
    ...
    for (int i = 0; i != act[ispace]; ++i) {
      if (buf2->element(i,i) < 0.0)
        blas::scale_n(-1.0, buf2->element_ptr(0,i), act[ispace]);
    }
    ...
    assert(buf2->ndim() == act[ispace]);
    assert(buf2->mdim() == act[ispace]);
    assert(vec2.size() == act[ispace]);
    switch (ispace) {
      case 0: //A
        ...
        break;
      case 1: //B
        outm->copy_block(act[0], act[0], buf2->ndim(), buf2->mdim(), buf2);
        for (int i = 0; i != vec2.size(); ++i) {
          outv[act[0]+i] = vec2(i);
        }
        break;
      default:
        cout << "undefined monomer space" << endl;
        assert(false);
        break;
    }
  } //ispace
