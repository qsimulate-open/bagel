...
// This is a base class for various ASDCASSCF solvers.
...
#ifndef __BAGEL_ASD_CASSCF_H
#define __BAGEL_ASD_CASSCF_H
...
#include <src/asd/orbital/activerotfile.h>
...
class ASDCASSCF : public Method, public std::enable_shared_from_this<ASDCASSCF> {
...
//END ADDED
  ...
  public:
    ASDCASSCF(const std::shared_ptr<const PTree> idat, const std::shared_ptr<const Geometry> geom, const std::shared_ptr<const Reference> = nullptr);

  //ASDCASSCF(const std::shared_ptr<const PTree> idat, const std::shared_ptr<const Geometry> geom, const std::shared_ptr<const Reference> ref,
  //           std::tuple<std::shared_ptr<RDM<1>>,
  //                      std::shared_ptr<RDM<2>>> rdms );
    virtual ~ASDCASSCF();

    std::pair<std::shared_ptr<Matrix>, std::vector<double>> generate_natural_orbitals(std::shared_ptr<Matrix> rdm1);
