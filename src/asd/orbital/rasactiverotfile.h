...
class ASD_RAS_ActiveRotationMatrix {
  ...
  public:
    ...
    // RAS A
    DataType* ptr_aa21A() { return data() + nactA_*nactB_; }
    DataType& ele_aa21A(const int i2, const int i1) { return data_[nactA_*nactB_ 
                                                                  + i2 + i1*rasA_[1]]; }

    DataType* ptr_aa31A() { return data() + nactA_*nactB_ + rasA_[0]*rasA_[1]; }
    DataType& ele_aa31A(const int i3, const int i1) { return data_[nactA_*nactB_ 
                                                                  + rasA_[0]*rasA_[1] + i3 + i1*rasA_[2]]; }

    DataType* ptr_aa32A() { return data() + nactA_*nactB_ + rasA_[0]*rasA_[1] + rasA_[0]*rasA_[2]; }
    DataType& ele_aa32A(const int i3, const int i2) { return data_[nactA_*nactB_ 
                                                                  + rasA_[0]*rasA_[1] + rasA_[0]*rasA_[2] + i3 + i2*rasA_[2]]; }
    // RASB
    DataType* ptr_aa21B() { return data() + nactA_*nactB_ + rasA_[0]*rasA_[1] + rasA_[0]*rasA_[2] + rasA_[1]*rasA_[2]; }
    DataType& ele_aa21B(const int i2, const int i1) { return data_[nactA_*nactB_ 
                                                                  + rasA_[0]*rasA_[1] + rasA_[0]*rasA_[2] + rasA_[1]*rasA_[2] 
                                                                  + i2 + i1*rasB_[1]]; }

    DataType* ptr_aa31B() { return data() + nactA_*nactB_ + rasA_[0]*rasA_[1] + rasA_[0]*rasA_[2] + rasA_[1]*rasA_[2]
                                                                                                      + rasB_[0]*rasB_[1]; }
    DataType& ele_aa31B(const int i3, const int i1) { return data_[nactA_*nactB_ 
                                                                  + rasA_[0]*rasA_[1] + rasA_[0]*rasA_[2] + rasA_[1]*rasA_[2] 
                                                                  + rasB_[0]*rasB_[1] + i3 + i1*rasB_[2]]; }

    DataType* ptr_aa32B() { return data() + nactA_*nactB_ + rasA_[0]*rasA_[1] + rasA_[0]*rasA_[2] + rasA_[1]*rasA_[2]
                                                                                                      + rasB_[0]*rasB_[1] + rasB_[0]*rasB_[2]; }
    DataType& ele_aa32B(const int i3, const int i2) { return data_[nactA_*nactB_ 
                                                                  + rasA_[0]*rasA_[1] + rasA_[0]*rasA_[2] + rasA_[1]*rasA_[2] 
    ...
    const DataType& ele_aa21A(const int i2, const int i1) const { return data_[nactA_*nactB_ + i2 + i1*rasA_[1]]; }
    const DataType& ele_aa31A(const int i3, const int i1) const { return data_[nactA_*nactB_ + rasA_[0]*rasA_[1] + i3 + i1*rasA_[2]]; }
    const DataType& ele_aa32A(const int i3, const int i2) const { return data_[nactA_*nactB_ + rasA_[0]*rasA_[1] + rasA_[0]*rasA_[2] + i3 + i2*rasA_[2]]; }
    const DataType& ele_aa21B(const int i2, const int i1) const { return data_[nactA_*nactB_ + rasA_[0]*rasA_[1] + rasA_[0]*rasA_[2] + rasA_[1]*rasA_[2] + i2 + i1*rasB_[1]]; }
    const DataType& ele_aa31B(const int i3, const int i1) const { return data_[nactA_*nactB_ + rasA_[0]*rasA_[1] + rasA_[0]*rasA_[2] + rasA_[1]*rasA_[2] + rasB_[0]*rasB_[1] + i3 + i1*rasB_[2]]; }
    const DataType& ele_aa32B(const int i3, const int i2) const { return data_[nactA_*nactB_ + rasA_[0]*rasA_[1] + rasA_[0]*rasA_[2] + rasA_[1]*rasA_[2] + rasB_[0]*rasB_[1] + rasB_[0]*rasB_[2] + i3 + i2*rasB_[2]]; }

    const DataType* ptr_aa() const { return data(); }
    const DataType* ptr_aa21A() const { return data() + nactA_*nactB_; }
    const DataType* ptr_aa31A() const { return data() + nactA_*nactB_ + rasA_[0]*rasA_[1]; }
    const DataType* ptr_aa32A() const { return data() + nactA_*nactB_ + rasA_[0]*rasA_[1] + rasA_[0]*rasA_[2]; }
    const DataType* ptr_aa21B() const { return data() + nactA_*nactB_ + rasA_[0]*rasA_[1] + rasA_[0]*rasA_[2] + rasA_[1]*rasA_[2]; }
    const DataType* ptr_aa31B() const { return data() + nactA_*nactB_ + rasA_[0]*rasA_[1] + rasA_[0]*rasA_[2] + rasA_[1]*rasA_[2]
                                                                                                                  + rasB_[0]*rasB_[1]; }
    const DataType* ptr_aa32B() const { return data() + nactA_*nactB_ + rasA_[0]*rasA_[1] + rasA_[0]*rasA_[2] + rasA_[1]*rasA_[2]
                                                                                                                  + rasB_[0]*rasB_[1] + rasB_[0]*rasB_[2]; }
