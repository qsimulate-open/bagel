...
void ASD_BFGS::compute() {
  ...
  shared_ptr<SRBFGS<ASD_ActiveRotFile>> bfgs;
...
//END
    ...
    auto sigma = make_shared<ASD_ActiveRotFile>(nclosed_, nact_, nvirt_, nactA_, nactB_ );
    sigma->zero();
...
//ADDED
    ...
    // grad(a/a)
    grad_aa_small(mcfock, sigma);

    // if this is the first time, set up the BFGS solver
    if (iter == 0) {
      // BFGS and DIIS should start at the same time
      shared_ptr<const ASD_ActiveRotFile> denom = compute_denom_small(cfock, afock, rdm1_mat, mcfock);
      bfgs = make_shared<SRBFGS<ASD_ActiveRotFile>>(denom);
    }
    ...
    shared_ptr<const ASD_ActiveRotFile> a = bfgs->more_sorensen_extrapolate(sigma, xlog);
    cout << " ---------------------------------------------------- " << endl;
