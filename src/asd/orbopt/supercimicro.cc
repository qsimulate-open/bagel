...
void ASDSuperCIMicro::compute() {

  using SCIData = PairFile<Matrix,ASDRotFile>;
  ...
  auto cc1    = make_shared<ASDRotFile>(nclosed, nact, nvirt);
  // BFGS initialization
  auto mbfgs = make_shared<BFGS<SCIData>>(make_shared<SCIData>(cc0, make_shared<ASDRotFile>(*denom_)));

  for (int miter = 0; miter != casscf_->max_micro_iter(); ++miter) {
    Timer mtimer;

    shared_ptr<ASDRotFile> sigma1;
    ...
    if (miter+1 == casscf_->max_micro_iter()) throw runtime_error("max_micro_iter_ is reached in ASDSCF");
  ...
  cout << "cref =" << cref << endl;
  shared_ptr<ASDRotFile> tmp = result->second()->copy();
  *tmp *= 1.0/cref;
  tmp->synchronize();
  cc_ = tmp;
}
...
std::shared_ptr<ASDRotFile> ASDSuperCIMicro::form_sigma(std::shared_ptr<const ASDRotFile> cc) const {
...
void ASDSuperCIMicro::sigma_at_at_(shared_ptr<const ASDRotFile> cc, shared_ptr<ASDRotFile> sigma) const {
  const int nact = casscf_->nact();
  const int nvirt = casscf_->nvirt();
...
void ASDSuperCIMicro::sigma_ai_ai_(shared_ptr<const ASDRotFile> cc, shared_ptr<ASDRotFile> sigma) const {
  const int nclosed = casscf_->nclosed();
  const int nvirt = casscf_->nvirt();
  const int nocc = casscf_->nocc();
  const int nbasis = casscf_->nbasis();
...
void ASDSuperCIMicro::sigma_at_ai_(shared_ptr<const ASDRotFile> cc, shared_ptr<ASDRotFile> sigma) const {
  const int nclosed = casscf_->nclosed();
  const int nact = casscf_->nact();
  const int nvirt = casscf_->nvirt();
...
void ASDSuperCIMicro::sigma_ai_ti_(shared_ptr<const ASDRotFile> cc, shared_ptr<ASDRotFile> sigma) const {
  const int nclosed = casscf_->nclosed();
  const int nact = casscf_->nact();
  const int nvirt = casscf_->nvirt();
  const int nocc = casscf_->nocc();
