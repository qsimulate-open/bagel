...
// This is a base class for various ASDSCF2 solvers.
...
#ifndef __BAGEL_ASDSCF2_ASDSCF2_H
#define __BAGEL_ASDSCF2_ASDSCF2_H

#include <src/wfn/reference.h>
#include <src/ci/fci/knowles.h>
#include <src/asd/orbopt/asdrotfile2.h>
...
class ASDSCF2 : public Method, public std::enable_shared_from_this<ASDSCF2> {
...
//END ADDED
  ...
  public:
    ASDSCF2(const std::shared_ptr<const PTree> idat, const std::shared_ptr<const Geometry> geom, const std::shared_ptr<const Reference> = nullptr);

  //ASDSCF2(const std::shared_ptr<const PTree> idat, const std::shared_ptr<const Geometry> geom, const std::shared_ptr<const Reference> ref,
  //           std::tuple<std::shared_ptr<RDM<1>>,
  ...
  //std::shared_ptr<const FCI> fci() const { return fci_; };
    ...
    // TODO I need this function in CP-ASDSCF2, but only for denominator. Should be separated.
    void one_body_operators(std::shared_ptr<Matrix>&, std::shared_ptr<Matrix>&, std::shared_ptr<Matrix>&, std::shared_ptr<Matrix>&,
                            std::shared_ptr<ASDRotFile2>&, const bool superci=true) const;
...
const double occup_thresh2_ = 1.0e-10;
