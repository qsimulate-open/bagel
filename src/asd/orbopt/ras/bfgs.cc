...
//ADDED
    ...
    if (iter == 0) {
      ...
      shared_ptr<const ASDRASRotFile> denom = compute_denom(cfock, afock, qxr, rdm1_mat, mcfock);
      bfgs = make_shared<SRBFGS<ASDRASRotFile>>(denom);
    }
...
//  *x *= *natorb_mat;
    ...
    // restore the matrix from ASDRASRotFile
