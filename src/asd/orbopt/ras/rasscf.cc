...
ASDRASSCF::ASDRASSCF(std::shared_ptr<const PTree> idat, const shared_ptr<const Geometry> geom, const shared_ptr<const Reference> re) 
  : Method(idat, geom, re), hcore_(make_shared<Hcore>(geom)) {
  cout << "ASDRASSCF object constructed" << endl;
  if (!ref_) { //ref_ from method class
    cout << "ASDRASSCF ctor: no reference given, create reference.." << endl;
...
/*
ASDRASSCF::ASDRASSCF(std::shared_ptr<const PTree> idat, const shared_ptr<const Geometry> geom, const shared_ptr<const Reference> ref, 
...
void ASDRASSCF::common_init() {
  // at the moment I only care about C1 symmetry, with dynamics in mind
  if (dimer_->sgeom()->nirrep() > 1) throw runtime_error("ASDRASSCF: C1 only at the moment.");
  print_header();
...
//REMOVED
  // ASDRASSCF methods should have FCI member. Inserting "ncore" and "norb" keyword for closed and total orbitals.
//mute_stdcout();
...
//END REMOVED
  ...
  cout <<  "  === ASDRASSCF iteration (" + geom_->basisfile() + ") ===" << endl << endl;
...
void ASDRASSCF::print_header() const {
  cout << "  ---------------------------" << endl;
  cout << "      ASDRASSCF calculation     " << endl;
  cout << "  ---------------------------" << endl << endl;
}

void ASDRASSCF::print_iteration(int iter, int miter, int tcount, const vector<double> energy, const double error, const double time) const {
...
void ASDRASSCF::resume_stdcout() {
  cout.rdbuf(backup_stream_);
  delete ofs_;
}


shared_ptr<Matrix> ASDRASSCF::ao_rdm1(shared_ptr<const RDM<1>> rdm1, const bool inactive_only) const {
...
shared_ptr<Matrix> ASDRASSCF::Qvec(const int n, const int m, shared_ptr<const Matrix> coeff, const size_t nclosed) const {
  //orig: asd_nevpt2_mat.cc
  //use half_ (see form_natorb)
  assert(n == coeff->mdim());

  cout << "ASDRASSCF-Qvec: check 1" << endl;
...
void ASDRASSCF::one_body_operators(shared_ptr<Matrix>& f, shared_ptr<Matrix>& fact, shared_ptr<Matrix>& factp, shared_ptr<Matrix>& gaa,
                                shared_ptr<ASDRASRotFile>& d, const bool superci) const {
...
//END ADDED
...

  {
    ...
    for (int i = 0; i != nact_; ++i)
      for (int j = 0; j != nact_; ++j) {
#if 1
        if (occup_(i)+occup_(j) > occup_thresh3_)
...
//dgemv_("N", nact_*nact_, nact_*nact_, 1.0, fci_->rdm2_av()->data(), nact_*nact_, factp->data(), 1, 0.0, gaa->data(), 1);
  ...
  target = denom->ptr_ca();
  for (int i = 0; i != nact_; ++i) {
    if (2.0-occup_(i) > occup_thresh3_) {
...
shared_ptr<const Coeff> ASDRASSCF::update_coeff(const shared_ptr<const Matrix> cold, shared_ptr<const Matrix> mat) const {
...
shared_ptr<Matrix> ASDRASSCF::form_natural_orbs() {
...
shared_ptr<const Coeff> ASDRASSCF::semi_canonical_orb() const {
  cout << "ASDRASSCF: semi_canonical_orb: not implemented" << endl;
  assert(false);
#if 0
  auto rdm1mat = make_shared<Matrix>(nact_, nact_);
  copy_n(fci_->rdm1_av()->data(), rdm1mat->size(), rdm1mat->data());
...
shared_ptr<const Reference> ASDRASSCF::conv_to_ref() const {
  cout << "ASDRASSCF: conv_to_ref: not implemented" << endl;
  assert(false);
/*
...
void ASDRASSCF::print_natocc() const {
...
pair<shared_ptr<Matrix>, VectorB> ASDRASSCF::natorb_convert() {
  cout << "ASDRASSCF: natorb_convert.." << endl;
...
void ASDRASSCF::update_rdms(const shared_ptr<Matrix>& coeff) {
  cout << "ASDRASSCF: update_rdms.." << endl;
