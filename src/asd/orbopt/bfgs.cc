...
//ONLY GROUND STATE
    ...
    auto sigma = make_shared<ASDRotFile>(nclosed_, nact_, nvirt_);
    sigma->zero();
...
//ADDED
    ...
    if (iter == 0) {
      ...
      shared_ptr<const ASDRotFile> denom = compute_denom(cfock, afock, qxr);
      bfgs = make_shared<SRBFGS<ASDRotFile>>(denom);
    }
    ...
    *x *= *natorb_mat;
    auto xcopy = x->log(8);
    auto xlog  = make_shared<ASDRotFile>(xcopy, nclosed_, nact_, nvirt_);
    bfgs->check_step(evals, sigma, xlog);
    shared_ptr<ASDRotFile> a = bfgs->more_sorensen_extrapolate(sigma, xlog);
    cout << " ---------------------------------------------------- " << endl << endl;

    // restore the matrix from ASDRotFile
...
shared_ptr<const ASDRotFile> ASDBFGS::compute_denom(shared_ptr<const Matrix> cfock, shared_ptr<const Matrix> afock, shared_ptr<const Matrix> qxr) const {
  auto out = make_shared<ASDRotFile>(nclosed_, nact_, nvirt_);
...
// grad(a/i) (eq.4.3a): 4(cfock_ai+afock_ai)
void ASDBFGS::grad_vc(shared_ptr<const Matrix> cfock, shared_ptr<const Matrix> afock, shared_ptr<ASDRotFile> sigma) const {
...
void ASDBFGS::grad_va(shared_ptr<const Matrix> cfock, shared_ptr<const Matrix> qxr, shared_ptr<ASDRotFile> sigma) const {
...
// grad(r/i) (eq.4.3c): 4(cfock_ri+afock_ri) - 2cfock_iu gamma_ur - qxr_ir
void ASDBFGS::grad_ca(shared_ptr<const Matrix> cfock, shared_ptr<const Matrix> afock, shared_ptr<const Matrix> qxr, shared_ptr<ASDRotFile> sigma) const {
