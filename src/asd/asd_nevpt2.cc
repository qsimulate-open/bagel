...
//ref_ = casscf_->conv_to_ref();
  ...
  cout << endl << "  === DF-ASD_NEVPT2 calculation ===" << endl << endl;
...
//norm_thresh_ = idata_->get<double>("norm_thresh", 1.0e-13);
  abasis_ = "";
  norm_thresh_ = 1.0e-13;

}


void ASD_NEVPT2::compute() {
...
//nvirt_ = ref_->nvirt();
  ...
  cout << "      ASD_NEVPT2 correlation energy: " << fixed << setw(15) << setprecision(10) << energy_ << setw(10) << setprecision(2) << timer.tick() << endl << endl;
  for (auto& i : sect)
    cout << "          " << setw(7) << left << i.first << right << setw(15) << setprecision(10) << energy[i.second] << endl;
  cout << endl;

  energy_ += ref_->energy();
  cout << "      ASD_NEVPT2 total energy:       " << fixed << setw(15) << setprecision(10) << energy_ << endl << endl;
