...
class ASD_DistCAS : public ASD<DistDvec> {
  ...
  private:
    std::tuple<std::shared_ptr<RDM<1>>, std::shared_ptr<RDM<2>>> compute_rdm12_monomer(std::pair<int,int>, std::array<DistDvec,4>&) const override;
    std::tuple<std::shared_ptr<RDM<3>>, std::shared_ptr<RDM<4>>, std::shared_ptr<RDM<4>>> compute_rdm34_monomer(std::pair<int,int>, std::array<DistDvec,4>&) const override;
