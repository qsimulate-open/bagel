...
class ASD_DistRAS : public ASD<DistRASDvec> {
  ...
  private:
    ...
    std::tuple<std::shared_ptr<RDM<1>>,std::shared_ptr<RDM<2>>> compute_rdm12_monomer(std::shared_ptr<const DistRASDvec> civec, const int i, std::shared_ptr<const DistRASDvec> cipvec, const int j) const override;
};
