...
shared_ptr<Matrix> Reference::rdm2deriv_offset(shared_ptr<const Matrix> dmat, const int istate, const size_t offset, const size_t size) const {
  FCI_bare fci(ciwfn_);
  return fci.rdm2deriv_offset(dmat, istate, offset, size);
}
