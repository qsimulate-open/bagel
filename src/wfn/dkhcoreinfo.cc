...
shared_ptr<const Matrix> DKHcoreInfo::compute_tden(shared_ptr<const Matrix> rdm1) {
  ...
  //         for (int q = 0; q != nbasis_; ++q) {
  //           for (int p = 0; p != nbasis_; ++p) {
  //             (*den)(a, b) += (N(p, r) * O(q, r) / (2 * (E(p) + E(r)) * (E(q) + E(r)))) * ((G(q) * dF(p) + F(q) * dG(p) - (F(p) * G(q) * dE(p)
  //                         + F(q) * G(p) * dE(p)) / (E(p) + E(r))) * H(r) * wtrans_(a, p) * wtrans_(b, p) + (F(p) * dG(q) + G(p) * dF(q)
  //                         - (F(p) * G(q) * dE(q) + F(q) * G(p) * dE(q)) / (E(q) + E(r))) * H(r) * wtrans_(a, q) * wtrans_(b, q) + (F(p) * G(q) * dH(r)
  //                         + F(q) * G(p) * dH(r) - (F(p) * G(q) * H(r) * dE(r) + F(q) * G(p) * H(r) * dE(r)) / (E(p) + E(r))
  //                         - (F(p) * G(q) * H(r) * dE(r) + F(q) * G(p) * H(r) * dE(r)) / (E(q) + E(r))) * wtrans_(a, r) * wtrans_(b, r)) * CPW(p, q);
  //           }
  //         }
  //       }
  //     }
    ...
    for (int a = 0; a != nbasis_; ++a) {
      ...
      for (int q = 0; q != nbasis_; ++q) {
        for (int p = 0; p != nbasis_; ++p) {
          ...
          // for (int r = 0; r != nbasis_; ++r) {
          //   (*den)(a, b) += (1 / ((E(p) + E(r)) * (E(r) + E(q)))) * (-B(p) * smallnai_(p, r) * B(r) * E(r) * A(r) * nai_(r, q) * A(q)
          //               - A(p) * nai_(p, r) * A(r) * E(r) * B(r) * smallnai_(r, q) * B(q)
          //               + 2 * A(p) * nai_(p, r) * pow(A(r) * K(r), 2) * kinetic_(r) * E(r)
          //               + 0.5 * B(p) * smallnai_(p, r) * (pow(B(r) / K(r), 2) * E(p) / kinetic_(r)) * smallnai_(r, q) * B(q)
          //               - 0.5 * B(p) * smallnai_(p, r) * B(r) * A(r) * nai_(r, q) * A(q) * E(q)
          //               - 0.5 * A(p) * nai_(p, r) * A(r) * B(r) * smallnai_(r, q) * B(q) * E(q)
          //               + A(p) * nai_(p, r) * pow(A(r) * K(r), 2) * kinetic_(r) * nai_(r, q) * A(q) * E(q)
          //               + 0.25 * B(p) * smallnai_(p, r) * (pow(B(r) / K(r), 2) / kinetic_(r)) * smallnai_(r, q) * B(q) * E(q)
          //               - 0.5 * E(p) * B(p) * smallnai_(p, r) * B(r) * A(r) * nai_(r, q) * A(q)
          //               - 0.5 * E(p) * A(p) * nai_(p, r) * A(r) * B(r) * smallnai_(r, q) * B(q)
          //               + E(p) * A(p) * nai_(p, r) * pow(A(r) * K(r), 2) * kinetic_(r) * nai_(r, q) * A(q)
          //               + 0.25 * E(p) * B(p) * smallnai_(p, r) * (pow(B(r) / K(r), 2) / kinetic_(r)) * smallnai_(r, q) * B(q))
