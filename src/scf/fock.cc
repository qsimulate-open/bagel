...
// Fock matrix for standard basis without density fitting
template <>
void Fock<0, Matrix>::fock_two_electron_part(shared_ptr<const Matrix> den_ex) {
...
template<int DF, typename MatType>
void Fock<DF, MatType>::fock_two_electron_part(shared_ptr<const MatType> den_ex) {
  ...
  shared_ptr<MatType> coeff = den_ex->copy();
  *coeff *= -1.0;
  int nocc = 0;
  {
    VectorB vec(ndim());
    ...
    for (int i = 0; i != ndim(); ++i) {
      if (vec[i] < -1.0e-8) {
        ...
        std::for_each(coeff->element_ptr(0,i), coeff->element_ptr(0,i+1), [&fac](DataType& i) { i *= fac; });
      } else { break; }
    }
  }
  if (nocc == 0) return;
...
template<int DF, typename MatType>
void Fock<DF, MatType>::fock_two_electron_part_with_coeff(const MatView_<DataType> ocoeff, const bool rhf, const double scale_exchange) {
  ...
  if (scale_exchange != 0.0) {
    ...
    *this += *build_exchange(half, -1.0*scale_exchange);
    pdebug.tick_print("Exchange build");

    if (rhf) {
      MatType oc(ocoeff);
      auto coeff = make_shared<const MatType>(*oc.transpose()*2.0);
...
template<>
shared_ptr<ZMatrix> Fock<1, ZMatrix>::build_exchange(shared_ptr<DFHalfDist> half, double scale) {
  auto tmp = dynamic_pointer_cast<ComplexDFHalfDist>(half);
  assert(tmp);
  return tmp->complex_form_2index(half, scale);
}


// TODO enable_if should allow us to eliminate this
template<>
shared_ptr<ZMatrix> Fock<0, ZMatrix>::build_exchange(shared_ptr<DFHalfDist> half, double scale) {
  throw logic_error("Fock<DF>::build_exchange(...) is only for DF cases");
  return nullptr;
}

// Fock matrix for GIAO basis without density fitting
template<>
void Fock<0, ZMatrix>::fock_two_electron_part(shared_ptr<const ZMatrix> den_ex) {
            ...
            for (int j1 = b1offset; j1 != b1offset + b1size; ++j1) {
              ...
              for (int j2 = b2offset; j2 != b2offset + b2size; ++j2) {
                ...
                for (int j3 = b3offset; j3 != b3offset + b3size; ++j3, ++eridata) {
                  ...
                  element(j0, j1) += density_data[j2n + j3] * conj(intval2); // Coulomb  (ba|dc)
                  element(j0, j3) -= density_data[j2n + j1] * conj(intval);  // Exchange (da|bc)

                  element(j2, j3) += density_data[j0n + j1] * conj(intval2); // Coulomb  (dc|ba)
                  element(j2, j1) -= density_data[j0n + j3] * conj(intval);  // Exchange (bc|dz)
                }
              }
            }
          }
...
template class bagel::Fock<0, Matrix>;
template class bagel::Fock<1, Matrix>;
template class bagel::Fock<0, ZMatrix>;
template class bagel::Fock<1, ZMatrix>;

BOOST_CLASS_EXPORT_IMPLEMENT(Fock_0)
BOOST_CLASS_EXPORT_IMPLEMENT(Fock_1)
BOOST_CLASS_EXPORT_IMPLEMENT(Fock_2)
BOOST_CLASS_EXPORT_IMPLEMENT(Fock_3)
