    ...
    if (method == "molecule") {
      ...
      if (algorithm == "second" || algorithm == "") {
        ...
        if (aniso_data) {
          ...
          const int nele = geom->nele() - zcas->charge() - zcas->nclosed()*2;
          //auto ps = std::make_shared<Pseudospin>(nspin, nele, zcas->nact(), aniso_data);
          auto ps = std::make_shared<Pseudospin>(nspin, nele, geom, zcas->fci()->conv_to_ciwfn(), aniso_data);
          ps->compute(*zcas->fci(), zcas->energy(), zcas->coeff()->active_part());
          out.clear();
