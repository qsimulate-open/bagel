...
void NAIBatch::compute() {
  ...
  for (int xj = 0; xj != screening_size_; ++xj) {
    ...
    for (int r = 0; r != rank_; ++r) {
      r1x[r] = p_[i * 3    ] - ax - (p_[i * 3    ] - catom->position(0)) * croots[r]; 
      r1y[r] = p_[i * 3 + 1] - ay - (p_[i * 3 + 1] - catom->position(1)) * croots[r]; 
      r1z[r] = p_[i * 3 + 2] - az - (p_[i * 3 + 2] - catom->position(2)) * croots[r]; 
