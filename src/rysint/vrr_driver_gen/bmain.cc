...
";
  ...
  for (int rank = 1; rank <= 13; ++rank) {
    ...
    ss << "          worktx[i+" << rank << "*(ia+amax2*ic)] = pq[0]*workx[i+" << rank << "*(ia+amax2*ic)] + (ia==0 ? 0.0 : ia*oxp2*workx[i+" << rank << "*(ia-1+amax2*ic)]) - (ic==0 ? 0.0 : ic*oxq2*workx[i+" << rank << "*(ia+amax2*(ic-1))]);" << endl;
