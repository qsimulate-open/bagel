...
void CarSphList::carsph_31(const int nloop, const double* source, double* target) {
  ...
  for (int iloop = 0; iloop != nloop; ++iloop, target += 21, source += 30) {
    ...
    target[6] =  c2 * (source[12] - source[18]);
    target[7] =  c2 * (source[13] - source[19]);
    target[8] =  c2 * (source[14] - source[20]);
    target[9] =  c3 * source[15];
    target[10] =  c3 * source[16];
    target[11] =  c3 * source[17];
    target[12] =  c4 * source[21] - c5 * (source[0] + source[6]);
    target[13] =  c4 * source[22] - c5 * (source[1] + source[7]);
    target[14] =  c4 * source[23] - c5 * (source[2] + source[8]);
    target[15] =  c4 * source[24] - c5 * (source[3] + source[9]);
    target[16] =  c4 * source[25] - c5 * (source[4] + source[10]);
    target[17] =  c4 * source[26] - c5 * (source[5] + source[11]);
    target[18] =  source[27] - c6 * (source[12] + source[18]);
    target[19] =  source[28] - c6 * (source[13] + source[19]);
    target[20] =  source[29] - c6 * (source[14] + source[20]);
  }
}
