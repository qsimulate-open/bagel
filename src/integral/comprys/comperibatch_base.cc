...
#include <src/integral/comprys/comperibatch_base.h>
...
CompERIBatch_base::CompERIBatch_base(const array<shared_ptr<const Shell>,4>& o, const double max_density, const int deriv, const int breit, shared_ptr<StackMem> stack)
 : RysIntegral<std::complex<double>>(o, stack) {
...
void CompERIBatch_base::root_weight(const int ps) {

  // These six lines get it to compile without complaining about a type mismatch.  In the final version, we must link the correct
  // input values to the T_, roots_, and weights_ pointers (among others).  Trying to do calculations with this setup will surely segfault.
  complex<double> placeholder(0.0,0.0);
  complex<double> placeholder2(0.0,0.0);
  complex<double> placeholder3(0.0,0.0);
  complex<double> *cccT_ = &placeholder;
  complex<double> *cccroots_ = &placeholder2;
  complex<double> *cccweights_ = &placeholder3;

  if (breit_ == 0) {
      // comperi.root(rank_, T_, roots_, weights_, ps);
      comperi.root(rank_, cccT_, cccroots_, cccweights_, ps); // Once the above placeholders can be removed, delete this line & use the proper inputs
    }
  } else if (breit_ == 1) {
    throw logic_error("Breit integrals have not set up in a London orbital basis");
  } else if (breit_ == 2) {
    throw logic_error("Spin2 integrals have not set up in a London orbital basis");
  } else {
    assert(false);
  }
}

/* TODO Prepare a complex analogue fo the NAIBatch as well
...
void CompERIBatch_base::compute_ssss(const double integral_thresh) {
