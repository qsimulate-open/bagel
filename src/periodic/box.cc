    ...
    for (int j = 0; j <= lmax_; ++j) {
      ...
      for (int m = 0; m <= 2 * l; ++m) {
        for (int k = 0; k <= 2 * j; ++k) {
          const int b = m - l - k + j;
          if (abs(b) <= a && a >= 0) {
            double prefactor = rr * plm0[a*a+a+b] / f(a+abs(b));
    ...
    for (int j = 0; j <= lmax_; ++j) {
      ...
      for (int m = 0; m <= 2 * l; ++m) {
        for (int k = 0; k <= 2 * j; ++k) {
          const int b = k - j - m + l;
          if (abs(b) <= a && a >= 0) {
            const double prefactor = rr * plm0[a*a+a+b] / f(a+abs(b));
            const complex<double> Oab = polar(prefactor, -b*phi);
...
vector<shared_ptr<const ZMatrix>> Box::shift_multipolesX(const vector<shared_ptr<ZMatrix>>& oa, array<double, 3> rab) const {
  ...
  for (int l = 0; l <= lmax_; ++l) {
    for (int m = 0; m <= 2 * l; ++m) {
      shared_ptr<ZMatrix> out = oa[0]->clone();
      for (int j = 0; j <= lmax_; ++j) {
        const int a = l - j;
        const double rr = pow(r, a);
        for (int k = 0; k <= 2 * j; ++k) {
...
vector<shared_ptr<const ZMatrix>> Box::shift_localLX(const vector<shared_ptr<ZMatrix>>& mr, array<double, 3> rb) const {
  ...
  for (int l = 0; l <= lmax_; ++l) {
    for (int m = 0; m <= 2 * l; ++m) {
      shared_ptr<ZMatrix> out = mr[0]->clone();
      for (int j = 0; j <= lmax_; ++j) {
        const int a = j - l;
        const double rr = pow(r, a);
        for (int k = 0; k <= 2 * j; ++k) {
