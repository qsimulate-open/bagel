...
void Box::init() {
  ...
  if (do_multiresolution_ && (nchild() == 0)) get_branches();
}
...
void Box::get_branches() {
  ...
  for (int i = 0; i != nbranch; ++i) {
    if (!br[i].empty()) {
      branch_[cnt] = make_shared<const Branch>(i, centre_, br[i]);
      nsp += br[i].size();
      ++cnt;
    }
  }
...
shared_ptr<const Matrix> Box::compute_Fock_nf_corr(shared_ptr<const Matrix> density, shared_ptr<const VectorB> max_den) const {
