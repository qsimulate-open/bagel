...
class DFDist : public ParallelDF {
  ...
  protected:
//    std::pair<const double*, std::shared_ptr<RysIntegral<std::complex<double>>>> compute_batch(std::array<std::shared_ptr<const Shell>,4>& input); // Use only for testing ComplexERIBatch
