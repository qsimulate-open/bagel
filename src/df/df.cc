...
shared_ptr<DFHalfDist> DFDist::compute_half_transform(std::shared_ptr<const View2<double>> c) const {
...
shared_ptr<DFHalfDist> DFDist::compute_half_transform_swap(std::shared_ptr<const View2<double>> c) const {
  const int nocc = c->range(1).size();
  auto out = make_shared<DFHalfDist>(shared_from_this(), nocc);
  for (auto& i : block_)
...
shared_ptr<DFFullDist> DFHalfDist::compute_second_transform(std::shared_ptr<const View2<double>> c) const {
  const int nocc = c->range(1).size();
...
shared_ptr<DFDist> DFHalfDist::back_transform(std::shared_ptr<const View2<double>> c) const{
...
shared_ptr<DFHalfDist> DFFullDist::back_transform(std::shared_ptr<const View2<double>> c) const {
