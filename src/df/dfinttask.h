...
template <typename TBatch, int N, typename DataType=double, typename BlockType=DFBlock>
class DFIntTask {
  protected:
    const std::array<std::shared_ptr<const Shell>,4> shell_;
    const std::array<int,3> offset_; // at most 3 elements
    std::array<std::shared_ptr<BlockType>,N> dfblocks_;
  ...
  public:
    DFIntTask(std::array<std::shared_ptr<const Shell>,4>&& a, std::array<int,3>&& b, std::array<std::shared_ptr<BlockType>,N>& df)
