...
class Small1e_London : public Matrix1eArray<4*Batch::Nblocks(), ZMatrix> {
  protected:
    ...
    template <int ...S>
    SmallInts1e_London<Batch, std::shared_ptr<const Molecule>, Args...> get_batch(std::array<std::shared_ptr<const Shell>,2> input, std::shared_ptr<const Molecule> mol, seq<S...>) {
      SmallInts1e_London<Batch, std::shared_ptr<const Molecule>, Args...> out(input, mol, std::get<S>(args_) ...);
      return out;
    }
    ...
    void computebatch(const std::array<std::shared_ptr<const Shell>,2>& input, const int offsetb0, const int offsetb1, std::shared_ptr<const Molecule> mol) override {
      ...
      SmallInts1e_London<Batch, std::shared_ptr<const Molecule>, Args...> batch = get_batch(input, mol, typename gens<sizeof...(Args)>::type());
      batch.compute();
