...
DKH2grad::DKH2grad(shared_ptr<const Geometry> mol) : Relgrad_base(mol) {
...
void DKH2grad::init() {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    for (int j = 0; j < natom; j++) {
      ...
      shared_ptr<VectorB> EpR = make_shared<VectorB>(*Ep % *R);
      store_mat(EpR);
      shared_ptr<VectorB> EpRX = make_shared<VectorB>(*Ep_X % *R + *Ep % *R_X);
      store_mat(EpRX);
      shared_ptr<VectorB> EpRinv = make_shared<VectorB>(*Ep % *R_inv);
      store_mat(EpRinv);
      shared_ptr<VectorB> EpRinvX = make_shared<VectorB>(*Ep_X % *R_inv + *Ep % *R_invX);
      store_mat(EpRinvX);
