...
void DKH2grad::init() {
  ...
  const Matrix T_pX = *kineticgrad();
  const Matrix s_X = *overlapgrad();
  const Matrix V_pX = *naigrad();
  const Matrix O_pX = *smallnaigrad();
  ...
  *******************************/
  ...
  store_mat(A_X);
  // shared_ptr<VectorB> K_X = make_shared<VectorB>(VectorB(nunc));
  // store_mat(K_X);
