...
//
//NOTE this does not use complex MOFile. It simply treats real MOFile outputs as a+0i. This was done for convenience at this stage.
void ZKnowlesHandy::const_denom() {
  ...
  for (int i = 0; i != norb_; ++i) {
    for (int j = 0; j <= i; ++j) {
      jop[i*norb_+j] = jop[j*norb_+i] = complex<double>(0.5*jop_->mo2e_kh(j, j, i, i), 0);
    }
  }
  for (int i = 0; i != norb_; ++i) {
    for (int j = 0; j <= i; ++j) {
      kop[i*norb_+j] = kop[j*norb_+i] = complex<double>(0.5*jop_->mo2e_kh(j, i, j, i), 0);
    }
  }
  for (int i = 0; i != norb_; ++i) {
    fk[i] = 0.0;
  ...
  denom_ = make_shared<ZCivec>(det());
  const int nspin = det()->nspin();
  const int nspin2 = nspin*nspin;

  complex<double>* iter = denom_->data();
  for (auto& ia : det()->stringa()) {
    for (auto& ib : det()->stringb()) {
      const int nopen = (ia^ib).count();
      const complex<double> F = (nopen >> 1) ? (static_cast<double>(nspin2 - nopen)/(nopen*(nopen-1))) : 0.0;
      *iter = 0.0;
      for (int i = 0; i != norb_; ++i) {
        const int nia = ia[i];
        const int nib = ib[i];
        ...
        *iter += complex<double>((jop_->mo1e(i,i) + fk[i]), 0) * niab - kop[i+norb_*i] * 0.5 * (Ni - niab*niab);
      }
      ++iter;
    }
  }
