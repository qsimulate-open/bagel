  ...
  public:
    ...
    RelDvector(std::shared_ptr<const Space_base> space, const size_t ij) : space_(space) {
      for (auto& isp : space->detmap())
        dvecs_.insert({std::make_pair(isp.second->nelea(), isp.second->neleb()),
    ...
    RelDvector(const RelDvector<DataType>& o) : space_(o.space_) {
      for (auto& i : o.dvecs_)
        dvecs_.insert({i.first, i.second->copy()});
    }
    ...
    RelDvector(std::shared_ptr<const RelDvector<DataType>> o) : space_(o->space_) {
      for (auto& i : o->dvecs_)
        dvecs_.insert({i.first, std::make_shared<Dvector<DataType>>(i.second)});
    }
    ...
    RelDvector(const std::vector<std::shared_ptr<RelDvector<DataType>>>& o) : space_(o.front()->space_) {
      for (auto& isp : space_->detmap())
        dvecs_.insert({std::make_pair(isp.second->nelea(), isp.second->neleb()), std::make_shared<Dvector<DataType>>(isp.second, o.size())});
      int j = 0;
      for (auto& i : o)
        set_data(j++, i);
    }
    ...
    std::vector<std::shared_ptr<const RelDvector<DataType>>> split(const int nstart, const int nend) const {
      ...
      for (int i = nstart; i != nend; ++i) {
        ...
        for (auto& j : dvecs_) {
          ...
          tmp.insert({j.first, std::make_shared<Dvector<DataType>>(tmp1)});
        }
