...
void ZKnowlesHandy::sigma_1(shared_ptr<const ZCivec> cc, shared_ptr<ZCivec> sigma, shared_ptr<const MOFile> jop) const {
  ...
  for (int ip = 0; ip != ij; ++ip) {
    const complex<double> h = complex<double>(jop->mo1e(ip), 0);
    for (auto& iter : cc->det()->phia(ip)) {
      const complex<double> hc = complex<double>(h.real() * iter.sign, h.imag() * iter.sign);
...
void ZKnowlesHandy::sigma_2b(shared_ptr<ZDvec> d, shared_ptr<ZDvec> e, shared_ptr<const MOFile> jop) const {
  ...
  zgemm3m_("n", "n", lenab, ij, ij, complex<double>(0.5, 0.0), d->data(), lenab, new complex<double>(*(jop->mo2e_ptr()), 0.0) , ij, complex<double>(0.0, 0.0), e->data(), lenab);
}
