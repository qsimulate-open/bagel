...
array<shared_ptr<const Matrix>,6> bagel::mblock(const Shell& shell, const double exponent) {
  ...
  if (shell.aux_decrement()) assert(ndec == (shell.aux_decrement()->angular_number()+1) * (shell.aux_decrement()->angular_number()+2) / 2);
  else assert (ndec == 0);
  ...
  for (int i=0; i!=norig; i++) {
    int x, y, z;
    size_t column = 0;
    for (int i=0; i<=angular_number; i++) {
      z = i;
      for (int j=0; j<=(angular_number-i); j++) {
        y = j;
        x = angular_number - i - j;
  ...
  if (shell.spherical()) {
    auto carsphmatrix = carsph_matrix(angular_number);
    for (int i=0; i!=6; i++)
      if (mcart[i])
...
array<shared_ptr<const ZMatrix>,9> bagel::mblock(const Shell& shell, const double exponent, const array<double,3> magnetic_field, const bool london) {
  ...
  if (shell.aux_same()) assert(norig == (shell.aux_same()->angular_number()+1) * (shell.aux_same()->angular_number()+2) / 2);
  else assert (london);
  ...
  // convert this block from cartesian to shell.spherical
  if (shell.spherical()) {
    auto carsphmatrix = make_shared<const ZMatrix> (*carsph_matrix(angular_number), 1.0);
    for (int i=0; i!=9; i++)
      if (mcart[i])
...
array<shared_ptr<const Matrix>,3> bagel::moment_compute(const Shell& shell) {
  ...
  int norig;
  int n;
  norig = ( (angular_number+1) * (angular_number+2) / 2 );
  const int ninc = norig + (angular_number + 2);
  const int ndec = norig - (angular_number + 1);
  n = ninc + ndec;
  if (shell.spherical() ) norig = 2*angular_number+1;

  assert(ninc == (shell.aux_increment()->angular_number()+1) * (shell.aux_increment()->angular_number()+2) / 2 );
  if (shell.aux_decrement()) assert(ndec == (shell.aux_decrement()->angular_number()+1) * (shell.aux_decrement()->angular_number()+2) / 2 );
  else assert(ndec == 0);
  assert(shell.aux_increment()->num_primitive() == shell.num_primitive());
  if (shell.aux_decrement()) assert(shell.aux_decrement()->num_primitive() == shell.num_primitive());
  ...
  for (int j=0; j!=shell.num_primitive(); j++) {
    auto thisblock = mblock(shell, shell.exponents(j));
    for (int i=0; i!=3; i++)
  ...
  auto contract = make_shared<Matrix>(norig*shell.num_primitive(), norig*shell.num_contracted(), true);
  for (int j=0; j!=shell.num_contracted(); j++)
    for (int k=shell.contraction_ranges(j).first; k!=shell.contraction_ranges(j).second; k++)
      for (int l=0; l!=norig; l++)
        contract->element(k*norig+l, j*norig+l) = (shell.contractions(j))[k];
...
array<shared_ptr<const ZMatrix>,3> bagel::moment_compute(const Shell& shell, const array<double,3> magnetic_field, const bool london) {
  ...
  if (shell.aux_same()) assert(nsame == (shell.aux_same()->angular_number()+1) * (shell.aux_same()->angular_number()+2) / 2 );
  else assert(nsame == 0);
  assert(shell.aux_increment()->num_primitive() == shell.num_primitive());
  if (shell.aux_decrement()) assert(shell.aux_decrement()->num_primitive() == shell.num_primitive());
  if (!london) assert(shell.aux_same()->num_primitive() == shell.num_primitive());
  else assert(!shell.aux_same());
  ...
  for (int j=0; j!=shell.num_primitive(); j++) {
    auto thisblock = mblock(shell, shell.exponents(j), magnetic_field, london);
    for (int i=0; i!=3; i++)
      tmp[i]->copy_block(j*ninc, j*norig, ninc, norig, thisblock[i]);
    if (shell.aux_decrement())
      for (int i=0; i!=3; i++)
  ...
  // build the contraction matrix
  auto contract = make_shared<ZMatrix>(norig*shell.num_primitive(), norig*shell.num_contracted(), true);
  for (int j=0; j!=shell.num_contracted(); j++)
    for (int k=shell.contraction_ranges(j).first; k!=shell.contraction_ranges(j).second; k++)
      for (int l=0; l!=norig; l++)
        contract->element(k*norig+l, j*norig+l) = (shell.contractions(j))[k];
