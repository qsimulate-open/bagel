...
ZMatrix1e::ZMatrix1e(const shared_ptr<const Molecule> mol) : ZMatrix(mol->nbasis(), mol->nbasis()) {
  zero();
}
...
class ZMatrix1eTask {
  ...
  public:
    ZMatrix1eTask(array<shared_ptr<const Shell>,2> a, size_t b, size_t c, shared_ptr<const Molecule> m, ZMatrix1e* d)
      : parent_(d), ob0(b), ob1(c), bas(a), mol(m) { }
    void compute() const { parent_->computebatch(bas, ob0, ob1, mol); }
};
}

void ZMatrix1e::init(shared_ptr<const Molecule> mol) {
  ...
  TaskQueue<ZMatrix1eTask> task(nshell*(nshell+1)/2);
