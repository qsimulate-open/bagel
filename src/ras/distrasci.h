...
#ifndef __BAGEL_RAS_DISTRASCI_H
#define __BAGEL_RAS_DISTRASCI_H
...
class DistRASCI : public Method {
  protected:
    ...
    std::shared_ptr<DistRASDvec> cc_;
    ...
    // denominator
    std::shared_ptr<DistRASCivec> denom_;

    // some init functions
    void common_init(); // may end up unnecessary
    void create_Jiiii();
    // obtain determinants for guess generation
    void generate_guess(const int nspin, const int nstate, std::shared_ptr<DistRASDvec>& out);
  ...
  public:
    ...
    DistRASCI(std::shared_ptr<const PTree>, std::shared_ptr<const Geometry>, std::shared_ptr<const Reference>);
    ...
    std::shared_ptr<const DistRASCivec> denom() const { return denom_; }
    ...
    // returns CI vectors
    std::shared_ptr<DistRASDvec> civectors() const { return cc_; }
