...
void GVRRList::_gvrr_6060(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 6; ++t)
    data_[258+t] = C00_[t] * data_[252+t] + cB00_current[t] * data_[210+t];

  for (int t = 0; t != 6; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 6; ++t)
    data_[264+t] = C00_[t] * data_[258+t] + B10_current[t] * data_[252+t] + cB00_current[t] * data_[216+t];

  for (int t = 0; t != 6; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 6; ++t)
    data_[270+t] = C00_[t] * data_[264+t] + B10_current[t] * data_[258+t] + cB00_current[t] * data_[222+t];

  for (int t = 0; t != 6; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 6; ++t)
    data_[276+t] = C00_[t] * data_[270+t] + B10_current[t] * data_[264+t] + cB00_current[t] * data_[228+t];

  for (int t = 0; t != 6; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 6; ++t)
    data_[282+t] = C00_[t] * data_[276+t] + B10_current[t] * data_[270+t] + cB00_current[t] * data_[234+t];

  for (int t = 0; t != 6; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 6; ++t)
    data_[288+t] = C00_[t] * data_[282+t] + B10_current[t] * data_[276+t] + cB00_current[t] * data_[240+t];
}
