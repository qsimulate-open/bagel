...
void GVRRList::_gvrr_5050(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 5; ++t)
    data_[125+t] = C00_[t] * data_[120+t] + cB00_current[t] * data_[90+t];

  for (int t = 0; t != 5; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 5; ++t)
    data_[130+t] = C00_[t] * data_[125+t] + B10_current[t] * data_[120+t] + cB00_current[t] * data_[95+t];

  for (int t = 0; t != 5; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 5; ++t)
    data_[135+t] = C00_[t] * data_[130+t] + B10_current[t] * data_[125+t] + cB00_current[t] * data_[100+t];

  for (int t = 0; t != 5; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 5; ++t)
    data_[140+t] = C00_[t] * data_[135+t] + B10_current[t] * data_[130+t] + cB00_current[t] * data_[105+t];

  for (int t = 0; t != 5; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 5; ++t)
    data_[145+t] = C00_[t] * data_[140+t] + B10_current[t] * data_[135+t] + cB00_current[t] * data_[110+t];
  ...
  for (int t = 0; t != 5; ++t)
    data_[155+t] = C00_[t] * data_[150+t] + cB00_current[t] * data_[120+t];

  for (int t = 0; t != 5; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 5; ++t)
    data_[160+t] = C00_[t] * data_[155+t] + B10_current[t] * data_[150+t] + cB00_current[t] * data_[125+t];

  for (int t = 0; t != 5; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 5; ++t)
    data_[165+t] = C00_[t] * data_[160+t] + B10_current[t] * data_[155+t] + cB00_current[t] * data_[130+t];

  for (int t = 0; t != 5; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 5; ++t)
    data_[170+t] = C00_[t] * data_[165+t] + B10_current[t] * data_[160+t] + cB00_current[t] * data_[135+t];

  for (int t = 0; t != 5; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 5; ++t)
    data_[175+t] = C00_[t] * data_[170+t] + B10_current[t] * data_[165+t] + cB00_current[t] * data_[140+t];
}
