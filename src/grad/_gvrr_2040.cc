...
void GVRRList::_gvrr_2040(double* data_, const double* C00_, const double* D00_, const double* B00_, const double* B01_, const double* B10_) {
  ...
  data_[36] = D00_[0] * data_[27] + B01_current[0] * data_[18];
  data_[37] = D00_[1] * data_[28] + B01_current[1] * data_[19];
  data_[38] = D00_[2] * data_[29] + B01_current[2] * data_[20];

  cB00_current[0] += B00_[0];
  cB00_current[1] += B00_[1];
  cB00_current[2] += B00_[2];

  data_[39] = C00_[0] * data_[36] + cB00_current[0] * data_[27];
  data_[40] = C00_[1] * data_[37] + cB00_current[1] * data_[28];
  data_[41] = C00_[2] * data_[38] + cB00_current[2] * data_[29];

  data_[42] = C00_[0] * data_[39] + B10_[0] * data_[36] + cB00_current[0] * data_[30];
  data_[43] = C00_[1] * data_[40] + B10_[1] * data_[37] + cB00_current[1] * data_[31];
  data_[44] = C00_[2] * data_[41] + B10_[2] * data_[38] + cB00_current[2] * data_[32];
}
