...
void GVRRList::_gvrr_20c0(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 7; ++t)
    data_[231+t] = D00_[t] * data_[210+t] + B01_current[t] * data_[189+t];

  for (int t = 0; t != 7; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 7; ++t)
    data_[238+t] = C00_[t] * data_[231+t] + cB00_current[t] * data_[210+t];

  for (int t = 0; t != 7; ++t)
    data_[245+t] = C00_[t] * data_[238+t] + B10_[t] * data_[231+t] + cB00_current[t] * data_[217+t];

  for (int t = 0; t != 7; ++t)
    B01_current[t] += B01_[t];

  for (int t = 0; t != 7; ++t)
    data_[252+t] = D00_[t] * data_[231+t] + B01_current[t] * data_[210+t];

  for (int t = 0; t != 7; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 7; ++t)
    data_[259+t] = C00_[t] * data_[252+t] + cB00_current[t] * data_[231+t];

  for (int t = 0; t != 7; ++t)
    data_[266+t] = C00_[t] * data_[259+t] + B10_[t] * data_[252+t] + cB00_current[t] * data_[238+t];
}
