...
void GVRRList::_gvrr_b050(double* data_, const double* C00_, const double* D00_, const double* B00_, const double* B01_, const double* B10_) {
  ...
  for (int t = 0; t != 8; ++t)
    data_[384+t] = D00_[t] * data_[288+t] + B01_current[t] * data_[192+t];

  for (int t = 0; t != 8; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 8; ++t)
    data_[392+t] = C00_[t] * data_[384+t] + cB00_current[t] * data_[288+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[400+t] = C00_[t] * data_[392+t] + B10_current[t] * data_[384+t] + cB00_current[t] * data_[296+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[408+t] = C00_[t] * data_[400+t] + B10_current[t] * data_[392+t] + cB00_current[t] * data_[304+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[416+t] = C00_[t] * data_[408+t] + B10_current[t] * data_[400+t] + cB00_current[t] * data_[312+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[424+t] = C00_[t] * data_[416+t] + B10_current[t] * data_[408+t] + cB00_current[t] * data_[320+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[432+t] = C00_[t] * data_[424+t] + B10_current[t] * data_[416+t] + cB00_current[t] * data_[328+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[440+t] = C00_[t] * data_[432+t] + B10_current[t] * data_[424+t] + cB00_current[t] * data_[336+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[448+t] = C00_[t] * data_[440+t] + B10_current[t] * data_[432+t] + cB00_current[t] * data_[344+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[456+t] = C00_[t] * data_[448+t] + B10_current[t] * data_[440+t] + cB00_current[t] * data_[352+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[464+t] = C00_[t] * data_[456+t] + B10_current[t] * data_[448+t] + cB00_current[t] * data_[360+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[472+t] = C00_[t] * data_[464+t] + B10_current[t] * data_[456+t] + cB00_current[t] * data_[368+t];

  for (int t = 0; t != 8; ++t)
    B01_current[t] += B01_[t];

  for (int t = 0; t != 8; ++t)
    data_[480+t] = D00_[t] * data_[384+t] + B01_current[t] * data_[288+t];

  for (int t = 0; t != 8; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 8; ++t)
    data_[488+t] = C00_[t] * data_[480+t] + cB00_current[t] * data_[384+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[496+t] = C00_[t] * data_[488+t] + B10_current[t] * data_[480+t] + cB00_current[t] * data_[392+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[504+t] = C00_[t] * data_[496+t] + B10_current[t] * data_[488+t] + cB00_current[t] * data_[400+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[512+t] = C00_[t] * data_[504+t] + B10_current[t] * data_[496+t] + cB00_current[t] * data_[408+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[520+t] = C00_[t] * data_[512+t] + B10_current[t] * data_[504+t] + cB00_current[t] * data_[416+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[528+t] = C00_[t] * data_[520+t] + B10_current[t] * data_[512+t] + cB00_current[t] * data_[424+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[536+t] = C00_[t] * data_[528+t] + B10_current[t] * data_[520+t] + cB00_current[t] * data_[432+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[544+t] = C00_[t] * data_[536+t] + B10_current[t] * data_[528+t] + cB00_current[t] * data_[440+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[552+t] = C00_[t] * data_[544+t] + B10_current[t] * data_[536+t] + cB00_current[t] * data_[448+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[560+t] = C00_[t] * data_[552+t] + B10_current[t] * data_[544+t] + cB00_current[t] * data_[456+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[568+t] = C00_[t] * data_[560+t] + B10_current[t] * data_[552+t] + cB00_current[t] * data_[464+t];
}
