...
void GVRRList::_gvrr_0040(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
...
#ifdef __GNUC__
  double B01_current[2]__attribute__((aligned(32)));
#else
  ...
  for (int t = 0; t != 2; ++t)
    data_[4+t] = D00_[t] * data_[2+t] + B01_current[t];

  for (int t = 0; t != 2; ++t)
    B01_current[t] += B01_[t];

  for (int t = 0; t != 2; ++t)
    data_[6+t] = D00_[t] * data_[4+t] + B01_current[t] * data_[2+t];

  for (int t = 0; t != 2; ++t)
    B01_current[t] += B01_[t];

  for (int t = 0; t != 2; ++t)
    data_[8+t] = D00_[t] * data_[6+t] + B01_current[t] * data_[4+t];
}
