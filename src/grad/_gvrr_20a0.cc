...
void GVRRList::_gvrr_20a0(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 6; ++t)
    data_[180+t] = D00_[t] * data_[162+t] + B01_current[t] * data_[144+t];

  for (int t = 0; t != 6; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 6; ++t)
    data_[186+t] = C00_[t] * data_[180+t] + cB00_current[t] * data_[162+t];

  for (int t = 0; t != 6; ++t)
    data_[192+t] = C00_[t] * data_[186+t] + B10_[t] * data_[180+t] + cB00_current[t] * data_[168+t];
}
