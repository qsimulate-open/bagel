...
void GVRRList::_gvrr_40c0(double* data_, const double* C00_, const double* D00_, const double* B00_, const double* B01_, const double* B10_) {
  ...
  for (int t = 0; t != 8; ++t)
    data_[440+t] = D00_[t] * data_[400+t] + B01_current[t] * data_[360+t];

  for (int t = 0; t != 8; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 8; ++t)
    data_[448+t] = C00_[t] * data_[440+t] + cB00_current[t] * data_[400+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[456+t] = C00_[t] * data_[448+t] + B10_current[t] * data_[440+t] + cB00_current[t] * data_[408+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[464+t] = C00_[t] * data_[456+t] + B10_current[t] * data_[448+t] + cB00_current[t] * data_[416+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[472+t] = C00_[t] * data_[464+t] + B10_current[t] * data_[456+t] + cB00_current[t] * data_[424+t];

  for (int t = 0; t != 8; ++t)
    B01_current[t] += B01_[t];

  for (int t = 0; t != 8; ++t)
    data_[480+t] = D00_[t] * data_[440+t] + B01_current[t] * data_[400+t];

  for (int t = 0; t != 8; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 8; ++t)
    data_[488+t] = C00_[t] * data_[480+t] + cB00_current[t] * data_[440+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[496+t] = C00_[t] * data_[488+t] + B10_current[t] * data_[480+t] + cB00_current[t] * data_[448+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[504+t] = C00_[t] * data_[496+t] + B10_current[t] * data_[488+t] + cB00_current[t] * data_[456+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[512+t] = C00_[t] * data_[504+t] + B10_current[t] * data_[496+t] + cB00_current[t] * data_[464+t];
}
