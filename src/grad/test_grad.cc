...
using namespace std;
tuple<vector<shared_ptr<Matrix1e> >, vector<shared_ptr<Matrix1e> > > grad_1e(const shared_ptr<const Geometry>);
...
tuple<vector<shared_ptr<Matrix1e> >, vector<shared_ptr<Matrix1e> > > grad_1e(const shared_ptr<const Geometry> geom) {
  ...
  return make_tuple(grad1e, grado);
}
