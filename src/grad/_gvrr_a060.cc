...
void GVRRList::_gvrr_a060(double* data_, const double* C00_, const double* D00_, const double* B00_, const double* B01_, const double* B10_) {
  ...
  for (int t = 0; t != 8; ++t)
    data_[536+t] = C00_[t] * data_[528+t] + cB00_current[t] * data_[440+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[544+t] = C00_[t] * data_[536+t] + B10_current[t] * data_[528+t] + cB00_current[t] * data_[448+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[552+t] = C00_[t] * data_[544+t] + B10_current[t] * data_[536+t] + cB00_current[t] * data_[456+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[560+t] = C00_[t] * data_[552+t] + B10_current[t] * data_[544+t] + cB00_current[t] * data_[464+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[568+t] = C00_[t] * data_[560+t] + B10_current[t] * data_[552+t] + cB00_current[t] * data_[472+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[576+t] = C00_[t] * data_[568+t] + B10_current[t] * data_[560+t] + cB00_current[t] * data_[480+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[584+t] = C00_[t] * data_[576+t] + B10_current[t] * data_[568+t] + cB00_current[t] * data_[488+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[592+t] = C00_[t] * data_[584+t] + B10_current[t] * data_[576+t] + cB00_current[t] * data_[496+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[600+t] = C00_[t] * data_[592+t] + B10_current[t] * data_[584+t] + cB00_current[t] * data_[504+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[608+t] = C00_[t] * data_[600+t] + B10_current[t] * data_[592+t] + cB00_current[t] * data_[512+t];
}
