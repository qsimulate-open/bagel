...
void GVRRList::_gvrr_00d0(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 7; ++t)
    data_[84+t] = D00_[t] * data_[77+t] + B01_current[t] * data_[70+t];

  for (int t = 0; t != 7; ++t)
    B01_current[t] += B01_[t];

  for (int t = 0; t != 7; ++t)
    data_[91+t] = D00_[t] * data_[84+t] + B01_current[t] * data_[77+t];
}
