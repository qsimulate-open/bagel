...
void GVRRList::_gvrr_40a0(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 7; ++t)
    data_[350+t] = D00_[t] * data_[315+t] + B01_current[t] * data_[280+t];

  for (int t = 0; t != 7; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 7; ++t)
    data_[357+t] = C00_[t] * data_[350+t] + cB00_current[t] * data_[315+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[364+t] = C00_[t] * data_[357+t] + B10_current[t] * data_[350+t] + cB00_current[t] * data_[322+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[371+t] = C00_[t] * data_[364+t] + B10_current[t] * data_[357+t] + cB00_current[t] * data_[329+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[378+t] = C00_[t] * data_[371+t] + B10_current[t] * data_[364+t] + cB00_current[t] * data_[336+t];
}
