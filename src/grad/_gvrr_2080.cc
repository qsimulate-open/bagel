...
void GVRRList::_gvrr_2080(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 5; ++t)
    data_[105+t] = D00_[t] * data_[90+t] + B01_current[t] * data_[75+t];

  for (int t = 0; t != 5; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 5; ++t)
    data_[110+t] = C00_[t] * data_[105+t] + cB00_current[t] * data_[90+t];

  for (int t = 0; t != 5; ++t)
    data_[115+t] = C00_[t] * data_[110+t] + B10_[t] * data_[105+t] + cB00_current[t] * data_[95+t];

  for (int t = 0; t != 5; ++t)
    B01_current[t] += B01_[t];

  for (int t = 0; t != 5; ++t)
    data_[120+t] = D00_[t] * data_[105+t] + B01_current[t] * data_[90+t];

  for (int t = 0; t != 5; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 5; ++t)
    data_[125+t] = C00_[t] * data_[120+t] + cB00_current[t] * data_[105+t];

  for (int t = 0; t != 5; ++t)
    data_[130+t] = C00_[t] * data_[125+t] + B10_[t] * data_[120+t] + cB00_current[t] * data_[110+t];
}
