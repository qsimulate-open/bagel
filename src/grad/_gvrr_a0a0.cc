...
void GVRRList::_gvrr_a0a0(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 10; ++t)
    data_[890+t] = C00_[t] * data_[880+t] + cB00_current[t] * data_[770+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[900+t] = C00_[t] * data_[890+t] + B10_current[t] * data_[880+t] + cB00_current[t] * data_[780+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[910+t] = C00_[t] * data_[900+t] + B10_current[t] * data_[890+t] + cB00_current[t] * data_[790+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[920+t] = C00_[t] * data_[910+t] + B10_current[t] * data_[900+t] + cB00_current[t] * data_[800+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[930+t] = C00_[t] * data_[920+t] + B10_current[t] * data_[910+t] + cB00_current[t] * data_[810+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[940+t] = C00_[t] * data_[930+t] + B10_current[t] * data_[920+t] + cB00_current[t] * data_[820+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[950+t] = C00_[t] * data_[940+t] + B10_current[t] * data_[930+t] + cB00_current[t] * data_[830+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[960+t] = C00_[t] * data_[950+t] + B10_current[t] * data_[940+t] + cB00_current[t] * data_[840+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[970+t] = C00_[t] * data_[960+t] + B10_current[t] * data_[950+t] + cB00_current[t] * data_[850+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[980+t] = C00_[t] * data_[970+t] + B10_current[t] * data_[960+t] + cB00_current[t] * data_[860+t];
  ...
  for (int t = 0; t != 10; ++t)
    data_[1000+t] = C00_[t] * data_[990+t] + cB00_current[t] * data_[880+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1010+t] = C00_[t] * data_[1000+t] + B10_current[t] * data_[990+t] + cB00_current[t] * data_[890+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1020+t] = C00_[t] * data_[1010+t] + B10_current[t] * data_[1000+t] + cB00_current[t] * data_[900+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1030+t] = C00_[t] * data_[1020+t] + B10_current[t] * data_[1010+t] + cB00_current[t] * data_[910+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1040+t] = C00_[t] * data_[1030+t] + B10_current[t] * data_[1020+t] + cB00_current[t] * data_[920+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1050+t] = C00_[t] * data_[1040+t] + B10_current[t] * data_[1030+t] + cB00_current[t] * data_[930+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1060+t] = C00_[t] * data_[1050+t] + B10_current[t] * data_[1040+t] + cB00_current[t] * data_[940+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1070+t] = C00_[t] * data_[1060+t] + B10_current[t] * data_[1050+t] + cB00_current[t] * data_[950+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1080+t] = C00_[t] * data_[1070+t] + B10_current[t] * data_[1060+t] + cB00_current[t] * data_[960+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1090+t] = C00_[t] * data_[1080+t] + B10_current[t] * data_[1070+t] + cB00_current[t] * data_[970+t];
  ...
  for (int t = 0; t != 10; ++t)
    data_[1110+t] = C00_[t] * data_[1100+t] + cB00_current[t] * data_[990+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1120+t] = C00_[t] * data_[1110+t] + B10_current[t] * data_[1100+t] + cB00_current[t] * data_[1000+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1130+t] = C00_[t] * data_[1120+t] + B10_current[t] * data_[1110+t] + cB00_current[t] * data_[1010+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1140+t] = C00_[t] * data_[1130+t] + B10_current[t] * data_[1120+t] + cB00_current[t] * data_[1020+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1150+t] = C00_[t] * data_[1140+t] + B10_current[t] * data_[1130+t] + cB00_current[t] * data_[1030+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1160+t] = C00_[t] * data_[1150+t] + B10_current[t] * data_[1140+t] + cB00_current[t] * data_[1040+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1170+t] = C00_[t] * data_[1160+t] + B10_current[t] * data_[1150+t] + cB00_current[t] * data_[1050+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1180+t] = C00_[t] * data_[1170+t] + B10_current[t] * data_[1160+t] + cB00_current[t] * data_[1060+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1190+t] = C00_[t] * data_[1180+t] + B10_current[t] * data_[1170+t] + cB00_current[t] * data_[1070+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1200+t] = C00_[t] * data_[1190+t] + B10_current[t] * data_[1180+t] + cB00_current[t] * data_[1080+t];
}
