...
void GVRRList::_gvrr_8060(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 7; ++t)
    data_[322+t] = C00_[t] * data_[315+t] + cB00_current[t] * data_[252+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[329+t] = C00_[t] * data_[322+t] + B10_current[t] * data_[315+t] + cB00_current[t] * data_[259+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[336+t] = C00_[t] * data_[329+t] + B10_current[t] * data_[322+t] + cB00_current[t] * data_[266+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[343+t] = C00_[t] * data_[336+t] + B10_current[t] * data_[329+t] + cB00_current[t] * data_[273+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[350+t] = C00_[t] * data_[343+t] + B10_current[t] * data_[336+t] + cB00_current[t] * data_[280+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[357+t] = C00_[t] * data_[350+t] + B10_current[t] * data_[343+t] + cB00_current[t] * data_[287+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[364+t] = C00_[t] * data_[357+t] + B10_current[t] * data_[350+t] + cB00_current[t] * data_[294+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[371+t] = C00_[t] * data_[364+t] + B10_current[t] * data_[357+t] + cB00_current[t] * data_[301+t];
  ...
  for (int t = 0; t != 7; ++t)
    data_[385+t] = C00_[t] * data_[378+t] + cB00_current[t] * data_[315+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[392+t] = C00_[t] * data_[385+t] + B10_current[t] * data_[378+t] + cB00_current[t] * data_[322+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[399+t] = C00_[t] * data_[392+t] + B10_current[t] * data_[385+t] + cB00_current[t] * data_[329+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[406+t] = C00_[t] * data_[399+t] + B10_current[t] * data_[392+t] + cB00_current[t] * data_[336+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[413+t] = C00_[t] * data_[406+t] + B10_current[t] * data_[399+t] + cB00_current[t] * data_[343+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[420+t] = C00_[t] * data_[413+t] + B10_current[t] * data_[406+t] + cB00_current[t] * data_[350+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[427+t] = C00_[t] * data_[420+t] + B10_current[t] * data_[413+t] + cB00_current[t] * data_[357+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[434+t] = C00_[t] * data_[427+t] + B10_current[t] * data_[420+t] + cB00_current[t] * data_[364+t];
}
