...
// returns double array of length 2548
void GVRRList::_gvrr_d0d0(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 13; ++t)
    data_[2366+t] = D00_[t] * data_[2184+t] + B01_current[t] * data_[2002+t];

  for (int t = 0; t != 13; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 13; ++t)
    data_[2379+t] = C00_[t] * data_[2366+t] + cB00_current[t] * data_[2184+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2392+t] = C00_[t] * data_[2379+t] + B10_current[t] * data_[2366+t] + cB00_current[t] * data_[2197+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2405+t] = C00_[t] * data_[2392+t] + B10_current[t] * data_[2379+t] + cB00_current[t] * data_[2210+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2418+t] = C00_[t] * data_[2405+t] + B10_current[t] * data_[2392+t] + cB00_current[t] * data_[2223+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2431+t] = C00_[t] * data_[2418+t] + B10_current[t] * data_[2405+t] + cB00_current[t] * data_[2236+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2444+t] = C00_[t] * data_[2431+t] + B10_current[t] * data_[2418+t] + cB00_current[t] * data_[2249+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2457+t] = C00_[t] * data_[2444+t] + B10_current[t] * data_[2431+t] + cB00_current[t] * data_[2262+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2470+t] = C00_[t] * data_[2457+t] + B10_current[t] * data_[2444+t] + cB00_current[t] * data_[2275+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2483+t] = C00_[t] * data_[2470+t] + B10_current[t] * data_[2457+t] + cB00_current[t] * data_[2288+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2496+t] = C00_[t] * data_[2483+t] + B10_current[t] * data_[2470+t] + cB00_current[t] * data_[2301+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2509+t] = C00_[t] * data_[2496+t] + B10_current[t] * data_[2483+t] + cB00_current[t] * data_[2314+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2522+t] = C00_[t] * data_[2509+t] + B10_current[t] * data_[2496+t] + cB00_current[t] * data_[2327+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2535+t] = C00_[t] * data_[2522+t] + B10_current[t] * data_[2509+t] + cB00_current[t] * data_[2340+t];
}
