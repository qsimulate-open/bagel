...
void GVRRList::_gvrr_4080(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 6; ++t)
    data_[210+t] = D00_[t] * data_[180+t] + B01_current[t] * data_[150+t];

  for (int t = 0; t != 6; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 6; ++t)
    data_[216+t] = C00_[t] * data_[210+t] + cB00_current[t] * data_[180+t];

  for (int t = 0; t != 6; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 6; ++t)
    data_[222+t] = C00_[t] * data_[216+t] + B10_current[t] * data_[210+t] + cB00_current[t] * data_[186+t];

  for (int t = 0; t != 6; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 6; ++t)
    data_[228+t] = C00_[t] * data_[222+t] + B10_current[t] * data_[216+t] + cB00_current[t] * data_[192+t];

  for (int t = 0; t != 6; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 6; ++t)
    data_[234+t] = C00_[t] * data_[228+t] + B10_current[t] * data_[222+t] + cB00_current[t] * data_[198+t];

  for (int t = 0; t != 6; ++t)
    B01_current[t] += B01_[t];

  for (int t = 0; t != 6; ++t)
    data_[240+t] = D00_[t] * data_[210+t] + B01_current[t] * data_[180+t];

  for (int t = 0; t != 6; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 6; ++t)
    data_[246+t] = C00_[t] * data_[240+t] + cB00_current[t] * data_[210+t];

  for (int t = 0; t != 6; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 6; ++t)
    data_[252+t] = C00_[t] * data_[246+t] + B10_current[t] * data_[240+t] + cB00_current[t] * data_[216+t];

  for (int t = 0; t != 6; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 6; ++t)
    data_[258+t] = C00_[t] * data_[252+t] + B10_current[t] * data_[246+t] + cB00_current[t] * data_[222+t];

  for (int t = 0; t != 6; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 6; ++t)
    data_[264+t] = C00_[t] * data_[258+t] + B10_current[t] * data_[252+t] + cB00_current[t] * data_[228+t];
}
