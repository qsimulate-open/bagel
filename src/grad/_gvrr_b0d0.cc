...
void GVRRList::_gvrr_b0d0(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 12; ++t)
    data_[1872+t] = D00_[t] * data_[1728+t] + B01_current[t] * data_[1584+t];

  for (int t = 0; t != 12; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 12; ++t)
    data_[1884+t] = C00_[t] * data_[1872+t] + cB00_current[t] * data_[1728+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1896+t] = C00_[t] * data_[1884+t] + B10_current[t] * data_[1872+t] + cB00_current[t] * data_[1740+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1908+t] = C00_[t] * data_[1896+t] + B10_current[t] * data_[1884+t] + cB00_current[t] * data_[1752+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1920+t] = C00_[t] * data_[1908+t] + B10_current[t] * data_[1896+t] + cB00_current[t] * data_[1764+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1932+t] = C00_[t] * data_[1920+t] + B10_current[t] * data_[1908+t] + cB00_current[t] * data_[1776+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1944+t] = C00_[t] * data_[1932+t] + B10_current[t] * data_[1920+t] + cB00_current[t] * data_[1788+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1956+t] = C00_[t] * data_[1944+t] + B10_current[t] * data_[1932+t] + cB00_current[t] * data_[1800+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1968+t] = C00_[t] * data_[1956+t] + B10_current[t] * data_[1944+t] + cB00_current[t] * data_[1812+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1980+t] = C00_[t] * data_[1968+t] + B10_current[t] * data_[1956+t] + cB00_current[t] * data_[1824+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1992+t] = C00_[t] * data_[1980+t] + B10_current[t] * data_[1968+t] + cB00_current[t] * data_[1836+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[2004+t] = C00_[t] * data_[1992+t] + B10_current[t] * data_[1980+t] + cB00_current[t] * data_[1848+t];
}
