...
void GVRRList::_gvrr_1070(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 4; ++t)
    data_[48+t] = D00_[t] * data_[40+t] + B01_current[t] * data_[32+t];

  for (int t = 0; t != 4; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 4; ++t)
    data_[52+t] = C00_[t] * data_[48+t] + cB00_current[t] * data_[40+t];

  for (int t = 0; t != 4; ++t)
    B01_current[t] += B01_[t];

  for (int t = 0; t != 4; ++t)
    data_[56+t] = D00_[t] * data_[48+t] + B01_current[t] * data_[40+t];

  for (int t = 0; t != 4; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 4; ++t)
    data_[60+t] = C00_[t] * data_[56+t] + cB00_current[t] * data_[48+t];
}
