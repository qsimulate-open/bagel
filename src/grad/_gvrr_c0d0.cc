...
void GVRRList::_gvrr_c0d0(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 13; ++t)
    data_[2197+t] = D00_[t] * data_[2028+t] + B01_current[t] * data_[1859+t];

  for (int t = 0; t != 13; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 13; ++t)
    data_[2210+t] = C00_[t] * data_[2197+t] + cB00_current[t] * data_[2028+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2223+t] = C00_[t] * data_[2210+t] + B10_current[t] * data_[2197+t] + cB00_current[t] * data_[2041+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2236+t] = C00_[t] * data_[2223+t] + B10_current[t] * data_[2210+t] + cB00_current[t] * data_[2054+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2249+t] = C00_[t] * data_[2236+t] + B10_current[t] * data_[2223+t] + cB00_current[t] * data_[2067+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2262+t] = C00_[t] * data_[2249+t] + B10_current[t] * data_[2236+t] + cB00_current[t] * data_[2080+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2275+t] = C00_[t] * data_[2262+t] + B10_current[t] * data_[2249+t] + cB00_current[t] * data_[2093+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2288+t] = C00_[t] * data_[2275+t] + B10_current[t] * data_[2262+t] + cB00_current[t] * data_[2106+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2301+t] = C00_[t] * data_[2288+t] + B10_current[t] * data_[2275+t] + cB00_current[t] * data_[2119+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2314+t] = C00_[t] * data_[2301+t] + B10_current[t] * data_[2288+t] + cB00_current[t] * data_[2132+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2327+t] = C00_[t] * data_[2314+t] + B10_current[t] * data_[2301+t] + cB00_current[t] * data_[2145+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2340+t] = C00_[t] * data_[2327+t] + B10_current[t] * data_[2314+t] + cB00_current[t] * data_[2158+t];

  for (int t = 0; t != 13; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 13; ++t)
    data_[2353+t] = C00_[t] * data_[2340+t] + B10_current[t] * data_[2327+t] + cB00_current[t] * data_[2171+t];
}
