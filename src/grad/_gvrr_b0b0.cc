...
void GVRRList::_gvrr_b0b0(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 11; ++t)
    data_[1463+t] = C00_[t] * data_[1452+t] + cB00_current[t] * data_[1320+t];

  for (int t = 0; t != 11; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 11; ++t)
    data_[1474+t] = C00_[t] * data_[1463+t] + B10_current[t] * data_[1452+t] + cB00_current[t] * data_[1331+t];

  for (int t = 0; t != 11; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 11; ++t)
    data_[1485+t] = C00_[t] * data_[1474+t] + B10_current[t] * data_[1463+t] + cB00_current[t] * data_[1342+t];

  for (int t = 0; t != 11; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 11; ++t)
    data_[1496+t] = C00_[t] * data_[1485+t] + B10_current[t] * data_[1474+t] + cB00_current[t] * data_[1353+t];

  for (int t = 0; t != 11; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 11; ++t)
    data_[1507+t] = C00_[t] * data_[1496+t] + B10_current[t] * data_[1485+t] + cB00_current[t] * data_[1364+t];

  for (int t = 0; t != 11; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 11; ++t)
    data_[1518+t] = C00_[t] * data_[1507+t] + B10_current[t] * data_[1496+t] + cB00_current[t] * data_[1375+t];

  for (int t = 0; t != 11; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 11; ++t)
    data_[1529+t] = C00_[t] * data_[1518+t] + B10_current[t] * data_[1507+t] + cB00_current[t] * data_[1386+t];

  for (int t = 0; t != 11; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 11; ++t)
    data_[1540+t] = C00_[t] * data_[1529+t] + B10_current[t] * data_[1518+t] + cB00_current[t] * data_[1397+t];

  for (int t = 0; t != 11; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 11; ++t)
    data_[1551+t] = C00_[t] * data_[1540+t] + B10_current[t] * data_[1529+t] + cB00_current[t] * data_[1408+t];

  for (int t = 0; t != 11; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 11; ++t)
    data_[1562+t] = C00_[t] * data_[1551+t] + B10_current[t] * data_[1540+t] + cB00_current[t] * data_[1419+t];

  for (int t = 0; t != 11; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 11; ++t)
    data_[1573+t] = C00_[t] * data_[1562+t] + B10_current[t] * data_[1551+t] + cB00_current[t] * data_[1430+t];
}
