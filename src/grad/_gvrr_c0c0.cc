...
void GVRRList::_gvrr_c0c0(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 12; ++t)
    data_[1728+t] = C00_[t] * data_[1716+t] + cB00_current[t] * data_[1560+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1740+t] = C00_[t] * data_[1728+t] + B10_current[t] * data_[1716+t] + cB00_current[t] * data_[1572+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1752+t] = C00_[t] * data_[1740+t] + B10_current[t] * data_[1728+t] + cB00_current[t] * data_[1584+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1764+t] = C00_[t] * data_[1752+t] + B10_current[t] * data_[1740+t] + cB00_current[t] * data_[1596+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1776+t] = C00_[t] * data_[1764+t] + B10_current[t] * data_[1752+t] + cB00_current[t] * data_[1608+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1788+t] = C00_[t] * data_[1776+t] + B10_current[t] * data_[1764+t] + cB00_current[t] * data_[1620+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1800+t] = C00_[t] * data_[1788+t] + B10_current[t] * data_[1776+t] + cB00_current[t] * data_[1632+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1812+t] = C00_[t] * data_[1800+t] + B10_current[t] * data_[1788+t] + cB00_current[t] * data_[1644+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1824+t] = C00_[t] * data_[1812+t] + B10_current[t] * data_[1800+t] + cB00_current[t] * data_[1656+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1836+t] = C00_[t] * data_[1824+t] + B10_current[t] * data_[1812+t] + cB00_current[t] * data_[1668+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1848+t] = C00_[t] * data_[1836+t] + B10_current[t] * data_[1824+t] + cB00_current[t] * data_[1680+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1860+t] = C00_[t] * data_[1848+t] + B10_current[t] * data_[1836+t] + cB00_current[t] * data_[1692+t];
  ...
  for (int t = 0; t != 12; ++t)
    data_[1884+t] = C00_[t] * data_[1872+t] + cB00_current[t] * data_[1716+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1896+t] = C00_[t] * data_[1884+t] + B10_current[t] * data_[1872+t] + cB00_current[t] * data_[1728+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1908+t] = C00_[t] * data_[1896+t] + B10_current[t] * data_[1884+t] + cB00_current[t] * data_[1740+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1920+t] = C00_[t] * data_[1908+t] + B10_current[t] * data_[1896+t] + cB00_current[t] * data_[1752+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1932+t] = C00_[t] * data_[1920+t] + B10_current[t] * data_[1908+t] + cB00_current[t] * data_[1764+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1944+t] = C00_[t] * data_[1932+t] + B10_current[t] * data_[1920+t] + cB00_current[t] * data_[1776+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1956+t] = C00_[t] * data_[1944+t] + B10_current[t] * data_[1932+t] + cB00_current[t] * data_[1788+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1968+t] = C00_[t] * data_[1956+t] + B10_current[t] * data_[1944+t] + cB00_current[t] * data_[1800+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1980+t] = C00_[t] * data_[1968+t] + B10_current[t] * data_[1956+t] + cB00_current[t] * data_[1812+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[1992+t] = C00_[t] * data_[1980+t] + B10_current[t] * data_[1968+t] + cB00_current[t] * data_[1824+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[2004+t] = C00_[t] * data_[1992+t] + B10_current[t] * data_[1980+t] + cB00_current[t] * data_[1836+t];

  for (int t = 0; t != 12; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 12; ++t)
    data_[2016+t] = C00_[t] * data_[2004+t] + B10_current[t] * data_[1992+t] + cB00_current[t] * data_[1848+t];
}
