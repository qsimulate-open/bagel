...
void GVRRList::_gvrr_b090(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 10; ++t)
    data_[1090+t] = C00_[t] * data_[1080+t] + cB00_current[t] * data_[960+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1100+t] = C00_[t] * data_[1090+t] + B10_current[t] * data_[1080+t] + cB00_current[t] * data_[970+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1110+t] = C00_[t] * data_[1100+t] + B10_current[t] * data_[1090+t] + cB00_current[t] * data_[980+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1120+t] = C00_[t] * data_[1110+t] + B10_current[t] * data_[1100+t] + cB00_current[t] * data_[990+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1130+t] = C00_[t] * data_[1120+t] + B10_current[t] * data_[1110+t] + cB00_current[t] * data_[1000+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1140+t] = C00_[t] * data_[1130+t] + B10_current[t] * data_[1120+t] + cB00_current[t] * data_[1010+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1150+t] = C00_[t] * data_[1140+t] + B10_current[t] * data_[1130+t] + cB00_current[t] * data_[1020+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1160+t] = C00_[t] * data_[1150+t] + B10_current[t] * data_[1140+t] + cB00_current[t] * data_[1030+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1170+t] = C00_[t] * data_[1160+t] + B10_current[t] * data_[1150+t] + cB00_current[t] * data_[1040+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1180+t] = C00_[t] * data_[1170+t] + B10_current[t] * data_[1160+t] + cB00_current[t] * data_[1050+t];

  for (int t = 0; t != 10; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 10; ++t)
    data_[1190+t] = C00_[t] * data_[1180+t] + B10_current[t] * data_[1170+t] + cB00_current[t] * data_[1060+t];
}
