...
void GVRRList::_gvrr_20d0(double* data_, const double* C00_, const double* D00_, const double* B00_, const double* B01_, const double* B10_) {
  ...
  for (int t = 0; t != 8; ++t)
    data_[312+t] = D00_[t] * data_[288+t] + B01_current[t] * data_[264+t];

  for (int t = 0; t != 8; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 8; ++t)
    data_[320+t] = C00_[t] * data_[312+t] + cB00_current[t] * data_[288+t];

  for (int t = 0; t != 8; ++t)
    data_[328+t] = C00_[t] * data_[320+t] + B10_[t] * data_[312+t] + cB00_current[t] * data_[296+t];
}
