...
void GVRRList::_gvrr_1030(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
...
#ifdef __GNUC__
  double cB00_current[2]__attribute__((aligned(32)));
#else
  ...
  for (int t = 0; t != 2; ++t)
    data_[14+t] = C00_[t] * data_[12+t] + cB00_current[t] * data_[8+t];
}
