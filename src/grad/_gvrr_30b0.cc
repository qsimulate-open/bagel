...
void GVRRList::_gvrr_30b0(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 7; ++t)
    data_[252+t] = D00_[t] * data_[224+t] + B01_current[t] * data_[196+t];

  for (int t = 0; t != 7; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 7; ++t)
    data_[259+t] = C00_[t] * data_[252+t] + cB00_current[t] * data_[224+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[266+t] = C00_[t] * data_[259+t] + B10_current[t] * data_[252+t] + cB00_current[t] * data_[231+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[273+t] = C00_[t] * data_[266+t] + B10_current[t] * data_[259+t] + cB00_current[t] * data_[238+t];

  for (int t = 0; t != 7; ++t)
    B01_current[t] += B01_[t];

  for (int t = 0; t != 7; ++t)
    data_[280+t] = D00_[t] * data_[252+t] + B01_current[t] * data_[224+t];

  for (int t = 0; t != 7; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 7; ++t)
    data_[287+t] = C00_[t] * data_[280+t] + cB00_current[t] * data_[252+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[294+t] = C00_[t] * data_[287+t] + B10_current[t] * data_[280+t] + cB00_current[t] * data_[259+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[301+t] = C00_[t] * data_[294+t] + B10_current[t] * data_[287+t] + cB00_current[t] * data_[266+t];

  for (int t = 0; t != 7; ++t)
    B01_current[t] += B01_[t];

  for (int t = 0; t != 7; ++t)
    data_[308+t] = D00_[t] * data_[280+t] + B01_current[t] * data_[252+t];

  for (int t = 0; t != 7; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 7; ++t)
    data_[315+t] = C00_[t] * data_[308+t] + cB00_current[t] * data_[280+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[322+t] = C00_[t] * data_[315+t] + B10_current[t] * data_[308+t] + cB00_current[t] * data_[287+t];

  for (int t = 0; t != 7; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 7; ++t)
    data_[329+t] = C00_[t] * data_[322+t] + B10_current[t] * data_[315+t] + cB00_current[t] * data_[294+t];
}
