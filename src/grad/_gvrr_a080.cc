...
void GVRRList::_gvrr_a080(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 9; ++t)
    data_[801+t] = C00_[t] * data_[792+t] + cB00_current[t] * data_[693+t];

  for (int t = 0; t != 9; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 9; ++t)
    data_[810+t] = C00_[t] * data_[801+t] + B10_current[t] * data_[792+t] + cB00_current[t] * data_[702+t];

  for (int t = 0; t != 9; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 9; ++t)
    data_[819+t] = C00_[t] * data_[810+t] + B10_current[t] * data_[801+t] + cB00_current[t] * data_[711+t];

  for (int t = 0; t != 9; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 9; ++t)
    data_[828+t] = C00_[t] * data_[819+t] + B10_current[t] * data_[810+t] + cB00_current[t] * data_[720+t];

  for (int t = 0; t != 9; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 9; ++t)
    data_[837+t] = C00_[t] * data_[828+t] + B10_current[t] * data_[819+t] + cB00_current[t] * data_[729+t];

  for (int t = 0; t != 9; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 9; ++t)
    data_[846+t] = C00_[t] * data_[837+t] + B10_current[t] * data_[828+t] + cB00_current[t] * data_[738+t];

  for (int t = 0; t != 9; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 9; ++t)
    data_[855+t] = C00_[t] * data_[846+t] + B10_current[t] * data_[837+t] + cB00_current[t] * data_[747+t];

  for (int t = 0; t != 9; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 9; ++t)
    data_[864+t] = C00_[t] * data_[855+t] + B10_current[t] * data_[846+t] + cB00_current[t] * data_[756+t];

  for (int t = 0; t != 9; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 9; ++t)
    data_[873+t] = C00_[t] * data_[864+t] + B10_current[t] * data_[855+t] + cB00_current[t] * data_[765+t];

  for (int t = 0; t != 9; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 9; ++t)
    data_[882+t] = C00_[t] * data_[873+t] + B10_current[t] * data_[864+t] + cB00_current[t] * data_[774+t];
}
