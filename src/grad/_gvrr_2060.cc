...
void GVRRList::_gvrr_2060(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 4; ++t)
    data_[64+t] = C00_[t] * data_[60+t] + cB00_current[t] * data_[48+t];

  for (int t = 0; t != 4; ++t)
    data_[68+t] = C00_[t] * data_[64+t] + B10_[t] * data_[60+t] + cB00_current[t] * data_[52+t];
  ...
  for (int t = 0; t != 4; ++t)
    data_[76+t] = C00_[t] * data_[72+t] + cB00_current[t] * data_[60+t];

  for (int t = 0; t != 4; ++t)
    data_[80+t] = C00_[t] * data_[76+t] + B10_[t] * data_[72+t] + cB00_current[t] * data_[64+t];
}
