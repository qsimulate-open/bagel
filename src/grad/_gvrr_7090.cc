...
void GVRRList::_gvrr_7090(double* data_, const double* C00_, const double* D00_, const double* B00_, const double* B01_, const double* B10_) {
  ...
  for (int t = 0; t != 8; ++t)
    data_[576+t] = D00_[t] * data_[512+t] + B01_current[t] * data_[448+t];

  for (int t = 0; t != 8; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 8; ++t)
    data_[584+t] = C00_[t] * data_[576+t] + cB00_current[t] * data_[512+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[592+t] = C00_[t] * data_[584+t] + B10_current[t] * data_[576+t] + cB00_current[t] * data_[520+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[600+t] = C00_[t] * data_[592+t] + B10_current[t] * data_[584+t] + cB00_current[t] * data_[528+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[608+t] = C00_[t] * data_[600+t] + B10_current[t] * data_[592+t] + cB00_current[t] * data_[536+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[616+t] = C00_[t] * data_[608+t] + B10_current[t] * data_[600+t] + cB00_current[t] * data_[544+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[624+t] = C00_[t] * data_[616+t] + B10_current[t] * data_[608+t] + cB00_current[t] * data_[552+t];

  for (int t = 0; t != 8; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 8; ++t)
    data_[632+t] = C00_[t] * data_[624+t] + B10_current[t] * data_[616+t] + cB00_current[t] * data_[560+t];
}
