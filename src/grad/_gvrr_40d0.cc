...
void GVRRList::_gvrr_40d0(double* data_, const double* C00, const double* D00, const double* B00, const double* B01, const double* B10) {
#ifdef __GNUC__
  ...
  for (int t = 0; t != 9; ++t)
    data_[540+t] = D00_[t] * data_[495+t] + B01_current[t] * data_[450+t];

  for (int t = 0; t != 9; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 9; ++t)
    data_[549+t] = C00_[t] * data_[540+t] + cB00_current[t] * data_[495+t];

  for (int t = 0; t != 9; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 9; ++t)
    data_[558+t] = C00_[t] * data_[549+t] + B10_current[t] * data_[540+t] + cB00_current[t] * data_[504+t];

  for (int t = 0; t != 9; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 9; ++t)
    data_[567+t] = C00_[t] * data_[558+t] + B10_current[t] * data_[549+t] + cB00_current[t] * data_[513+t];

  for (int t = 0; t != 9; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 9; ++t)
    data_[576+t] = C00_[t] * data_[567+t] + B10_current[t] * data_[558+t] + cB00_current[t] * data_[522+t];

  for (int t = 0; t != 9; ++t)
    B01_current[t] += B01_[t];

  for (int t = 0; t != 9; ++t)
    data_[585+t] = D00_[t] * data_[540+t] + B01_current[t] * data_[495+t];

  for (int t = 0; t != 9; ++t)
    cB00_current[t] += B00_[t];

  for (int t = 0; t != 9; ++t)
    data_[594+t] = C00_[t] * data_[585+t] + cB00_current[t] * data_[540+t];

  for (int t = 0; t != 9; ++t)
    B10_current[t] = B10_[t];

  for (int t = 0; t != 9; ++t)
    data_[603+t] = C00_[t] * data_[594+t] + B10_current[t] * data_[585+t] + cB00_current[t] * data_[549+t];

  for (int t = 0; t != 9; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 9; ++t)
    data_[612+t] = C00_[t] * data_[603+t] + B10_current[t] * data_[594+t] + cB00_current[t] * data_[558+t];

  for (int t = 0; t != 9; ++t)
    B10_current[t] += B10_[t];

  for (int t = 0; t != 9; ++t)
    data_[621+t] = C00_[t] * data_[612+t] + B10_current[t] * data_[603+t] + cB00_current[t] * data_[567+t];
}
