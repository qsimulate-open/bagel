...
nbasis_(A->geom()->nbasis(), A->geom()->nbasis())
{
   ...
   ************************************************************/
   ...
   sref_ = shared_ptr<Reference>(new Reference(sgeom_, scoeff_, nclosed_, nact, nvirt ));
}
...
void Dimer::scf(multimap<string, string> idata) {
  ...
  if (localmethod != "none") {
    ...
    shared_ptr<Matrix> fock(new Fock<1>(sgeom_, sref_->hcore(), dimerdensity, dimercoeff));
