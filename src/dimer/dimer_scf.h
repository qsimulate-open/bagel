...
class DimerSCF : public SCF_base {
  ...
  public:
    DimerSCF(const std::multimap<std::string, std::string>& idata_, const std::shared_ptr<const Dimer> dimer)
      : SCF_base(idata_, dimer->sgeom(), dimer->sref()) {
    ...
    void compute() override {
      ...
      if (SCF_base::coeff_ == nullptr) {
        throw std::runtime_error("Improper use of DimerSCF. Previous references need to be provided.");
      }
      ...
      std::cout << indent << "    * DIIS with " << (density_change_ ? "density changes" : "orbital gradients") << " will be used."
                << std::endl << std::endl;
      std::cout << indent << "=== DimerRHF iteration (" + geom_->basisfile() + ") ===" << std::endl << indent << std::endl;
